...
- ⏳ **Round 6 结构化终态诊断** —— benchmark 模式只把最终结构化 `turn/end` 投影为闭合、脱敏的模型/运行时原因族；per-session 仲裁最多写一次，且 bridge/conformance 专项原因优先于后续 generic runtime。恢复成功的 transient retry 不写失败；message、raw stderr、路径、prompt、request id 与凭证不跨控制通道。本轮只改善归因，不改 retry、prompt、tools、evaluator 或 score，新的冻结 treatment evidence 仍待运行。
