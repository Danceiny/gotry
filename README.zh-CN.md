...
- 既有 Booking 搜索失效机制（#468）：revision 增加会清除依赖它的酒店与报价选择；revision 不变时不能变更这些字段；缺失的可选字段仍可比较。持续运行时证明覆盖 `search.run` 与 `search.patch`，包括拒绝后仍保留 pending action。真实 Booking UAT 仍在 #142 开放。
