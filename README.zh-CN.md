...
为什么源码模式先装 root/TS 闭包并构建 dist:源码入口与 npm 包都解析 root 锁定的 DSH alpha.3；源码普通运行的 dsh cwd 仍是 `ts/dsh-runtime/`，因此状态继续落 `ts/dsh-runtime/gotry-state/`；benchmark opt-in 与 npm 包运行使用调用目录隔离。`ts/dsh-runtime/` 的旧 vendored runtime 只保留为非 benchmark legacy 解析兼容，不承诺可运行，也不再是推荐安装路径。
