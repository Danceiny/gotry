#!/usr/bin/env node
/** Build a self-contained, production gotry-backend release from clean HEAD.
 *
 * 与 build-booking-copilot-release.mjs 逐字同构,仅入口常量不同:
 *   bin = gotry-backend.js(同时携带 booking-copilot bin 供切换期共存)
 *   启动自检 import = dist/src/gotry-backend.js(模块注册表面,import 安全无副作用)
 *   provenance = gotry.backend.release-provenance.v1
 * 新能力做成 gotry 模块后,发布物由本脚本产出(单一 gotry-backend 服务)。 */
...
{ const committedBuilder = execFileSync('git', [...gitArgs, 'show', `${artifactId}:scripts/build-gotry-backend-release.mjs`], { cwd: ROOT, env: childEnv })
...
const temp = join(parent, `.gotry-backend-release-${process.pid}-${Date.now()}`)
...
try {
  ...
  const runtimePackageJson = {
    ...
    type: 'module',
    bin: { 'gotry-backend': 'bin/gotry-backend.js', 'gotry-booking-copilot': 'bin/gotry-booking-copilot.js' },
  ...
  execFileSync('/usr/bin/env', ['node', '--input-type=module', '-e',
    `await import(${JSON.stringify('file://' + join(release, 'dist/src/gotry-backend.js'))})`], { env: childEnv })
