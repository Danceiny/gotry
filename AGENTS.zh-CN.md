...
- 文档纪律：单一文件承载单一关注点，版本历史归 git，不设 vN 后缀；GoTry 文档一律放本仓；目录税则/命名/头部块/生命周期/可读性纪律与总索引见 `docs/README.md`。**双语纪律（2026-09-10 founder 拍板，loopx 约定）**：每篇文档成对维护——英文基座 `x.md` + 中文镜像 `x.zh-CN.md`（机器生成的 `CHANGELOG.md` 豁免）；改任何一侧必须同提交同步另一侧，**双语不一致视为 bug**；`node scripts/check-docs-i18n.mjs` 是机械校验，已接入 `run-all-tests.sh` 回归闸。**中文标点（硬性规则，2026-09-10 founder 拍板）**：中文文档散文一律用中文全角标点（，；：！？（）「」）——代码块、行内码、链接目标、HTML 标签、URL 与代码字面量（如 `node:sqlite`、`ok:false`、`[实时API:open-meteo]`）除外。
- 状态同步纪律：只更新 `architecture.md` §11 所列且事实确实变化的专职权威面，其他文档保留短指针。把 issue 段落重复塞入 README、路线图、架构与历史设计属于文档缺陷。M-exit 提交在提交信息中附保鲜清单勾稽结果。
...
- 对外发布纪律（2026-08-28 founder 改为确认制）：**发不发、发哪个版本由 founder 确认；确认后打 tag / 推 remote / npm 发布 / 发版说明由执行 agent 完成，无需 owner 亲为**。npm publish 与 dist-tag 写操作受账号级 2FA 保护——**标准动作 = 发布时一次浏览器 approve，由 founder 点击**（bypass-2FA token 正被 npm 收紧，details 见 `docs/tokens.md`）；dist-tag 维护（rc tag 迁移/deprecate/杂散清理）在同一次授权窗口内顺手完成（#50③）。发布命令 dist-tag 必须显式传（`TAG=latest ./scripts/publish-npm.sh`，#50①）。发布前必过发布闸——① 全栈回归绿 ② 按 `architecture.md` §11 对账受影响权威面 ③ README 用法逐条实测（npm 形态必须真装真跑插件加载，web 200 不算数）④ License 明确 ⑤ 版本号在 tag 与全部文档间一致；发布后必须从 registry 实测回拉验证（npm view dist-tags + 干净安装）——**回拉未做不得在任何权威面写「已发布」**（rc.16 教训：tag 已推、文档宣称已发布，registry 实无此版本，#50 勘误）。未经 founder 确认不得发布；未过闸不得宣称「已发布」。
