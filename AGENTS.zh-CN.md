...
- 文档纪律:单一文件承载单一关注点,版本历史归 git,不设 vN 后缀;GoTry 文档一律放本仓;目录税则/命名/头部块/生命周期/可读性纪律与总索引见 `docs/README.md`。**双语纪律(2026-09-10 founder 拍板,loopx 约定)**:每篇文档成对维护——英文基座 `x.md` + 中文镜像 `x.zh-CN.md`(机器生成的 `CHANGELOG.md` 豁免);改任何一侧必须同提交同步另一侧,**双语不一致视为 bug**;`node scripts/check-docs-i18n.mjs` 是机械校验,已接入 `run-all-tests.sh` 回归闸。
