...
6. **Unverifiable means blocked.** The fact gate refuses to deliver any itinerary whose bookable claims cannot trace to exact-date tool results — it is never presented as a verified plan. Policy rows (#359) are full-content fingerprinted against the registered `PolicyFact`: anchored policy lines are compared verbatim (subject/statement/source/fetched_at/query_id/as_of), so any tamper while `fact_id` and `as_of` are preserved still fails closed as `fact_anchor_unknown` — the legitimate `review_by` and `tripStart`-derived reminder are both preserved.
...
- **Policy anchor full-content fingerprint (#359)** — `renderPolicyFact` and the fact gate share the same `policyCanonicalBody` constructor: anchored policy lines are full-content compared (subject/statement/source/fetched_at/query_id/as_of), so any tamper while `fact_id` and `as_of` are preserved still fails closed as `fact_anchor_unknown`. Legitimate `review_by` and `tripStart`-derived reminders both pass. `fact-gate-tests §14` (14 assertions, all deterministic synthetic) plus `scripts/policy-anchor-359-e2e.ts` walking the real fact-log path (`appendFacts` → `loadFactRegistry`) prove the registered fact-gate definition only; this slice does not close parent #273, claim live policy/supplier correctness, or open M5/M6.
