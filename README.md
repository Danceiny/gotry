...
- **External benchmark generalization** — every frozen external run to date remains diagnostic-only (no score, no uplift claim). Round 8 adds a typed, recoverable bridge-tool/result contract with nonempty output-key coverage, exact domain/failure separation, and a latest-response terminal fence; offline unit/E2E evidence exists, but its frozen treatment has not run, and provider routing, scorer, evaluator, and default product mode are unchanged. The round-by-round engineering ledger lives in [`docs/benchmark-environment-bridge.md`](docs/benchmark-environment-bridge.md)
