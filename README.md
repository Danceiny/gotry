...
- ⏳ **External benchmark generalization / Phase 1 bridge** — the Round 1 treatment-attested installed GoTry run reached the frozen ChinaTravel cases: exact DeepSeek was environment-unavailable/schema-invalid (score 0), while GLM timed out at 300 s. Round 2's default-off owner-local bridge passed unit and source/installed-package synthetic E2E; its config requires fixed executable/cwd/argv allowlist, bounded output/timeout, and host/owner enforcement of write/network denial, and can enforce per-tool positive visible-output key contracts via `allowed_output_keys`. Opt-in is cold-start only: each benchmark agent is forced to native presentation and exactly the original bridge definition; final-assembly or downstream pre-step surface drift fails closed. Agent disposal releases scoped effects; plugin unload leaves a live agent fail-closed and quarantined until agent/process disposal, so HMR cannot hot-attach to it. The frozen ChinaTravel treatment is still pending; no uplift or external benchmark closure is claimed. See [`docs/benchmark-environment-bridge.md`](docs/benchmark-environment-bridge.md).
