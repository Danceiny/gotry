...
| **A2A face (M2)** | `gotry a2a` | A2A v1.0-subset server: agent card + JSON-RPC message/send, tasks/get/cancel; Bearer API key (GOTRY_A2A_API_KEY, fail-closed); user-token passthrough; per-IP rate limit (GOTRY_A2A_RATE_LIMIT, default 30/min) + GET /a2a/metrics; live conversation via headless driver per message (honest failure without a valid LLM key) |
