...
- **Fact gate** — pre-delivery check on itinerary artifacts: every bookable claim (flight no. / time / airport / price / policy) must trace to an exact-date tool result; unverifiable means blocked — never presented as a verified plan. Render primitives attach a typed anchor to each fact row so the gate verifies a deterministic fact_id (and for policies, that the rendered `as_of` agrees with the registry) — hand-edited or unknown anchors fail closed. For flight/train rows, a comparable hard CNY price mismatch is `price_contradicted` and blocked; fuzzy/start prices, other currencies, and hotel `priceRaw` are not guessed or compared ([#300](https://github.com/Danceiny/gotry/issues/300), parent [#273](https://github.com/Danceiny/gotry/issues/273)).
