...
- **IANA tz offset replaces hand-written residual (#343)** — flight pack v2 goes through `ts/src/tz-resolver.ts` (cached `Intl.DateTimeFormat`, no new deps); DST gap/overlap and unknown zones are rejected at the parse boundary; the home-zone work window uses `readWallParts(homeZone, depUtcMs)` to project a known instant directly into the home-zone wall clock (authoritative), no longer treating the dep-local date as the home-zone date. `ts/src/model.ts`'s `doorToDoorFromMove` is the single canonical d2d arithmetic (v2 uses real UTC instant; v1 preserves byte-for-byte `(arrMin-depMin)-tzOffsetMin`). Current local evidence is 33 focused cases plus 18 sanity cases, including the 3 root counterexamples; `data/flights_2026.json` v1 stays 4/4 compatible. Real-entry adapters merge only profile schedule fields onto a v2 pack window and retain its `homeZone`; #343 real provider/session calibration remains TODO. Public path is offline fixtures only — not live schedules/prices/inventory; the foreground full suite remains a merge TODO.
