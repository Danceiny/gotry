...
6. **Unverifiable means blocked.** The fact gate refuses to deliver any itinerary whose bookable claims cannot trace to exact-date tool results — it is never presented as a verified plan. Policy rows are full-content fingerprinted against the registered PolicyFact: anchored lines must equal the renderer's canonical line exactly (single `<!-- fact:<id> -->`, no prefix/suffix, no duplicate anchor), so any tamper while `fact_id` and `as_of` are preserved still fails closed as `fact_anchor_unknown`. Legitimate `review_by` and `tripStart`-derived reminders both pass; the public tool derives `tripStart` from the already-loaded itinerary `trip_start`, not from a caller-supplied test-only field.
...
- **Policy anchor full-content fingerprint** — when a policy row contains a `<!-- fact:` anchor, it must equal the renderer's canonical line exactly and carry exactly one canonical `<!-- fact:<id> -->`; the registered `gotry_fact_gate` executes against this rule. Malformed/duplicated anchors, non-canonical prefix/suffix, or content drift with the registered PolicyFact while `fact_id` and `as_of` are preserved still fail closed as `fact_anchor_unknown`. Policy rows that contain no `<!-- fact:` token at all fall under the broader unanchored/free-text heuristic path and remain out of scope for this slice — left to parent #273/D-26. Legitimate `review_by` and `tripStart`-derived reminders both pass; `tripStart` is derived from the already-loaded itinerary `trip_start`. Deterministic synthetic / isolated evidence only; does not close parent #273, claim live policy/supplier correctness, or open M5/M6.
- **Flight/hotel anchor field fingerprint ([#363](https://github.com/Danceiny/gotry/issues/363), parent [#273](https://github.com/Danceiny/gotry/issues/273))** — anchored flight rows are re-tokenized for `flight_no` and must equal `fact.flight_no` (uppercase-normalised); anchored hotel rows must literally contain `fact.destination`, `fact.check_in`, and `fact.check_out`. Any drift fails closed as `fact_anchor_unknown` (same kind as unknown anchors). **Field fingerprint does not replace whole-line strict comparison** (root contract clause 2): comparable flight_no / carrier / time / route / price, train canonical, policy canonical, §4b price-compatible forms (total budget / baggage fee / grand total / explicit fare label / thousands separators / 待询 / `¥7xx` / approximate fares), and `price_contradicted` / `unverified_price_claim` classifications are not collapsed — they keep the existing verdict primitives. Borrowed / unknown / duplicate anchors remain the existing `fact_anchor_unknown` path; the field fingerprint only closes the remaining root counterexample where `anchor / registry / fact_id` survive while the visible `flight_no / destination / dates` are tampered with. No new code path is added to the registered `gotry_fact_gate` execute; it reuses `loadFactRegistry` + `gateArtifact`. The fact log uses real append-only JSONL (`appendFacts`) and a temporary isolated `stateRoot` for the registered execute fixture. Deterministic synthetic / isolated evidence only; does not close parent #273, claim live supplier/12306/policy correctness, or open M5/M6.
