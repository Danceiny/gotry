...
- **External benchmark generalization** — every frozen external run to date remains diagnostic-only (no score, no uplift claim). Round 8 adds a typed, recoverable bridge-tool/result contract with offline unit/E2E evidence; its frozen treatment has not run yet, and provider routing, scorer, evaluator, and default product mode are unchanged. The round-by-round engineering ledger lives in [`docs/benchmark-environment-bridge.md`](docs/benchmark-environment-bridge.md)
