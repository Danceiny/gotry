...
**当前形态(M2 已退出,dsh 成品可用)**:一键入口 `./gotry`(DeepSeek 原生,人格+五工具:可行性/骨架/酒店/动机/愿望池);机票数据三层组合(OpenFlights 骨架 168 对/OpenSky 校验/bookedResources 锚点)+ 酒店桥(hbcli 实时/静态降级);证据链贯穿([骨架]/[实时API]/[静态包] 用户可见)。经全新场景验收(深圳→云南带爸妈:动机先行→引擎判决→证据链→选择题)。当前里程碑 = M3 最小可用产品,见 `docs/roadmap.md`。
