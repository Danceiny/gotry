...
- **Future named-year planning window (Issue #2)** — the planner and registered `gotry_feasibility_check` boundary derive the reference date from the host clock, reject past, cross-year, or undated options before solving, refuse expired-year rollover, and preserve explicit historical/backtesting intent; negated past language and conflicting years do not grant a historical bypass. Deterministic proofs use an injected clock and isolated fixtures.
