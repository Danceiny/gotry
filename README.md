...
- **Policy anchor full-content fingerprint** — anchored policy lines must equal the renderer's canonical line exactly; the registered `gotry_fact_gate` executes against this rule. Any prefix/suffix text, missing or duplicated anchor, or content mismatch with the registered PolicyFact while `fact_id` and `as_of` are preserved still fails closed as `fact_anchor_unknown`. Legitimate `review_by` and `tripStart`-derived reminders both pass; `tripStart` is derived from the already-loaded itinerary `trip_start`. Deterministic synthetic / isolated evidence only; does not close parent #273, claim live policy/supplier correctness, or open M5/M6.
