...
- **Bounded ground-transfer slice (#341)** — explicit origin/destination coordinates with `mode=driving` delegate through the registered public `map_driving_route` tool; outbound (A→B) and return (B→A) directions are bound and cached separately so a miss / error / stale entry on one side preserves only that side's estimate. Per-direction minutes attach as `minutesOut` / `minutesRet` overrides on the named static `taxi` transfer and are consumed by `evaluateChoice` without further network calls; only an exact static `taxi` destination transfer may receive route-estimated minutes, while its static price remains `[静态包:估算]`. Per-apply cache age, host-observed `asOf`, provenance, provider errors, invalid coordinates, unsupported modes, and binding mismatches are explicit; live traffic, transit/rail, and fares remain outside this slice
