...
| **Artifacts** | `gotry_artifacts_list` / `gotry_artifacts_read` | Discover & read generated artifacts (async deliverables + working-dir markdown) as a read-only, line-numbered view. The host persists structured `presentationMeta` and standard `SearchPathsResultView`/`ReadResultView` payloads; the installed DSH Web client does not key custom `gotry_artifacts_*` names to its native cards, so actual file preview uses the installed workspace/sidebar surface. Scope: `stateRoot` + the session working dir (excludes `node_modules`/`.git`); text extensions only (`md/txt/json/jsonl/csv/log/yaml/yml`); >2 MB / out-of-root / symlink-escape / unsupported extension → `ok: false` + `hint`. See [user guide](docs/user-guide.md). |
