...
- ⏳ **External benchmark generalization / Phase 1 bridge** — the Round 1 treatment-attested installed GoTry run reached the frozen ChinaTravel cases: exact DeepSeek was environment-unavailable/schema-invalid (score 0), while GLM timed out at 300 s. Round 2's default-off owner-local bridge passed unit and source/installed-package synthetic E2E; its config requires fixed executable/cwd/argv allowlist, bounded output/timeout, and host/owner enforcement of write/network denial. The frozen ChinaTravel treatment is still pending; no uplift or external benchmark closure is claimed. See [`docs/benchmark-environment-bridge.md`](docs/benchmark-environment-bridge.md).
