...
export function isBookingDateTime(value: unknown): value is string {
...
const scan = (v: unknown, path = '$'): boolean => typeof v === 'string'
  ...
  : Array.isArray(v)
    ...
    : !!v && typeof v === 'object'
      ...
      : false
export function isSafeBookingRequestKey(value: unknown): value is string { return safeRequestKey(value) }
export type BookingSurfaceValidationResult = { ok: true } | { ok: false; errors: string[] }
...
function workspaceErrors(workspace: Record<string, any> | undefined, path: string): string[] {
  ...
  if (verified !== undefined) {
    ...
    if (!safeOpaque(verified.verifiedOfferRef) || !isBookingDateTime(verified.expiresAt)) e.push(`${path}.verifiedOffer: safe ref and parseable expiry required`)
  }
  return e
}
function cross(value: unknown): string[] {
  ...
  if (x.schemaVersion === 'booking.surface' && Array.isArray(x.loadedOffers)) e.push(...workspaceErrors(x, '$'))
  ...
  if (kinds.has(String(x.kind)) && x.relaxationApprovalRef) {
    ...
    if (!safeOpaque(ref.nonce) || !isBookingDateTime(ref.expiresAt)) e.push('$.relaxationApprovalRef: invalid expiry or nonce')
  }
...
export function validateBookingSurface(value: unknown): BookingSurfaceValidationResult { const ok = validate(value); const e = ok ? [] : errors(validate); e.push(...cross(value)); return e.length ? { ok: false, errors: e } : { ok: true } }
function fromCanonical(fn: ValidateFunction, value: unknown): BookingSurfaceValidationResult { const ok = fn(value); const e = ok ? [] : errors(fn); e.push(...cross(value)); return e.length ? { ok: false, errors: e } : { ok: true } }
export function validateBookingReadAction(value: unknown): BookingSurfaceValidationResult { return fromCanonical(validateActionCanonical, value) }
