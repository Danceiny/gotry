/**
 * 行程 deck 产物生成入口(issue #566,Phase B 切片 2;研究决策见
 * docs/research/karpo-deck-web-research.md 决策 #1;deck 渲染契约见
 * docs/design/itinerary-deck-renderer.md)。
 *
 * 与单页入口 itinerary-artifact.ts 完全对称的产品路径:渲染器只把「显式行程
 * + 已选事实」投影成单文件 deck HTML;本模块是 deck 形态的**唯一产品生成
 * 路径**——从当前配置状态根的事实注册表按 id 选事实、跑 deck 渲染器、把
 * 产物独占落盘到会话工作目录。
 *
 * 三条硬纪律(继承自单页入口,verbatim):
 *   1. 事实只从 <stateRoot>/gotry-state/bookable-facts.jsonl 注册表取:入参只收
 *      fact_ids,调用方自带的事实对象一律不接收;未知/重复/超量 id 显式拒绝,
 *      登记行畸形则由 deck 渲染器的运行时校验拒绝(不静默丢弃后仍宣称成功);
 *   2. 只新建文件:basename 限定 gotry-deck-<ASCII 安全 token>.html,父目录必须是宿主
...
export const ITINERARY_DECK_ARTIFACT_BASENAME_RE = /^gotry-deck-[A-Za-z0-9][A-Za-z0-9._-]{0,63}\.html$/
/** 随机默认名(仅 crypto 十六进制;显式 basename 优先) */
export const ITINERARY_DECK_ARTIFACT_RANDOM_BYTES = 8
const RANDOM_ATTEMPTS = 3
const DIR_DENY = ['.git', 'node_modules']
const ID_SAMPLE = 3

export interface ItineraryDeckArtifactDeps {
...
export type ItineraryDeckArtifactResult =
  | {
    ok: true
...
function takeBasename(value: unknown): { basename: string } | { error: string; hint?: string } {
  if (value === undefined || value === null)
    return { basename: `gotry-deck-${randomBytes(ITINERARY_DECK_ARTIFACT_RANDOM_BYTES).toString('hex')}.html` }
  if (typeof value !== 'string' || value.length === 0) return { error: 'basename 必须是字符串' }
  if (value.includes('/') || value.includes('\\') || value.includes('..')) {
    return { error: 'basename 只能是文件名,不能含路径分隔符或 ..', hint: '形如 gotry-deck-2027-thailand.html' }
  }
  if (!ITINERARY_DECK_ARTIFACT_BASENAME_RE.test(value)) {
    return { error: 'basename 不符合命名契约', hint: '必须是 gotry-deck-<ASCII 字母/数字/._->.html(前缀后 1-64 字符)' }
  }
...
/**
 * 生成一份行程 deck HTML 产物。写入唯一目标 = 会话工作目录顶层的新文件;
 * 事实唯一来源 = deps.stateRoot 的事实注册表。
 */
export async function generateItineraryDeckArtifact(input: unknown, deps: ItineraryDeckArtifactDeps): Promise<ItineraryDeckArtifactResult> {
  ...
  for (let attempt = 0; attempt < (input.basename === undefined || input.basename === null ? RANDOM_ATTEMPTS : 1); attempt++) {
    const basename = attempt === 0 ? named.basename : `gotry-deck-${randomBytes(ITINERARY_DECK_ARTIFACT_RANDOM_BYTES).toString('hex')}.html`
