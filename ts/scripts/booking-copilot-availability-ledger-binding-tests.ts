...
const offersReceipt = (actionId: string, revision: number): ActionReceipt => ({
...
const checkReceipt = (actionId: string, revision: number): ActionReceipt => ({
...
for (const [label, mutate] of [
  ['candidate digest', (payload: Record<string, any>) => { payload.availability.candidateSetDigest = 'f'.repeat(64); payload.availabilityDigest = bookingDigest(payload.availability) }],
  ['offer set digest', (payload: Record<string, any>) => { payload.availability.hotels.h1.currentGeneration.offerSetDigest = 'e'.repeat(64); payload.availabilityDigest = bookingDigest(payload.availability) }],
  ['source receipt digest', (payload: Record<string, any>) => { payload.availability.hotels.h1.currentGeneration.source.receiptDigest = 'd'.repeat(64); payload.availabilityDigest = bookingDigest(payload.availability) }],
  ['active ordinal', (payload: Record<string, any>) => { payload.availability.activeHotelOrdinal = 1; payload.availabilityDigest = bookingDigest(payload.availability) }],
  ['fake terminal', (payload: Record<string, any>) => { payload.availability.terminal = { code: 'availability_confirmed', hotelRefs: ['h1', 'h2'], reason: 'confirmed', evidence: 'conclusive' }; payload.availability.availabilityPhase = 'terminal'; payload.availabilityDigest = bookingDigest(payload.availability) }],
] as const) {
  const fixtureState = fixture('action')
