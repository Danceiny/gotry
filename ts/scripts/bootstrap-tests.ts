...
assert.ok(c8.out.includes('FlyAI(飞猪官方检索'), '体检应含 flyai 项')
assert.ok(c8.out.includes('匿名试用') || c8.out.includes('已配置,未验证'), '体检应明确 FlyAI 当前未验证/匿名降级面')
...
// #559 B 步:开启未配置 detail 必须显式「影响面」+「检索不受影响」,否则用户以为整盘挂
assert.match(c9d.out, /影响面/, 'bootstrap doctor calendar 开启未配置=影响面行必现')
assert.match(c9d.out, /机票|酒店|地图/, 'bootstrap doctor calendar 开启未配置=必显「检索不受影响」')
