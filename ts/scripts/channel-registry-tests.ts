...
// 7. doctor 集成:flyai 配额可见 + calendar 三态
...
const d2 = await runDoctorChecks({ repoRoot: emptyRepo, homeDir: emptyHome, env: { GOTRY_ENABLE_CALENDAR: '1' } })
assert.equal(d2.items.find(i => i.id === 'calendar')!.status, 'degraded', 'opt-in 未配置=degraded')
const profileDir = join(emptyHome, '.dsh/profiles/web')
await mkdir(profileDir, { recursive: true })
await writeFile(join(profileDir, 'cordis.patch.yml'), '- id: dsh-calendar\n  config:\n    username: someone\n', 'utf-8')
const d3 = await runDoctorChecks({ repoRoot: emptyRepo, homeDir: emptyHome, env: { GOTRY_ENABLE_CALENDAR: '1' } })
assert.equal(d3.items.find(i => i.id === 'calendar')!.status, 'ok', 'opt-in 已配置=ok')
...
console.log('7. doctor 集成(flyai 配额可见 + calendar 三态)OK')
