...
// #559 B 步:flyai setup 行必须显式提到 Trial limit reached 后果,给匿名试用用户
// 一眼看见「达限后会改道 gotry_session_search」(避免装好的人替没装的人以为整盘不可用)
assert.match(base.alternatives[0]!.setup ?? '', /Trial limit reached/, 'flyai setup 行必显 Trial limit reached')
assert.match(base.alternatives[0]!.setup ?? '', /gotry_session_search/, 'flyai setup 行必显 fallback 路径')
