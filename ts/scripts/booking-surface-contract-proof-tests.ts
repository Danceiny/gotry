...
import { BOOKING_READ_ACTION_KINDS, BOOKING_GAP_CODES, BOOKING_BLOCKER_CODES, BOOKING_SURFACE_SCHEMA_SHA256, bookingSurfaceAllowedActions } from '../src/booking-surface/contracts.ts'
import { validateBookingSurface, validateBookingReadAction, validateBookingSurfaceEvent, validateCriterionBlocker, validateRelaxationApproval, validateApprovalAgainstBlocker } from '../src/booking-surface/validation.ts'

let positive = 0
let negative = 0
function accept(label: string, value: unknown) { assert.equal(validateBookingSurface(value).ok, true, label); positive++ }
...
const actions = Object.fromEntries(BOOKING_READ_ACTION_KINDS.map((kind) => [kind,{schemaVersion:'booking.surface',kind,actionId:`a-${kind.replaceAll('.','-')}`,contextRef:'ctx-1',expectedRevision:0,reason:'typed request',factRefs:[],input:inputs[kind]}])) as Record<string, Record<string, unknown>>
...
reject('Book', {...actions['search.run'],kind:'book'}); reject('root minimum', {schemaVersion:'booking.surface'}); reject('workspace nested extra', {...workspace,results:{status:'idle',open:true}}); reject('workspace capability Book', {...workspace,capabilities:{...workspace.capabilities,allowedActions:['book']}})
...
assert.equal(validateCriterionBlocker(blocker).ok,true); positive++; assert.equal(validateRelaxationApproval(approval).ok,true); positive++; assert.equal(validateApprovalAgainstBlocker(approval,blocker).ok,true); positive++
for (const field of ['sourceReceiptDigest','valueDigest']) { assert.equal(validateCriterionBlocker({...blocker,[field]:'bad'}).ok,false); negative++ }
for (const field of ['code','scope','criterionPath']) { assert.equal(validateCriterionBlocker({...blocker,[field]:field==='criterionPath'?'open.path':'dynamic'}).ok,false); negative++ }
assert.equal(validateCriterionBlocker({...blocker,evidence:{...blocker.evidence,factRefs:['fact-1','fact-1']}}).ok,false); negative++
assert.equal(validateCriterionBlocker({...blocker,evidence:{...blocker.evidence,open:true}}).ok,false); negative++
...
for (const code of BOOKING_GAP_CODES) { accept(`valid declared gap ${code}`, {...receipt, resultContract:{...receipt.resultContract, gapCodes:[code]}}); reject(`dynamic gap ${code}`, {...receipt, resultContract:{...receipt.resultContract, gapCodes:[`${code}:dynamic`]}}) }
const observations = [
...
const turn={schemaVersion:'booking.surface',kind:'user.turn',taskId:'task-1',turnId:'turn-1',workspace,request:{text:'find hotels'}}; const ingress={schemaVersion:'booking.surface',kind:'user.turn.ingress',requestKey:'request-1',taskHandle:'opaque-task-handle-1',surfaceHint:'tenant',workspace:ingressWorkspace,request:{text:'find hotels'}}; const continuation={schemaVersion:'booking.surface',kind:'action.receipt.continuation',taskId:'task-1',workspace,receipt}
accept('valid user turn',turn); accept('valid ingress',ingress); accept('valid ingress approval carries executable typed approval',{...ingress,request:{text:'relax must criterion',approval}}); accept('valid continuation',continuation); reject('empty text',{...turn,request:{text:''}}); reject('multiple approvals',{...turn,request:{text:'x',approval:[approval,approval]}}); reject('ingress multiple approvals',{...ingress,request:{text:'x',approval:[approval,approval]}}); accept('text only remains non-approval',{...turn,request:{text:'please relax must'}})
...
assert.equal(validateCriterionBlocker({...blocker, evidence:{factRefs:[],gapCodes:[]}}).ok,false); negative++
...
assert.equal(validateBookingSurfaceEvent({...questionEvent,question:{...questionEvent.question,approvalOptions:[{approval:{...approval,blockerId:'wrong'}}]}}).ok,false); negative++
assert.equal(validateBookingSurfaceEvent({...questionEvent,question:{...questionEvent.question,approvalOptions:[{approval:{...approval,taskId:'task-other'}}]}}).ok,false); negative++
assert.equal(validateBookingSurfaceEvent({...questionEvent,question:{...questionEvent.question,approvalOptions:[{approval:{...approval,contextRef:'ctx-other'}}]}}).ok,false); negative++
for (const event of events) accept(`valid event ${event.kind}`,event)
assert.equal(validateBookingSurfaceEvent({...events[5],eventId:'event-4242-4242-4242-4242'}).ok,true); positive++
assert.equal(validateBookingSurfaceEvent({...events[3],explanation:{text:'card 4111 1111 1111 1111',factRefs:[]}}).ok,false); negative++
...
assert.equal(createHash('sha256').update(readFileSync(new URL('../../schemas/booking.surface.schema.json',import.meta.url))).digest('hex'),BOOKING_SURFACE_SCHEMA_SHA256); console.log(`BOOKING SURFACE CONTRACT PROOF: positive=${positive} negative=${negative}`); assert.ok(positive>=65&&negative>=105)
