...
// gotry-inner injects the vendored absolute path (ts/dsh-runtime/node_modules/dsh-calendar/...)
// ahead of bare-name resolution, so bare names alone cannot isolate calendar here.
const blockedPattern = /(^|[\\/])dsh-calendar([\\/.]|$)|^dsh-calendar$/
const originalResolve = Module._resolveFilename
Module._resolveFilename = function (request, parent, isMain, options) {
  if (blockedPattern.test(request)) {
...
// Hide the vendored calendar entry from gotry-inner's existsSync probe so the
// calendar plugin line is dropped from the patch instead of loaded degraded
// (dsh-calendar registers its tool schemas even without CalDAV credentials).
const originalExistsSync = fs.existsSync
fs.existsSync = function (path) {
  if (typeof path === 'string' && blockedPattern.test(path)) return false
...
const proof = {
  calendarBlocked: ['dsh-calendar', 'dsh-calendar/lib/index.js'].every(request => !resolve(request)),
