...
async function assertRuntimeContract(executableOverride?: string): Promise<void> {
  ...
  assert.ok(timedOut.requests.some(r => JSON.stringify(r).includes('timed_out')), `${target} real runner deadline is enforced; requests=${timedOut.requests.length}; output=${timedOut.output.slice(-2_000)}`)
...
async function assertPackagedPatchProjection(executable: string): Promise<void> {
  ...
  try {
    ...
    await runRejectedPatch('pre-existing service injection', basePatch.replace("      config:", "      inject: [missing-service]\n      config:"))
    await runRejectedPatch('quoted service injection', basePatch.replace("      config:", "      'inject': [missing-service]\n      config:"))
