/**
 * 行程 deck 产物生成入口的注册工具 E2E(issue #566,Phase B 切片 2;
 * 与 issue #442 单页入口镜像)。
 *
 * 真实执行面:在隔离临时 stateRoot 下 `apply(ctx, config)` 装载 gotry-tools 插件,
 * 取**注册定义** `gotry_itinerary_deck_render` 的 execute 直调(与本仓库既有
 * registered-e2e 同构),exec 上下文给出真实会话 cwd。事实经 `appendFacts` 写入
 * 隔离注册表(<stateRoot>/gotry-state/bookable-facts.jsonl),生成入口只能从那里取。
 *
 * 覆盖(对照单页 itinerary-artifact-tests.ts 同一份验收面;deck 差异在 §3/§4
 * 增补的形态契约:scroll-snap 零脚本翻页 / 11 张幻灯确定性派生 / 命名契约前缀):
 *   1. 空 fact_ids = 明确未核验计划:落盘成功、无整体「已验证」徽章、无写入 stateRoot;
 *   2. 选中 3 条事实:落盘字节 == 同输入 deck 渲染器输出(事实顺序/内容均由注册表决定);
 ...
 *  10. 随机默认名符合 `gotry-deck-` 契约且两次不同;同一输入两次生成字节一致(确定性);
 ...
 *      只有兄弟存在时带空格路径按不存在拒绝;
 *  15. deck 形态契约:零脚本 / 无远端依赖 / 纯 CSS scroll-snap 翻页 / 11 张幻灯确定性派生 /
 *      共享 evidence 边界声明。
 * 全部合成事实,无用户真实行程;不启动 dsh 宿主、无网络、无供应商调用。
 *
 * 运行(在 ts/ 下):
 *   PATH=/opt/homebrew/opt/node@24/bin:$PATH npx tsx scripts/itinerary-deck-artifact-tests.ts
 */
...
import { ITINERARY_DECK_ARTIFACT_BASENAME_RE } from '../capabilities/itinerary-deck-artifact.ts'
...
const FLIGHT_ID = 'i566flight0001'
const HOTEL_ID = 'i566hotel0001'
const POLICY_ID = 'i566policy0001'
const TRAIN_ID = 'i566train00001'
...
function deckFiles(dir: string): string[] {
  return readdirSync(dir).filter(n => n.startsWith('gotry-deck-') && n.endsWith('.html')).sort()
}

function renderDeck(facts: BookableFact[]): string {
  const r = renderItineraryDeck({ title: TITLE, itinerary: ITINERARY, facts })
...
async function main(): Promise<void> {
  const home = mkdtempSync(join(tmpdir(), 'gotry-566-artifact-'))
  ...
  const tool = tools.get('gotry_itinerary_deck_render')
  assert.ok(tool, 'gotry_itinerary_deck_render 必须已注册')
  // deck 工具与单页工具并存:注册定义同名检查
  ok(Boolean(tools.get('gotry_itinerary_render')), '§0a 单页入口(issue #442)仍注册')
  if (!tool) {
    console.log('\nITINERARY DECK ARTIFACT TESTS: 0 pass, 1 fail')
    console.log('  - 工具未注册')
    process.exit(1)
  }
  ...
  ok(/DECK|deck/i.test(tool.description), '§1h 描述声明 deck 形态')
  ok(/gotry-deck-/.test(tool.description), '§1i 描述声明 deck 命名前缀')

  // ---- §2 事实登记进隔离注册表 ----
  ok(await appendFacts(stateRoot, [FLIGHT, TRAIN, HOTEL, POLICY]), '§2a 合成事实写入隔离注册表')
  const registry = await loadFactRegistry(stateRoot)
  ok(registry.length === 4, `§2b 注册表读回 4 条(实测 ${registry.length})`)
  ok(deckFiles(cwd).length === 0, '§2c 生成前工作目录无 deck html')

  // ---- §3 空 fact_ids = 明确未核验计划 + deck 形态契约 ----
  ...
  ok(deckFiles(cwd).length === 1, `§3c 落盘 1 个 deck html(实测 ${deckFiles(cwd).length})`)
  ...
  ok(emptyDisk === renderDeck([]), '§3e 落盘字节 == 同输入 deck 渲染器输出')
  ...
  ok(!emptyDisk.includes('<script'), '§3j 产物无 script(零脚本翻页)')
  ok(!emptyDisk.includes('http://') && !emptyDisk.includes('https://'), '§3k 产物无远端资源')
  // deck 形态契约(issue #564):纯 CSS scroll-snap / 11 张幻灯 / 共享 evidence 边界
  ok(emptyDisk.includes('html{scroll-snap-type:y proximity}') && emptyDisk.includes('scroll-snap-align:start'),
    '§3l 纯 CSS scroll-snap 翻页')
  // 空事实输入:政策幻灯(仅当有政策事实时存在)被跳过,余 10 张
  ok(countOf(emptyDisk, '<section class="slide') === 10, `§3m 空事实输入 10 张幻灯确定性派生(实测 ${countOf(emptyDisk, '<section class="slide')})`)
  ok(emptyDisk.includes('本文档只投影调用方给出的显式行程与已注册事实'), '§3n 共享 evidence 边界声明(单面文案逐字一致)')
  ok(readdirSync(stateRoot).filter(n => n.endsWith('.html')).length === 0, '§3o stateRoot 下不写 html(只写会话工作目录)')

  // ---- §4 选中注册表事实:字节与 deck 渲染器一致,按调用方顺序选中 ----
  ...
  ok(selectedDisk === renderDeck([FLIGHT, HOTEL, POLICY]), '§4b 落盘字节 == 注册表选出的 3 条事实的 deck 输出')
  ...
  ok(selectedDisk.includes('slide-hotel-evidence') && selectedDisk.includes('slide-policy') && selectedDisk.includes('slide-leftovers') && selectedDisk.includes('slide-flights'),
    '§4f deck 证据幻灯齐全')
  ...
  ok(reversed.ok === true && reversed.bytes === selected.bytes, '§4h 顺序变化不改变 deck 文档字节(确定性投影)')

  // ---- §5 未知 id 拒绝,零写入 ----
  const before5 = deckFiles(cwd).length
  const unknown = await run({ title: TITLE, itinerary: ITINERARY, fact_ids: ['i566notthere'] })
  ok(unknown.ok === false && String(unknown.error).includes('i566notthere'), '§5a 未知 id 显式拒绝并回显 id')
  ok(deckFiles(cwd).length === before5, '§5b 未知 id 零写入')
  ...
  ok(deckFiles(cwd).length === before5, '§6b 重复 id 零写入')

  // ---- §7 超量 id 拒绝,零写入 ----
  const many = Array.from({ length: 201 }, (_, i) => `i566bulk${i}`)
  ...
  ok(deckFiles(cwd).length === before5, '§7b 超量 id 零写入')

  // ---- §8 登记行畸形 → deck 渲染器拒绝,零写入 ----
  const malformed = { ...HOTEL, fact_id: 'i566malformed01', verdict: 'hit', options_masked: 'many' }
  ok(await appendFacts(stateRoot, [malformed as unknown as BookableFact]), '§8a 畸形登记行写入注册表')
  const malformedRun = await run({ title: TITLE, itinerary: ITINERARY, fact_ids: ['i566malformed01'] })
  ...
  ok(deckFiles(cwd).length === before5, '§8d 畸形行零写入')

  // ---- §9 调用方自带 facts 不被接受 ----
  const forgedId = 'i566forged0001'
  ...
  ok(deckFiles(cwd).length === before5 + 1, '§9d 仅 §9a 新增一个文件')

  // ---- §10 显式 basename 撞车不覆盖(deck 命名契约) ----
  const named = 'gotry-deck-2027-phuket.html'
  const first = await run({ title: TITLE, itinerary: ITINERARY, fact_ids: [FLIGHT_ID], basename: named })
  ok(first.ok === true && first.basename === named, '§10a 合法 deck basename 生成成功')
  ...
  // ---- §11 符号链接目标不被改写 ----
  const linkName = 'gotry-deck-link.html'
  ...
  // ---- §12 deck basename 路径与命名契约(前缀不同 + 与单页契约互不兼容) ----
  const before12 = deckFiles(cwd).length
  const rejectNames = [
    '../../evil.html',
    'sub/evil.html',
    'sub\\evil.html',
    ...
    'evil.html',
    'gotry-deck-.html',
    'gotry-deck-2027-phuket.HTML',
    `gotry-deck-${'a'.repeat(70)}.html`,
    'gotry-deck-2027 phuket.html',
    // 单页前缀不应被 deck 工具接受(契约互不兼容,避免文件名撞车归责混乱)
  ...
  ok(deckFiles(cwd).length === before12, `§12b 拒绝的 basename 未产生 deck 文件(实测 ${deckFiles(cwd).length})`)
  ok(!readdirSync(outside).includes('evil.html'), '§12c 仓外无越级文件')
  // 单页契约仍由 gotry_itinerary_render 单独负责(两个工具并存、契约不互串)
  ok(ITINERARY_DECK_ARTIFACT_BASENAME_RE.source.startsWith('^gotry-deck-'), '§12d 契约 regex 锁定 gotry-deck- 前缀')
  ...
  ok(ITINERARY_DECK_ARTIFACT_BASENAME_RE.test(String(r1.basename)), '§14b 随机名符合 deck 命名契约')
  ...
  // ---- §15 畸形输入拒绝,零写入 ----
  const before15 = deckFiles(cwd).length
  ...
  ok(deckFiles(cwd).length === before15, `§15b 畸形输入零写入(实测 ${deckFiles(cwd).length})`)
  ...
  for (const bad of ['', '   ', undefined as unknown as string]) {
    const r = await generateItineraryDeckArtifact({ title: TITLE, itinerary: ITINERARY, fact_ids: [] }, { stateRoot, cwd: bad })
  ...
  // ---- §17 真实产出链路:注册工具生成 → gotry_artifacts_list → gotry_artifacts_read ----
  ...
  const chainName = 'gotry-deck-e2e-chain.html'
  const chain = await run({ title: TITLE, itinerary: ITINERARY, fact_ids: [FLIGHT_ID, HOTEL_ID, TRAIN_ID], basename: chainName })
  ...
  ok(Boolean(entry), '§17c list 在册发现生成的 deck HTML(同一文件)')
  ...
  ok(read.content === renderDeck([FLIGHT, HOTEL, TRAIN]), '§17k 读回内容 == 注册表选中事实的 deck 输出')
  ...
  const viaTool = await run({ title: TITLE, itinerary: ITINERARY, fact_ids: [], basename: 'gotry-deck-spaced.html' }, spacedExec)
  ok(viaTool.ok === true, `§18a 带尾随空格的会话 cwd 生成成功(实际:${JSON.stringify(viaTool.error ?? '')})`)
  ok(String(viaTool.path) === join(realpathSync(spacedDir), 'gotry-deck-spaced.html'), '§18b 落点 = 宿主给出的带空格目录')
  ok(readdirSync(spacedDir).includes('gotry-deck-spaced.html'), '§18c 文件确实写在带空格目录内')
  ok(deckFiles(plainDir).length === 0, '§18d 同名兄弟目录零写入(工具层未归一化)')

  const viaCap = await generateItineraryDeckArtifact(
    { title: TITLE, itinerary: ITINERARY, fact_ids: [], basename: 'gotry-deck-spaced2.html' },
    { stateRoot, cwd: spacedDir },
  )
  ok(viaCap.ok === true && readdirSync(spacedDir).includes('gotry-deck-spaced2.html'), '§18e 能力层写进带空格目录')
  ok(deckFiles(plainDir).length === 0, '§18f 能力层不落到同名兄弟目录')

  const plainOnly = join(home, 'plainonly')
  mkdirSync(plainOnly, { recursive: true })
  const ghost = await generateItineraryDeckArtifact(
    { title: TITLE, itinerary: ITINERARY, fact_ids: [] },
  ...
  console.log(`\nITINERARY DECK ARTIFACT TESTS: ${pass} pass, ${fail} fail`)
  if (fail > 0) {
    console.log('\nFAILURES:')
