...
| Group | Tool | Contract |
|---|---|---|
| **Real-time retrieval (OTA/official, read-only)** | `gotry_flyai_search` | Eight public FlyAI kinds: flight, train, hotel, poi, keyword, ai, marriott-hotel and marriott-package. Public filters and legacy aliases share a flat schema. Masked prices remain strings; only exact itinerary queries can write inventory facts. Authentication/trial failures do not retry; transient upstream failures retry once. See [integration contract](design/flyai-supplier-skill-design.md). |
