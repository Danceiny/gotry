...
| Group | Tool | Contract |
|---|---|---|
| **Real-time retrieval (OTA/official, read-only)** | `gotry_flyai_search` | Eight public FlyAI kinds via the pinned `@fly-ai/flyai-cli@1.0.16`: flight, train, hotel, poi, keyword, ai, marriott-hotel and marriott-package. Public filters and legacy aliases share a flat schema. Masked prices remain strings; only exact-date transport queries or exact-stay hotel queries can write inventory facts. Authentication/trial failures do not retry; transient upstream failures retry once. See [integration contract](design/flyai-supplier-skill-design.md). |
| | `gotry_flyai_setup` | `status` inspects source and verification; `check` queries read-only and updates verification metadata. Neither accepts credentials. Set/clear credentials locally with `gotry setup flyai`; configured does not mean verified. |
