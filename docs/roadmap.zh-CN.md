...
- **外部事件接缝设计（2026-09-04，issue #119/#82 兼容方向；#432 合同切片 2026-09-12）**：`docs/design/external-event-seam.md`——外部事件作为持久健康面与愿望池召回的新生产者，消费既有接缝不建新运行时；不做 push／总线／常驻监听。第 1 段（本地探针 tick + `'ok'` 恢复事件，channel-probe.ts）与第 2 段（愿望池消费：`conditions.channels` + down 否证召回）已于 2026-09-07 落地（run-all §52／§53）；持久健康面对 routing 的传播仍归 #436，因为 `routingAdvice` 仍读取进程内 channel state。已批准的 #432 切片只新增默认关闭的 inert W2A `w2a/0.1` adapter 合同与测试（run-all §53b）：exact tuple 准入、64 KiB 上限、untrusted metadata 投射、opaque／自由文本剥离，且无 listener／auth／consumer／状态写入。真实 sensor transport、authentication 与 consumer 激活仍归 #82。
