...
- 既有搜索失效机制（#468）：更高 revision 会清除依赖它的酒店／报价选择状态；revision 不变时拒绝变更，同时保持可选缺失字段可比较。运行时证明覆盖 `search.run` 与合法的 `search.patch`；真实 Booking UAT 仍在 #142 开放。
...
- **Lavish 本地评审工具（#443，父 #438）**：宿主以 `lavishAxiPackageRoot` 显式启用五个工具；默认关闭。工具绑定精确的宿主会话与工作目录，串行处理有界未受信任反馈。终态拒绝活动命令，stop 保留自有进程清理与失败结果；插件卸载关闭注册层。行程 HTML 经既有生成、发现和源码读取入口进入评审。契约见 [Lavish 本地会话](design/lavish-local.zh-CN.md#9-注册到产品工具面)；已注册的 Lavish 浏览器反馈链由 #443 CLOSED + PR #456 merged acceptance 覆盖，原生 HTML preview 实证在 #448 已接受、持久回归落在 `ts/scripts/dsh-artifact-web-e2e.ts`（可复跑：`GOTRY_ARTIFACT_WEB_E2E_OUT=<dir> npx tsx ts/scripts/dsh-artifact-web-e2e.ts`）。此工程能力不构成 M4／M5／M6 准入。
- **行程 HTML 渲染器 + 产品生成入口（#442 内部切片，父 #438；2026-09-12）**：纯有界渲染器 `ts/src/itinerary-html.ts`（计划面与证据面分离，无 IO/`Date.now`/Markdown 解析/日期·夜数·预算算术，显式 `ok:false` 拒绝集）已可从真实产品入口到达——注册工具 `gotry_itinerary_render`（`ts/capabilities/itinerary-artifact.ts`）接收 `title`、显式行程对象与 `fact_ids`，事实**只**从当前 `stateRoot` 事实注册表取（绝不接收调用方自带事实对象；未知/重复/超量 id 与畸形登记行一律拒绝），随后在会话工作目录顶层仅**新建**一个文件（`gotry-itinerary-<ASCII token>.html`，`O_CREAT|O_EXCL`——不覆盖、不跟随符号链接逃逸、拒绝 `.git`/`node_modules`；默认 crypto 随机名）并返回最终真实路径；非法输入零字节写入。空 `fact_ids` 是合法的明确未核验计划，不存在整体「已验证」徽章。证据 = run-all §6c（`itinerary-html-tests.ts` + `itinerary-artifact-tests.ts`，隔离 `mkdtemp` stateRoot/cwd 夹具，走注册工具 execute 路径）。生成产物的浏览器覆盖（真实浏览器导航、原生 details 与 Return 键盘、窄屏可读、转义）由 #442 CLOSED + PR #449 merged acceptance 收口。原生 HTML preview sandbox 验收 = #448 proof 已接受，持久回归落在 `ts/scripts/dsh-artifact-web-e2e.ts`（可复跑：`GOTRY_ARTIFACT_WEB_E2E_OUT=<dir> npx tsx ts/scripts/dsh-artifact-web-e2e.ts`）；已注册的 Lavish 浏览器反馈链由 #443 CLOSED + PR #456 merged acceptance 覆盖；本切片不是供应商证据，也不是 M3 / M4 / M5 准入。

- **产物 list 翻页 + 字面元数据搜索（2026-09-12，issue [#458](https://github.com/Danceiny/gotry/issues/458)，接 #25/#285/#441 之上）**：`listArtifacts` 新增 `offset`（零-based 非负整数）与 `search`（字面大小写不敏感子串，匹配 `id`／`title`／filename；trim 后空串 = 不过滤；非正则／非 glob，不搜文件正文）——统一走收集 → canonical-path 去重（账本与文件视图指向同一文件时保留账本权威） → 搜索过滤 → 确定性全局排序（`updated` DESC + `(source, id, canonical path)` 词典序 tie-break，使等 mtime 的页面边界在跨主机／跨 locale 下稳定）→ 分页切片。移除此前各源提前 `slice(0, limit)` 的早裁逻辑，让旧 cwd 产物在翻页过程中仍可达，不会被单源上限静默丢弃。`limit` 保留 pre-#458 整数零／负 clamp-on-1 合同（兼容），字符串／非有限／非整 predictably 拒绝，正值封顶 50；`offset` undefined 默认 0，null／字符串／非有限／非整／负数 predictably 拒绝；`search` 非字符串 predictably 拒绝，不静默退化为「不过滤」。`total` 反映过滤后集合的真实长度；`nextOffset` 仅当 `offset + returned < total` 时出现；越界 `offset` 返回空页 + 已知 `total` + `truncated:false` + 无 `nextOffset`；model-visible summary 文案在 `total > 0` 时显式标「空页 + 已知 total」而非「无在册产物」。注册工具层与能力层共用同一组验证助手（无重复校验器）。读路径沿用 #441 行为——root／扩展名／拒绝目录／符号链接边界、读 by-id/path/window 语义、HTML 源码预览规则、canonical-vs-literal 预检修复——全部保持不变。验收证据 = `scripts/artifacts-capability-tests.ts` 25 项隔离 fixture proof（新增 19–25 覆盖 >50 跨页枚举、等 mtime 词典序 tie-break、混合账本／legacy／cwd 去重、字面搜索、无效 `offset`／`limit`／`search` 类型拒绝且无静默圆整、兼容 limit 零／负夹 1、正大值夹 50，以及分页下读边界复检）+ `scripts/dsh-artifact-e2e.ts` 注册插件 list→分页／搜索→read→Host 表现证明（>50 混合源 fixture，tie-break／去重／过滤后 total／安全参数，越界 summary 文案）。本切片关闭 #285 暴露的 M4「找上次那一份」todo，不改变任何 M3/M4/M5/M6 准入 gate。
...
- **#271 Phase A 进程事故观察（2026-09-09，历史条目，已被下方边界收口覆盖）**：GoTry-owned `uncaughtExceptionMonitor` 记录 uncaught/rejection 并保留宿主 fatal 退出语义；single-fd append+fsync+close writer 失败返回 `false`；native Node24 ESM dist 反例覆盖 monitor/no-monitor、既有 host handler、fsync/close 与工具结构化失败。本切片已随 [#271](https://github.com/Danceiny/gotry/issues/271)（2026-09-11 关闭）一并关闭；当前状态见下方 outer-liveness 与边界收口两条；不改变 M3/M4-M6 真实证据 gate。
- **#282 Booking planner 纠偏（2026-09-09；权威边界已由 #3580 取代）**：planner 保留非空 occupancy 房间与 childAges，缺 `adults` 仍为 schema-invalid。插件返回 typed `INVALID_ARGS`，只有同一个 DSH run 内稍后的 canonical call 才能纠正。跨 run 重试只保留给有界 no-tool/prose 提示，provider error 原样暴露。注入 runPort 与本地 subprocess proof 都只是工程证据，不改变 M3/M4-M6 或真实 Booking UAT gate。
- **#271 外层 dsh liveness（2026-09-10，历史条目，已被下方边界收口覆盖）**：GoTry launcher 以独立 POSIX process group 管理 dsh child；exit/error/close（包括继承 stdio 造成的 delayed close）与 parent-only SIGINT/SIGTERM 走 bounded TERM→KILL，产品 incident 在退出前同步写入，benchmark 零退出仍保留 stdout drain/parser。Node24 真实安装版本 `dsh@0.1.5-rc.1` 的维护 proof 现由实际 package-shaped `bin/gotry-inner.js` fixture 覆盖非零、正常零退出和 TERM-resistant descendant，并保留 child crash、rejected promise、spawn ENOENT、SIGINT/SIGTERM 五个既有 focused cases，核对 parent/leader outcome、stderr、incident、post-failure marker 和 before/after descendant/group 清理。不改 vendor/lock，不改变 M3/M4-M6 真实 evidence gate。
- **#271 边界收口（2026-09-11，当前状态；issue [#271](https://github.com/Danceiny/gotry/issues/271) 2026-09-11 已关闭）**：三个开放面已由有界隔离实验补齐证据（记录于 #271 评论 5638940456，脚本 SHA256 绑定）——SDK direct transport（typed `TransportClosedError`、`close()` 有界；如实非等价：无进程组、spawn-ENOENT 经 SDK 入口不可达）、内部 supervisor（激活期 throw / boot 后 late rejection 均为有界 release 后放弃、exit 1、零 respawn）、部署（打包产物与源码形态逐字节复现 inherited-pipe leader-crash 契约）。当前 SDK direct transport 仍不在产品内——其激活 watch/trigger 单独在 [#422](https://github.com/Danceiny/gotry/issues/422) 追踪；上游进程组清理增强若 SDK 直连形态进入产品再议，仍为 founder 决策候选事项。不改变 M3/M4-M6 真实 evidence gate。
...
- **#327 重复 tool-call 参数修订（2026-09-10；历史条目，已由 #3580 取代）**：这里引入的受限相同重复对象恢复现已删除。当前 Booking planner 只接受一个 canonical JSON 对象；畸形/重复/嵌套/字符串化输入一律 fail-closed，schema shape 纠偏通过 typed `INVALID_ARGS` 与同一个 DSH run 内稍后的 canonical call 完成。相关 proof 不宣称真实 provider reliability、HotelByte UAT、M3/M4 cohort 或 M5/M6 admission。
...
- **外部事件接缝设计（2026-09-04，issue #119/#82 兼容方向；#432 合同切片 2026-09-12）**：`docs/design/external-event-seam.md`——外部事件作为持久健康面与愿望池召回的新生产者，消费既有接缝不建新运行时；不做 push／总线／常驻监听。第 1 段（本地探针 tick + `'ok'` 恢复事件，channel-probe.ts）与第 2 段（愿望池消费：`conditions.channels` + down 否证召回）已于 2026-09-07 落地（run-all §52／§53）；第 3 段（持久健康面 → routing 传导，#436）：六个非 hit 工具结果在工具结果边界以严格可用时间戳口径读取最新持久健康状态、投影纯 down 集合、注入 `routingAdvice`——持久 `down` 无需任何 verdict 调用即可排除通道；坏时间戳行在 latest-wins 覆盖前丢弃；只有 `down` 会新增持久排除（`cooldown` 仍是节律态，恢复／过期只是解除该排除），且都不解除会话 down（run-all §67；静态 persona 路由卡不变）。已批准的 #432 切片只新增默认关闭的 inert W2A `w2a/0.1` adapter 合同与测试（run-all §53b）：exact tuple 准入、64 KiB 上限、untrusted metadata 投射、opaque／自由文本剥离，且无 listener／auth／consumer／状态写入。真实 sensor transport、authentication 与 consumer 激活仍归 #82。
