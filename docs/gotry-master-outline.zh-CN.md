...
矩阵结论：**复用矩阵中不再有许可证硬阻塞**——open-source（MIT/Apache）直接 import；内部资产 bridge 运行时；AGPL 项目（TREK）与内部代码（T 系统）一律照着重写。内核清单已冻结（2026-09-11，issue #234，PR 待链）：GoTry 内核模块面（引擎/账本/闸，5 个文件，宁窄勿宽，逐项带 architecture §/ADR 依据）机械化为 SHA256 冻结清单 + 运行模块证据闸（run-all §61）——内核漂移（改动/删除/绕过/未加载）即红；仅工程复用证据，M6 实施门仍以 #137 为前提。
