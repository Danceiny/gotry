...
- 多个可信扩展同时轮询同一桥时，票据名字预检与紧接着的检索绑定到领取预检作业的 Origin 和后台进程客户端标识；另一客户端不能领取检索或提交该作业的回包。标识随机生成、仅在后台进程存活期间有效：进程重启后，绑定的检索在时限内失败，不会切到另一 Chrome 配置。未上报标识的旧版扩展仍按 Origin 路由。跨次检索和默认产品路径仍须按 [#272](https://github.com/Danceiny/gotry/issues/272) 做真实核验。
