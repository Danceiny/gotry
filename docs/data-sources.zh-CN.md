...
| **地面交通（接驳/铁路）** | ⚠️ **#341 第一切片**：仅接受显式起点/终点经纬度与 `mode=driving`；`ts/capabilities/ground-transfer.ts` 通过已注册公开 `map_driving_route` 委托路线结果，只把分钟绑定到具名静态 `taxi` transfer；`bus`/`bus_plus_taxi` 不调用路线 provider，静态价格不改写 | 路线估算 + per-apply cache；**不是实时交通** | 路线事实=`map_driving_route` + `asOf`（宿主观察/查询时间，非 provider 发布时间）/freshness/cache；回退=`[静态包:估算]` | live traffic、transit/rail、fare、地址解析与更广泛 transfer 仍未接入，12306 不由本切片假设 |
...
- **M4 当前工程残余**：
  - **#341 地面接驳第一切片**：显式坐标 + `mode=driving` 通过注册的公开 `map_driving_route` 进入 deterministic planner adapter；仅静态 `taxi` transfer 接收路线分钟，`bus`/`bus_plus_taxi`、provider miss/error、过期重查失败、非法坐标和不支持模式保持静态分钟/价格并标 `[静态包:估算]`。过期缓存先重新查询；不覆盖单次 live OSRM、实时交通、transit/rail/fare 或地址解析。
