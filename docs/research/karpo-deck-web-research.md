...
|---|---|---|---|
| A | Design-system seeds, landing deck, image-card templates, fixture-driven `gotry try` | No — new surfaces only | May serve the open M3 exit |
| B | Deck spec + deck renderer (inherits the `itinerary-html.ts` contract) + static export + QR | No — render-layer extension | M3 |
