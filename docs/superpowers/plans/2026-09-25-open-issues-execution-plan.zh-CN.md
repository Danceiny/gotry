...
> 定位：为当前 30 个开放问题提出执行顺序、剩余工作与验收条件；不新增里程碑权威。
> 状态：第一波工程实现已交付，本地全量回归通过；待合并及剩余真实验收。核查日期为 2026-09-25。
...
| Issue | 剩余工作或分类 | 执行去向 |
|---|---|---|
| [#564](https://github.com/Danceiny/gotry/issues/564) | 渲染器经 #565 合入 | T0 收口候选 |
| [#566](https://github.com/Danceiny/gotry/issues/566) | 产品渲染入口经 #567 合入 | T0 收口候选 |
| [#568](https://github.com/Danceiny/gotry/issues/568) | 导出经 #570 合入；按已接受的 basename 前缀布局对齐原验收清单 | T0 收口候选 |
| [#569](https://github.com/Danceiny/gotry/issues/569) | 二维码实现 #575／#576 已合入；本波补独立解码与 `local_only`，仍待实体扫码 | T2，保持开放 |
| [#571](https://github.com/Danceiny/gotry/issues/571) | 离线脚本经 #572 合入；不代表已部署免安装 landing 网站 | T0 收口候选 |
| [#573](https://github.com/Danceiny/gotry/issues/573) | 同意／token／stub 经 #574 合入；本波修复无效时钟校验，待合并 | T1b，保持开放；外发未激活 |
| [#577](https://github.com/Danceiny/gotry/issues/577) | 召回信号配对经 #578 合入；尚未判定 wish 的 `conditions` | 按下述 T0 补缺，保持开放 |
| [#580](https://github.com/Danceiny/gotry/issues/580) | 链接契约经 #581 合入；本波修复缺 secret 与无效时钟校验，待合并 | T1b，保持开放；不激活路由或 scheme |
| [#559](https://github.com/Danceiny/gotry/issues/559) | 本波已修复 Agent-Reach 错误与恢复指引，待合并 | T1，随后 T4 产品走查 |
...
| 波次 | 交付物 | 并行方式与出口 |
|---|---|---|
| 第一波，下个有界执行批次 | T0 对账、T1 错误提示、T1b token 校验、T2 二维码验收、T3 发布准备、T6 采样准备 | 每 PR 一位负责人，独立审阅；T1／T1b／T2／T3 可并行，共享 runner／索引改动串行。以可审阅改动与证据为出口 |
...
当前执行：Claude Code 按用户确认的既有配置在隔离 worktree 中工作。T1／T1b 实现、T2 自动验收及 T3 离线准备已交付集成审阅。T0 核定四个收口候选及 #577 的范围缺口，尚未修改 issue 状态。T6 已准备采样交接，真实纳入及采集仍待完成。
...
- [x] 对原五个收口候选核对合并 SHA、原范围、验收证据及明确排除项。#564／#566／#568／#571 满足原切片；#577 因缺 wish 条件判定而保持开放。#568 对齐 `<stem>.html`、`<stem>.manifest.json`、`<stem>.qr.svg`；不为迎合旧清单而重命名正常工作的实现。
...
- [ ] 更新 issue 管理面时，对账 #18 已旧的 #512 状态。
...
- [ ] 对 #577，明确 `RecallSignal` 是原始观测还是已经完成条件匹配的权威输入，并明确 wish 的 `conditions` 由哪一层落实。实现判定或提供可执行证据，以离线用例覆盖命中与未命中，再对齐原范围和设计。补缺限定在纯评估器与只读集成内，运行时激活仍不属于本切片。
- [ ] 在 issue 更新／关闭得到明确授权后，写明范围及证据，仅关闭验收完全满足的功能票。父级证据门禁保留。本次规划本身不发表评论、不关闭 issue。
...
- [ ] 完成合并，对齐 #573／#580 验收后再关闭；全量回归已通过。
...
- [ ] 在实际显示尺寸及静默区条件下扫码，记录非私有载荷；保留超容量零写入与冲突回归。只有证据要求时调整 margin。
- [x] 重跑 export（170）、renderer（226）、artifact（131）、类型检查与隔离 smoke；干净依赖安装后 export 与类型检查也通过。
- [ ] 完成合并，处理尚缺的实体扫码验收后再关闭 #569；全量回归已通过。
...
集成源码在关闭真实供应商及会话开关的情况下通过 `scripts/run-all-tests.sh`（退出 0，`ALL SUITES GREEN`，运行时 Node 26.9.0）。定向类型检查、smoke 及改动套件也通过；发布分类共 159 项断言。可选 Agent Reach 安装、STAICLI tarball 证明及真实探针保留明确跳过项。最终文档改动通过双语及可读性门禁。
...
本次交付覆盖 T0–T3 工程与 T6 准备。下一步执行有界的 #577 补缺、实体二维码扫描，以及已准入的 T4–T7 证据路线；商店动作继续遵循已有发布授权。不同文件可采用独立实现／审阅路线，由一位集成负责人收口。本提案不新增定时任务、发布、外部消息、产品状态写入或 issue 关闭；后续动作仍遵循各自范围与授权。
