...
- **Stai Chrome Web Store 候选版（0.2.0.26，#346／#537）** — 源码包与锁文件配对为 `0.2.0-rc.26`，以保持扩展版本不变量；这是源码候选，不代表 npm 已发布。拟提交的商店包修正当前员工门户桥与一次性供应商登录行为的隐私和上架说明。线上商店在后台提审受理、过审并回拉验证前仍为 0.1.0；准确版本与提审窗口仍由创始人决定。
