...
- **L0(纯文档/数据面,零行为变化)**:本设计采纳 → 通道注册表数据文件 +
  doctor v2(patch 宿主插件节 + 配额探测)+ #96 核对关闭。
- **L1(契约迁移,过全栈回归 + 六状态面同步)**:4 个高流量工具 typed 参数迁移 →
  routing 建议字段 + 会话通道状态 → persona (19) 换生成片段。每步一个 PR;
  首个迁移工具带 benchmark canary 一轮(普通模型证据)。
- **L2(拍板后)**:D-7 配额归属正式化(#107 关)、D-9 calendar 默认挂载与否(#106 关)、
  D-8 编排策略确认(#108 关)。
