...
> **状态速览**:
- 2026-09-04 起,typed 参数契约迁移(issue #112,D-30,三刀):11 个工具(flyai/session/hotel/weather 四高流量 + session_login/doctor/artifacts_list/artifacts_read/fact_gate/agent_reach/turn_handoff_list 七高频)参数面 query blob → dsh typed ParameterSchemaSpec 平铺字段,模型可见逐字段 JSON Schema;刀法总纲=有 required 字段的工具宿主权拒畸形参数(ToolFailure 形状不变),全可选工具保留 interpretArgs 容忍层;迁移锁 smoke §12/channel-registry-tests §8;余量工具与普通模型 canary 记 `architecture.md` §10 D-30。
