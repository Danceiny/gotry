...
- Phase 1 environment bridge 当前态：default-off owner-local v3 config 以 `tools[]` 深冻结 descriptor 声明 name/description、closed/bounded `input_schema`、必填非空 `output_keys` 与有限 `domain_outcomes`；同一 schema 同时进入 native request 与 spawn 前 validator，adapter 只接受 exact `gotry_benchmark_tool_result_v1` result/domain/failure envelope。conformance 区分 concrete result、domain outcome 与 failure：domain-only 可进入 tagged terminal，后续 infrastructure failure 仍拒绝，后续 concrete result 可恢复；bridge 不自动 retry。benchmark opt-in 是 cold-start + headless one-shot 进程边界，不能热挂 live agent；每次请求仅允许 native、原始 bridge definition-only surface，最终 assembly 与 downstream pre-step 双闸 fail-closed。Round 3 conformance 把 CLI/shell/Python 操作映射为 native `query.action=call`；缺 call 或坏终态最多一次固定纠偏，格式纠偏禁止重复派发 bridge，parent stdout 再过同一 parser。Round 4 在 optional plugin 解析前把 benchmark 顶层 insert 投影为唯一 `gotry-tools`，未知/未来 sibling 安全裁掉；`gotry-tools` 与 config-path anchor 缺失/重复均在 dsh spawn/relay 前 fail-closed，default-off 组合不变。局部安装回滚与 agent disposal 属 S5 双层 effect 合同；插件先卸载时 live agent 保持 fail-closed quarantine 到 agent/process disposal，HMR 不热挂。config 要求 writes forbidden/network denied，但实际 enforcement 由 owner/host OS 负责，synthetic E2E 不证明 OS sandbox。bridge 不引入 Python/benchmark runtime 产品依赖；Round 8 frozen treatment 尚未运行，provider/scorer/evaluator 未改，不产生 score/uplift 或 external benchmark closure。Phase 0 foundation 的 deterministic planning/admission 边界不变。
...
- Round 7 minimal kernel:代码 SHA `edb9392896625adbb48abae4a2ecf968dbfc0349` 的 benchmark opt-in 仅保留 tool budget、model override、唯一 bridge、isolation/conformance；产品 prompt variables/process guards/consent/普通工具均不装，默认路径不变。system-prompt 投影为稳定 task-agnostic persona，root patch 只接受 canonical `insert` 与 `system-prompt` 各一，其他 root item/变体 fail-closed。ChinaTravel frozen treatment UID `e20241028160248698752`（`easy`，`deepseek-v4-flash`）preflight pass、未回退，80.463s 后 runner exit 1、terminal 0 bytes/invalid，evaluator 未进入、official null、不可计分；白名单归因为 `child_bridge_runner_failed`，不产生 uplift 或 external benchmark closure。下一轮问题聚焦 generic bridge tool schema 与可恢复 domain-error contract。
- Round 8 typed tool/result contract:bridge v3 用单一深冻结 descriptor 同源生成 native schema 与 spawn 前 validator；closed/bounded input 明示 required/type/enum/额外字段边界，exact versioned envelope 只接受已声明的 ok 或 miss/error tuple。`output_keys` 必填非空，每个 primitive leaf 必须位于 declared key 之下，顶层 record array 可用而无键 primitive/mixed array 拒绝。domain miss/error 以 exit-zero transport success 进入模型历史，模型可按固定 recovery enum 修参再调用，bridge 不自动 retry；非零退出仍归 infrastructure failure。conformance 明确区分 concrete result、domain outcome 与 failure，并以最新 bridge response 作为 tagged-terminal 时序下界：domain-only 可收敛，后续 infrastructure failure 拒绝，后续 concrete result 可恢复，旧 terminal 不能遮蔽更晚事实。source synthetic E2E 已覆盖 miss→模型修参→第二次 adapter 调用→tagged terminal，但 frozen treatment 尚未运行，provider/scorer/evaluator 未改，不产生 score/uplift 或 external benchmark closure。
