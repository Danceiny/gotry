...
> **状态速览**:
- 2026-09-04 起,typed 参数契约迁移(issue #112,D-30,四刀):18 个工具(`query:` blob 清零——四高流量+七高频+wish_pool/trip_log/flight_verify/anything/web/video/github)参数面 blob → dsh typed ParameterSchemaSpec 平铺字段,模型可见逐字段 JSON Schema;刀法总纲=有 required 字段的工具宿主权拒畸形参数(ToolFailure 形状不变;trip_log evidence P0 红线进 schema),全可选工具保留 interpretArgs 容忍层;迁移锁 smoke §12/§6/channel-registry-tests §8;余量(feasibility/motivation/companion payload 形态)与普通模型 canary 记 `architecture.md` §10 D-30。
