...
- Round 8 typed tool/result contract:bridge v3 用单一深冻结 descriptor 同源生成 native schema 与 spawn 前 validator；closed/bounded input 明示 required/type/enum/额外字段边界，exact versioned envelope 只接受已声明的 ok 或 miss/error tuple。domain miss/error 以 exit-zero transport success 进入模型历史，模型可按固定 recovery enum 修参再调用，bridge 不自动 retry；非零退出仍归 infrastructure failure。source synthetic E2E 已覆盖 miss→模型修参→第二次 adapter 调用→tagged terminal，但 frozen treatment 尚未运行，provider/scorer/evaluator 未改，不产生 score/uplift 或 external benchmark closure。
