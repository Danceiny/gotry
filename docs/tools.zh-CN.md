...
| 组 | 工具 | 契约 |
|---|---|---|
| **实时检索（OTA/官方，只读）** | `gotry_flyai_search` | 通过固定版本 `@fly-ai/flyai-cli@1.0.16` 提供八类 FlyAI 公开能力：机票、火车、酒店、景点、关键词、AI、万豪酒店、万豪套餐。平铺参数保留旧别名并开放公开筛选；打码价原样展示，仅精确日期交通查询或精确入住查询写库存事实。鉴权和试用达限不重试，上游瞬时故障最多重试一次。见[集成契约](design/flyai-supplier-skill-design.zh-CN.md)。 |
