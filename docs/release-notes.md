...
- **Stai Chrome Web Store candidate (0.2.0.26, #346/#537)** — source package and lockfile are paired at `0.2.0-rc.26` to preserve the extension-version invariant; this is a source candidate, not an npm publication. The proposed store package corrects the privacy and listing disclosure for the current employee-portal bridge and one-time supplier-login behavior. The live store remains 0.1.0 until a dashboard submission is accepted, reviewed and pulled back; the founder still chooses the exact version and submission window.
