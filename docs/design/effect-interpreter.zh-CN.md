...
|---|---|---|---|---|---|
| `FLYAI_SEARCH` | cli | 上游瞬时错误 2 次／500ms 起；本地进程时限、信号与启动失败不重试 | 3 连错/开 60s | – | data-sources §8：Sentinel 限流绝不硬重试；熔断保护未公布配额；429 试用达限归 needs-setup（工具面阻断盲重试，2026-09-02 迪拜 session） |
