...
- 同样自 2026-09-12 起，#436 记录剩余的持久健康面 routing 缺口：local probe 事件已写入 `channel-health.jsonl`，持久健康面已被愿望池召回与既有 doctor 读取；但 `routingAdvice` 仍读取独立的进程内 verdict state，需后续聚焦切片接入持久最新健康面。
