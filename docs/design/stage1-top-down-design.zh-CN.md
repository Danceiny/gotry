...
> 定位：Stage 1 原始对话循环与组件契约的 frozen 历史设计。
> 状态：frozen(2026-09-13)。
> 当前权威：[GoTry 架构文档](../architecture.zh-CN.md)。runtime 清单、交付状态与未偿工作必须从该文档读取，不能从本快照推断。
> 解读方式：下文的数量、状态标签与“待定义”描述的是当时的 Stage 1 基线，不是当前实现流水账。
> 保留的历史决策：先用确定性 mock LLM 自顶向下验证循环，再接真实模型；模型访问影响智能质量，不决定架构能否验证。
