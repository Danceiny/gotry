...
- 同样自 2026-09-12 起，#441（父 #438）把既有产物 list→read 链路扩到 HTML 行程文件：工作目录顶层发现覆盖 `.md`／`.html`／`.htm` 且大小写不敏感，mtime 排序、各源 limit 与截断语义不变（Markdown 兼容性保持），`.html`／`.htm` 读回为源码文本——`lang: html`、原始行号、全文指纹、分页——走同一组 canonical 路径／realpath／拒绝目录／2 MB 护栏。HTML 永不被解析或执行（脚本、内联事件处理器、远程加载一律不运行、不抓取；不建 iframe），Web 客户端仍以 React 文本节点渲染这些行；交互式行程渲染仍归 #438。隔离 list→read 用例还暴露并修复了一个真实预检缺口：list 返回的 canonical cwd 路径在 macOS `/var` 符号链接文件系统上读不回来；realpath 之后的 canonical 复检仍是边界权威且未改动。
