...
> **状态速览**：
- 2026-09-13 起，内嵌 Booking Copilot personaPrefix 已到达首次外发的 DSH 模型请求（#467）；仅为工程 slice，真实 Booking UAT 仍属 #142。
- 既有 Booking 搜索失效机制（#468）通过提高 workspace revision 清除依赖它的酒店与报价选择；同 revision 的变更会被拒绝，可选缺失字段仍可比较。运行时证明覆盖 `search.run` 与合法的 `search.patch`；这项工程证据不关闭 #142 的真实 Booking UAT。
- #443 新增五个默认关闭的 Lavish 评审工具，采用可信宿主配置、精确会话与工作目录绑定、有界未受信任反馈及自有进程清理。既有行程生成与产物发现入口衔接评审，见 [工具契约](lavish-local.zh-CN.md#9-注册到产品工具面)。已注册的 Lavish 浏览器反馈链由 #443 CLOSED + PR #456 merged acceptance 覆盖；原生 HTML preview 实证在 #448 已接受，持久回归落在 `ts/scripts/dsh-artifact-web-e2e.ts`（可复跑：`GOTRY_ARTIFACT_WEB_E2E_OUT=<dir> npx tsx ts/scripts/dsh-artifact-web-e2e.ts`）。
- 2026-09-12 起，#442（父 #438，内部切片）让纯行程 HTML 渲染器可从真实产品入口到达：注册工具 `gotry_itinerary_render`（`ts/capabilities/itinerary-artifact.ts`）接收 `title`、显式行程对象与 `fact_ids`，事实**只**从当前 `stateRoot` 事实注册表取（绝不接收调用方自带事实对象；未知/重复/超量 id 与畸形登记行一律拒绝），以 `O_CREAT\|O_EXCL` 在会话工作目录顶层仅新建一个文件（不覆盖、不跟随符号链接逃逸、拒绝 `.git`/`node_modules`，默认 crypto 随机文件名），非法输入零字节写入，并返回最终真实路径；空 `fact_ids` 是合法的明确未核验计划，不存在整体「已验证」徽章。原生 HTML preview 实证在 #448 已接受，持久回归落在 `ts/scripts/dsh-artifact-web-e2e.ts`（可复跑：`GOTRY_ARTIFACT_WEB_E2E_OUT=<dir> npx tsx ts/scripts/dsh-artifact-web-e2e.ts`）；已注册的 Lavish 浏览器反馈链由 #443 CLOSED + PR #456 merged acceptance 覆盖；run-all §6c 承载聚焦套件。
...
- 同样自 2026-09-12 起，#441（父 #438）把既有产物 list→read 链路扩到 HTML 行程文件：工作目录顶层发现覆盖 `.md`／`.html`／`.htm` 且大小写不敏感，mtime 排序、各源 limit 与截断语义不变（Markdown 兼容性保持），`.html`／`.htm` 读回为源码文本——`lang: html`、原始行号、全文指纹、分页——走同一组 canonical 路径／realpath／拒绝目录／2 MB 护栏。读取／源码卡不解析标记、不运行脚本或内联事件处理器、不发起任何抓取（不建 iframe），Web 客户端在 HTML 源码标题下以 React 文本节点渲染这些行；列表的主动打开是另一个动作，标注 `Open HTML preview` 并带可见徽标与「页面脚本可能运行」提示，因为它把文件交给宿主原生 HTML 预览——宿主 renderer 行为单独记录，读取路径的「不执行」结论不对它作声明；已注册的 Lavish 浏览器反馈链由 #443 CLOSED + PR #456 merged acceptance 覆盖；原生 HTML preview 实证在 #448 已接受，持久回归落在 `ts/scripts/dsh-artifact-web-e2e.ts`（可复跑：`GOTRY_ARTIFACT_WEB_E2E_OUT=<dir> npx tsx ts/scripts/dsh-artifact-web-e2e.ts`）。隔离 list→read 用例还暴露并修复了一个真实预检缺口：list 返回的 canonical cwd 路径在 macOS `/var` 符号链接文件系统上读不回来；realpath 之后的 canonical 复检仍是边界权威且未改动。
- 自 2026-09-12 起，#458（在 #25／#285／#441 之上）为产物 list 加上分页 + 字面元数据搜索：`offset`（零-based 非负整数）与 `search`（字面大小写不敏感子串，匹配 `id`／`title`／filename；trim 后空串 = 不过滤；非正则／非 glob，不搜文件正文）——模型走收集 → canonical-path 去重（账本与文件视图指向同一文件时保留账本权威）→ 搜索过滤 → 确定性全局排序（`updated` DESC + `(source, id, canonical path)` 词典序 tie-break，使等 mtime 的页面边界在跨主机／跨 locale 下保持稳定）→ 分页切片。移除此前各源提前 `slice(0, limit)` 的早裁逻辑，避免旧 cwd 产物被单源上限静默丢弃。`limit` 保留 pre-#458 整数零／负 clamp-on-1 合同（兼容），字符串／非有限／非整 predictably 拒绝，正值封顶 50；`offset` undefined 默认 0，null／字符串／非有限／非整／负数 predictably 拒绝；`search` 非字符串 predictably 拒绝，不静默退化为「不过滤」。`total` 反映过滤后集合的真实长度；`nextOffset` 仅当 `offset + returned < total` 时出现；越界 `offset` 返回空页 + 已知 `total` + `truncated:false` + 无 `nextOffset`。读路径沿用 #441 行为不变。验收证据 = `scripts/artifacts-capability-tests.ts` 25 项隔离 fixture proof（新增 19–25 覆盖 >50 跨页枚举、等 mtime 词典序 tie-break、混合账本／legacy／cwd 去重跨分页保留、字面搜索（含大小写无关／字面正则不命中／非内容匹配）、无效 offset／limit／search 类型拒绝且无静默圆整、limit 零／负兼容夹 1、大值夹 50，以及分页下读边界复检）+ `scripts/dsh-artifact-e2e.ts` 已注册插件 list→分页／搜索→read→Host 表现证明（>50 混合源 fixture，tie-break／去重／过滤后 total／安全参数，越界 summary 文案）。
- 2026-09-11 起，行为契约 23→13 条（瘦身，M4 人格校准线；founder 指令「profile 提示词太长、窄域说辞不具启发性」）：窄域细则去重归位工具描述/通道路由卡（渠道 setup→agent_reach 描述、会话授权/登录→session 工具描述、可下单事实细则→fact_gate 描述，均原文已在）；persona 只留跨场景行为契约，四个动态变量注入面进回归锚；persona-surface-guard-tests 改锚 6/6，历史事故锚全保留。
 ...
 - 2026-09-10 起（相对年份自 2026-09-11），Issue #2 命名年份的未来规划——四位年份，或按本轮锚点换算的相对年份（今年、明年、后年、年内）——在 `time-anchor.ts` 派生本轮参考日；注册工具 `gotry_feasibility_check` 对带日期候选即使省略 planning 也默认施加宿主时钟 future 下界，显式 future 年份再施加年末上界，预期拒绝返回结构化 validation result 而不进入 incident。完全 dateless 仍保持旧可行性计算，过期年份不滚年，历史/回测需明确 `historical` 模式；loop 仅对显式给出的规划年份（四位或上述相对形式）应用窗口，否定过去推荐和多年份歧义不误判为历史。`time-eval-tests.ts` §6 与 `smoke.ts` registered execute fixture 以注入时钟验证，仅为隔离工程证据，不替代供应商或真实业务准入。
...
- 2026-09-10 起，PR #327 曾短暂允许受限的相同重复参数恢复；#3580 已取代并删除该路径。embedded planner 现只通过一次 `JSON.parse` 接受一个 canonical JSON 对象；畸形、重复、嵌套/字符串化、带垃圾、非对象与 coercion 输入全部 fail-closed。schema shape 纠偏必须经 typed `INVALID_ARGS`，再由同一个 DSH run 内稍后的 canonical call 完成。公共与本地 subprocess fixture 仅是工程证据，不构成真实 provider reliability、HotelByte UAT、M3/M4 cohort 或 M5/M6 admission。
...
- 2026-09-10 起，#352 FlyAI `data.itemList` 机/火/酒店边界区分精确空列表 `miss`、完整合法列表 `hit` 与任一 item malformed 的整体 `error`；后者丢弃有效 sibling 且不进入负库存事实。`flyai-tests.ts` 仅为隔离假 CLI/production effect/fact sidecar 离线 proof，不构成真实 provider/UAT 或 issue closure。
- 2026-09-10 起，#335 离线 sf-summary 子任务按 canonical producer filename 选择单一批次，按批次采集时间处理新旧证据，保留 per-record `started_at` 与 requested/effective/fallback/provenance；最新批次缺 query 或含 corrupt JSON 时 fail-closed，legacy 无批次身份记录为 unknown/ineligible。run-all §44b 的 synthetic CLI proof 不满足 #272 live session/calibration 或 packaged connected/degraded gate。Issue #420（2026-09-12）入口 alias 子切片：独立 CLI main 守卫现以 `realpathSync` 同时规范化 `process.argv[1]` 与 `import.meta.url`，文件符号链接与父目录符号链接调用均能进入 main() 并产生与 canonical 路径相同的汇总；import 静默保留。仅限入口守卫，不改 scoring / selection / source bucket / fail-closed 语义。
...
- 2026-09-10 起，#271 外层 liveness 收口：GoTry launcher 将 dsh child 放入独立 POSIX process group；exit/error/close（含继承 stdio 的 delayed close）与 parent-only SIGINT/SIGTERM 经过 bounded TERM→KILL，确认 direct child/group 清空后保留原始终止语义，既有 incident observer/writer 仍是唯一记录面；benchmark 零退出继续走原 stdout drain/parser。Node24 真实安装版本 dsh `0.1.5-rc.1` 的维护 proof 由实际 package-shaped `bin/gotry-inner.js` 覆盖非零、正常零退出和 TERM-resistant descendant，并保留 child crash、rejected promise、spawn ENOENT、SIGINT/SIGTERM，核对 parent/leader outcome、stderr、产品 incident、post-failure marker 与 before/after descendant/group 清理。dsh SDK direct transport、内部 supervisor 和真实 deployment contract 仍是开放 TODO；本切片不改 vendor/lock、不安装 daemon、不提升任何真实里程碑 gate。
- 2026-09-09 起，#282 Booking planner 纠偏（权威边界已由 #3580 取代）：嵌入式 read-action planner 保留非空 occupancy 房间及 childAges，缺 `adults` 仍为 schema-invalid。插件返回 typed `INVALID_ARGS`；只有同一个 DSH run 内稍后的 canonical call 才能纠正。provider-run 有界预算只适用于 no-tool/prose 提示，provider failure 透传。注入 runPort 与本地 subprocess proof 都只是工程证据，真实供应商库存/UAT 仍开放。
...
- 2026-09-10 起，#254 只读修复计划：复制 db/`-wal`/`-shm` 到临时目录后盘点，复制前后与读取结束对账源字节，变化即 fail-closed；显式证据映射才可计划搬移。未 checkpoint WAL 与 v1 零目录写均由隔离测试覆盖。
- 2026-09-11 起，#254 授权 apply/rollback：`repair-apply` 需要 `--mapping` + 匹配的 `planDigest` + `--i-authorize-apply`，写前强制带校验和的 backup，CAS 更新 `tenant_id`（保留 `seq`），重建受影响 tenant，并支持 `repair-rollback` 与回执 schema；run-all §29c 仅为隔离夹具证据。founder 授权的真实 repair 回执（或确认无需搬移）仍是 issue 关闭条件，不构成 M5/M6 gate。
...
- 2026-09-10 起，事实闸覆盖面政策锚点全字段内容指纹（issue #359，父 #273，D-26 残余收口）：渲染与注册闸共用单一权威构造器；**当政策行含有 `<!-- fact:` 锚点时**，必须严格等于 canonical 整行 + 单一 `<!-- fact:<id> -->`（subject/statement/source/fetched_at/query_id/as_of），任一改动 → fact_anchor_unknown，与未知锚点同源；多锚点、格式异常锚点、非 canonical 前/后置非空文本均 fail-closed。**不含** `<!-- fact:` 的政策行仍走 unanchored/free-text 启发式，不在本切片范围（交父 #273 / D-26）。`opts.itinerary.trip_start` 桥接到 `opts.tripStart`，与 `review_by` 同为合法复核提醒形态。确定性 synthetic / isolated evidence，不宣称 live policy、supplier、M5/M6 准入；父 #273 残余债务保留。
- 2026-09-10 起，事实闸覆盖面航班/酒店锚点字段指纹（issue #363，父 #273，D-26 残余收口）：渲染原语（`renderFlightFact`/`renderHotelFact`）与注册闸共用同一权威事实——航班行重新抽取 `flight_no` token 严格 == 事实（大写归一），酒店行严格包含 `destination + check_in + check_out`（精确子串），任一不等即 `fact_anchor_unknown` fail-closed，与未知锚点同源。**字段指纹不替代整行严格比对**（根 contract 第 2 条）：flight_no/承运/时刻/route/价格的可比性、列车 canonical、政策 canonical、§4b 价格兼容形态（总预算/行李费/总计/显式票价/千分位/价待询/¥7xx/约价）、`price_contradicted` 与 `unverified_price_claim` 分类与排序均不被新指纹吞并，沿用既有判定原语。借用/未知/重复锚点仍由既有 `fact_anchor_unknown` 路径负责，字段指纹只补齐「保留 anchor/registry/fact_id 改写可见 flight_no/目的地/档期」的根反例 fail-closed。`gotry_fact_gate` 注册闸执行面无新增路径，沿用既有 `loadFactRegistry` + `gateArtifact`；事实日志走真实 append-only JSONL（`appendFacts`），隔离临时 `stateRoot` 装载注册定义 → 捕获真实 `gotry_fact_gate.execute()`。确定性 synthetic / isolated evidence，不宣称 live supplier、live 12306、live policy 或 M5/M6 准入；父 #273 残余债务保留。
- 2026-09-10 起，事实闸覆盖面机/火行内价格矛盾（issue [#300](https://github.com/Danceiny/gotry/issues/300)，父 [#273](https://github.com/Danceiny/gotry/issues/273)）：exact-date 回溯成功后，flight/train 共享 gate 原语仅在 canonical fare 字段或明确票价标签字段内对对应航班/车次事实的完整可比较 CNY 硬价 `¥NNN`/`CNY NNN` 做一致性校验；分隔后的带非票价文字标签金额不参与，裸/未归属或畸形 fare money = `unverified_price_claim` blocked。数值不一致 = `price_contradicted` blocked；源事实缺价、不支持的硬币种、或无锚点且价格超出启发式 120 字窗口 = `unverified_price_claim` blocked；非数字/起价/约价不比较，不猜汇率，酒店 `priceRaw` 不参与；价格容忍度、汇率/多币种策略仍待 owner/founder 拍板，不关闭 D-26 全部残余。
- 2026-09-10 起，事实闸覆盖面·非关键词住宿 category + 政策词有限并集（issues [#301](https://github.com/Danceiny/gotry/issues/301)/[#302](https://github.com/Danceiny/gotry/issues/302)，父 #273，D-26 残余）：手写/历史 read-side claim 启发式扩词：5 类非关键词住宿 category + lodging heading 下无 token 可住断言升为 hotel claim，heading depth 状态机激活/退出/继承（ATX 1–6）；政策 13 词有限并集，缺 as_of 直读 fail-closed；Latin 词大小写不敏感但词边界有界。fact-gate-tests §12/§13 覆盖。确定性 fixture 覆盖，不宣称实时政策正确性、供应商就绪或 M5/M6 准入。
- 2026-09-10 起，#299/#273/#355 事实闸分流与 typed rail 接入：完整 G/D/C/Z + 3–4 位车次进入独立 `trains` claim 集合，不按首字母误删 `CZ8582`/`9C8781`；flight verdict 只看 flight facts，rail verdict 只看 train exact-date/route/code/bookable facts，缺证据输出 `rail_claim_unverified`。12306 session 经 `gotry_session_search kind=train` 接入 typed registry：parser 区分 recognized empty/nonempty、malformed、transport-runtime error，正事实以 host-captured invocation date/binding 与精确 response URL 为准，行内 startTrainDate 仅校验并保留、15 分钟非未来采集时间、`canWebBuy=Y` 与闭集可用座位 token；列表无票价，不使用 static-schedule 替代。仅有确定性 fixture 证据，不宣称真实供应商可售性或关闭 issue。
...
- Round 5 runtime resolution：source checkout 与 clean package 都解析同一组 232 个精确直接依赖的 DSH `0.1.5-rc.1` closure（2026-09-09 issue #268 从 `0.1.2-alpha.3` 216 包闭包精确迁移：15 新增 sentinel + 移除 `dsh-tool-subagent-report`，全部 230 个 `@deepseek-ai/dsh*` 包钉死精确版本，拒绝 `^0.1.5-alpha.1` 匹配 `0.1.5-alpha.2` 的 semver 预发布漂移；CI `npm ci --strict-peer-deps` / pnpm `--strict-peer-dependencies` 显式严格；run-all §23a-§23e 五个确定性证明。设置行为不变；尚未发布 tag 或 npm 版本；这些确定性证明不构成 M5/M6 准入）；manifest、package-lock 与 root pnpm importer 的 DSH 名称集合必须同为 232，publish preverify 对漏钉、混版或 range 声明 fail-closed。source 普通运行的 dsh cwd 保持 `ts/dsh-runtime/`，状态继续落 `ts/dsh-runtime/gotry-state/`，benchmark opt-in 与 npm package 使用调用目录隔离；legacy vendored alpha.1 只作非 benchmark 解析兼容，不承诺可运行。benchmark 在 spawn 前拒绝实际 DSH 版本漂移；Node 下界统一为 22.15，并在 DSH import/spawn 前稳定拒绝旧版本。
...
> **Issue #67 会话 benchmark 状态**：`sf-live-benchmark --golden=static` 以 OpenFlights 固定修订提供 route/carrier，以手工 manifest 提供估算时刻／价格带；evidence 显式记录 requested/effective source、provenance、estimated fields 与 fallback reason。静态源失败会 stderr 告警并回退 manual，不伪装成实时班期／票价／库存；真实会话侧仍依赖用户 Chrome 扩展。离线合同归 run-all §44。自 #411（2026-09-12）起，live 批次在首个 `challenged`／`challenge_stop`／`guard_violation` 即停，只有正常完成八条查询的批次才是 `batch_complete`；run-all §44c 以落盘 JSON、CLI 输出、请求计数、临时根和零网络覆盖首条／中途／末条截断。生产节律保持 35 秒，仅隔离 harness mock 子进程 timer。
