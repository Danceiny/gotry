...
> 状态：living（issue #225）。#231（可信审批 receipt + 原子 outbox core）、#232/#233（hotelbyte-cli 纯契约 adapter 与 cancel/refund/commission 契约）、#234（kernel manifest + import trace proof 脚本）、#235（sponsor plugin 契约 + 默认 off fixture）等 pre-entry 机制已入 main 并在运行时边界保持封存；整个 program 仍为 TODO——M3 真实 seed cohort 与 M4 真实 `observed_private` N≥5 repeat cohort 未闭合，M5/M6 Entry gate 未满足，且未发生真实 supplier UAT、founder P6 批准或试点签约。本文是计划与责任图，不替代 M3/M4/M5/M6 的 Exit 证据。按 AGENTS 合同要求，EN/ZH 双语须在同一次提交中保持一致。
...
| M5-2 | TODO（pre-entry 已入 main） | #231 | m5-core worktree | 持久化可信审批与原子 outbox 机制已入 main（ts/src/write-gate.ts 等）；真实 supplier effect 与 M5 Entry 仍 TODO（#136） |
| M5-3 | TODO（pre-entry 已入 main） | #232 | m5-adapter worktree | hotelbyte-cli 纯契约 adapter + fake-CLI 已入 main（hotelbyte-transaction.ts 等）；真实 supplier UAT 与 M5 Entry 仍 TODO（#136） |
| M5-4 | TODO（pre-entry 已入 main） | #233 | m5-refund worktree | cancel/refund/wallet outcome + commission/披露 纯契约已入 main（cancel-refund-commission.ts 等）；真实补偿矩阵与 M5 Exit 仍 TODO |
...
| M6-3 | TODO（pre-entry 已入 main） | #234 | m6-proof worktree | kernel manifest + import trace proof 脚本已入 main（`ts/data/kernel-manifest.json` 已存在；`ts/scripts/kernel-manifest-gate.ts:63` 加载冻结 manifest，`:100–103` 校验 schema/integrity/functional coverage，`:116` 运行 runtime import trace；`ts/scripts/kernel-manifest-evidence.ts:43` 构建 evidence；`scripts/run-all-tests.sh:561` 调用 gate）；M5 Exit + P6 founder YES + 真实 sponsor UAT 仍 TODO（#137） |
| M6-4 | TODO（pre-entry 已入 main） | #235 | m6-plugin worktree | sponsor plugin 契约 + 默认 off fixture 已入 main（`ts/capabilities/sponsor-plugin.ts` `createSponsorPlugin:96`、route authorization `:155`、ranking `:234`、confirmation fingerprint `:298`、`runBookingScenario:390`；`:138` `requireRuntimeActivation` 强制 default-off flag 边界）；激活 + 真实 agency E2E + 试点签约 仍 TODO（#137） |
...
- **状态**：pre-entry 机制已入 main（`ts/src/write-gate.ts` `preparePresentation:305` → `trustedHostConfirm:375` → `claimForDispatch:491` → `preDispatchRevalidate:524`；`scripts/run-all-tests.sh:554` 含 write-gate tests）；运行时边界封存；真实 supplier effect、M5 Entry 与 M5 Exit 仍 TODO（#136）。
...
- **状态**：pre-entry 契约 + fake-CLI 已入 main（`ts/capabilities/hotelbyte-transaction.ts` `classifyBookExecution:189` → `advanceReconciliation:226` → `newIntentAllowed:339`；`ts/scripts/hotelbyte-spawn-e2e-tests.ts` `runFake:139` 跑本地 fixture 子进程）；真实 adapter 接线、supplier UAT 与 M5 Entry 仍 TODO（#136）。
...
- **状态**：pre-entry 契约已入 main（`ts/src/booking-surface/cancel-refund-commission.ts:120/:137` cancel/refund 校验，`:299` 披露 → `:309` fingerprint 绑定）；真实补偿矩阵、客户退款路径与 M5 Exit 仍 TODO。
...
- **状态**：pre-entry 离线 proof 工具已入 main（`ts/data/kernel-manifest.json` 已存在；`ts/scripts/kernel-manifest-gate.ts:63` 加载冻结 manifest，`:100–103` 校验 schema/integrity/functional coverage，`:116` 运行 runtime import trace；`ts/scripts/kernel-manifest-evidence.ts:43` 构建 evidence；`scripts/run-all-tests.sh:561` 调用 gate）；M5 Exit、P6 founder YES 与真实 sponsor UAT 仍 TODO（#137）。
...
- **状态**：pre-entry 契约/fixture 已入 main（`ts/capabilities/sponsor-plugin.ts` `createSponsorPlugin:96`、route authorization `:155`、ranking `:234`、confirmation fingerprint `:298`、`runBookingScenario:390`；`:138` `requireRuntimeActivation` 强制 default-off flag 边界）；激活、真实 traveler 效果、真实 agency E2E 与试点签约 仍 TODO（#137）。
...
8. TODO（pre-entry 机制已入 main）：#231 可信审批 receipt + 原子 outbox core 代码已入 main（`ts/src/write-gate.ts` `preparePresentation` → `trustedHostConfirm` → `claimForDispatch` → `preDispatchRevalidate`）；运行时边界封存；真实 supplier effect、M5 Entry 与 M5 Exit 仍 TODO（#136）。
...
15. TODO（pre-entry 工具已入 main）：#234 M6 kernel manifest + import trace proof 脚本已入 main（`ts/data/kernel-manifest.json`；`ts/scripts/kernel-manifest-gate.ts`；`ts/scripts/kernel-manifest-evidence.ts`）；M5 Exit、P6 founder YES 与真实 sponsor UAT 仍 TODO（#137）。
16. TODO（pre-entry 契约/fixture 已入 main）：#235 M6 sponsor plugin 契约 + 默认 off fixture 已入 main（`ts/capabilities/sponsor-plugin.ts` 通过 `requireRuntimeActivation` 强制 default-off）；激活、真实 traveler 效果、真实 agency E2E 与试点签约 仍 TODO（#137）。
