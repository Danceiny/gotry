...
| marriott-package | search-marriott-package | 关键词、酒店名、省市、价格排序 |

对于 `marriott-hotel`，`destName` 映射为 `--dest-name`；`hotelBrands` 和 `hotelName` 与 `keyWords` 合并后映射为 CLI 唯一的 `--key-words` 值，床型、日期、最高价和排序筛选使用各自的官方参数。该 kind 拒绝通用的 `hotelTypes` 和 `hotelStars` 筛选。对于 `marriott-package`，`keyword`、`hotelName` 和 `provinceOrCity` 分别映射为 `--keyword`、`--hotel-name` 和 `--province-or-city`，至少填写一项。`sortType` 仅接受 `price_asc` 或 `price_desc`。
