...
> Status: product entries landed through Phase B (reconciled 2026-09-25): the pure renderer (2026-09-23), the deck render entry `gotry_itinerary_deck_render` (#566), and the static-export bundle `gotry_deck_export` with the real QR matrix (#568/#569); Phase C share/consent contracts and stubs are documented in [share-adapters.md](share-adapters.md). Static hosting/deploy and real outbound runtime remain inactive.
...
> Downstream: `ts/scripts/itinerary-deck-tests.ts` (run-all §6d); [Phase C share/consent contracts](share-adapters.md); separately admitted hosting and outbound runtime; design-system seeds for later phases.
...
Landed at issue #568 (Phase B slice 3a): the deck static-export bundle `ts/capabilities/itinerary-deck-export.ts` exposing the registered tool `gotry_deck_export` — writes three basename-prefixed files to a host-provided absolute `target_dir` (`<basename>.html` + `<basename>.manifest.json` + `<basename>.qr.svg` placeholder), with manifest provenance (sha256, bytes, fact counts by kind, source tags, evidence-chain summary, exported_at, optional target_url), bundle-level `O_CREAT|O_EXCL`, and an explicit refusal to follow symlinked target_dirs (lstat check before realpath). Verified by `ts/scripts/itinerary-deck-export-tests.ts` (170 assertions after the T2 decode-acceptance widening, run-all §6f).

Landed at issue #569 (Phase B slice 3b, the real QR matrix): the `<basename>.qr.svg` file now contains an actual QR matrix rendered by the `qrcode` npm package (MIT), encoding the `target_url` when provided or the literal marker `<local bundle; not yet hosted>` when not. `manifest.share_intent.qr` transitions from `placeholder` to `generated`. Same-input determinism asserted byte-level (same target_url → identical qr.svg bytes).

QR acceptance evidence (T2, 2026-09-25): the on-disk SVG is independently decoded — rasterized with `sharp` (already in the runtime closure transitively via `@deepseek-ai/dsh-attachment-local`; now declared explicitly as a devDependency — this change adds only test imports, no new runtime calls) and decoded with `jsqr` (Apache-2.0, newly added dev-only dependency; shares no code with the `qrcode` encoder) — recovering the exact payload byte-for-byte across the full payload × size matrix (ASCII URL, UTF-8 URL, and the pinned literal local marker × 128/240/480px), with the 2-module margin kept (no evidence required a change). The expected marker is pinned in the tests as the approved literal, independently of the production constant, and the two are drift-locked (an accidental marker change fails the suite). The manifest gains an additive `local_only` indicator (`true` exactly when `target_url` is absent): issue #569's `local_only` expectation is reconciled into the accepted `gotry_deck_manifest.v1` contract as a backward-compatible optional field, so "no online destination" is read explicitly from `local_only: true` instead of being inferred from a missing `target_url`. Oversized-input zero-write and bundle-collision regressions are retained; physical phone-scanner acceptance was not exercised in this lane (independent raster decode + rendered-image inspection is the recorded executable evidence).

The renderer and Phase B export slices do not provide static hosting/deploy or outbound runtime. Phase C share/consent contracts and stubs have landed separately; see [share-adapters.md](share-adapters.md) for their activation gates. The deck renderer only renders caller-supplied structure plus registry-selected facts; its fixtures are synthetic and stand for no supplier evidence.
