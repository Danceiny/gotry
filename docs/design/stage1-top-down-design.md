...
- 2026-09-08 起,M4→M6 program 任务图(issue #225):`milestone-delivery-plan.md` 把 #227 Z3 Node24 间歇 heap corruption 阻断、#223 M4 scorer 硬阈/隐私、#228 显式 opt-in planning lifecycle 采集/脱敏导出、真实 N≥5 repeat cohort、#136 M5 首供应链 `hotelbyte-cli` 协议核验 + WriteGate、#229/#237 tenant 隔离(已合入 main)、#236 state-cli 租户边界(已合入 main)、#230 ledger 验收、#137 P6 评审/试点条件拆成 owner/责任文件/E2E/否证/退出标准;后续实现 issue #231–#235;`write-gate-production-design.md` 仅为 proposal,不启封交易或 B2B 实现。
