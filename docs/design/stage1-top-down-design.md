...
> **状态速览**:
- 2026-09-09 起,酒店日期输入闸(issue #283):founder 截图实证 `gotry_hotel_search` 空/缺/单侧/非字符串/无法解析/倒序/同日日期仍派 hbcli 按当前窗口价返回。新增纯函数 `ts/src/hotel-date-gate.ts` `evaluateHotelStayDates`(复用既有 `buildTimeAnchor`/`resolveSlotDate`,不动时间锚点/slot-spec/其它工具/dsh 依赖/planner/incident observer);闸失败 → `verdict=input_required` 五字段(reason/missing/raw/message/evidence)一致,不调 `interpretEffect`、不发 hbcli、不写 bridge-latency、不触发 ensureStateDir,通过闸才进 HBCLI_HOTEL_SEARCH 解译层与 `--check-in/--check-out` 旗标。presentResult 渲染为「酒店:<dest> 需要入住日和退房日」行动卡(summary 直传,正文给出具体日期示例),不退回旧版「无结果」空卡。两层口径:`parseAbsoluteDate` 词表层拒 2026-02-30/2026-13-01/2025-02-29 等不存在日历日;闸终消费层 `isStrictIsoYmd` 复用 `isRealIsoDate` 拦截 +N 算术溢出(addDaysYmd → "NaN-NaN-NaN")与 JS 自动进位跨千年(9999-12-31+1 → "10000-01-01")归 unresolved。维护测试 `ts/scripts/hotel-date-gate-tests.ts` 覆盖空/完全缺失/单侧(双向)/混合缺失-空白/倒序/同日/无法解析/类型错误/有效日期 argv/自然语言解析(口径同 slot-spec,跨月用 Date 自动进位)/bridge-latency before-after 零增量 +1 行增量/隔离用户状态/空白串/presentResult 行动卡/+N 算术溢出/JS 自动进位跨千年/闰年错日等不存在日历日;断言数见脚本尾部自报 N/N,run-all §30d。该工程闸只闭合语义缺口,不启封 M5 写闸、不变 Booking 库存业务效果(D-29)。
