...
- 2026-09-10 起,#352 FlyAI `data.itemList` transport 边界区分精确空列表 `miss` 与非空全 malformed `error`;后者不进入负库存事实,有效混合响应保留 `hit` 并暴露 `n/N` evidence。`flyai-tests.ts` 仅为隔离假 CLI/production effect/fact sidecar 离线 proof,不构成真实 provider/UAT 或 issue closure。
...
- 2026-09-10 起,#299/#273 事实闸分流切片:完整 G/D/C/Z + 3–4 位车次进入独立 `trains` claim 集合,不按首字母误删 `CZ8582`/`9C8781` 等合法航司;flight verdict 只看 flight facts,rail verdict 只看 FlyAI 真实工具路径注册的结构化 train exact-date/route/code/bookable facts,缺证据输出 `rail_claim_unverified`。12306 session 仍只提供公开列表面,待 typed parser outcome + seat availability/freshness contract 后接 typed registry;列表无票价,不新增 RailFact/static-schedule 替代,不宣称真实供应商证据或关闭 issue。
