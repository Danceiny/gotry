...
- 2026-09-10 起,事实闸覆盖面政策锚点全字段内容指纹(issue #359,父 #273,D-26 残余收口):renderPolicyFact 与闸侧 anchor 比对共用 `policyCanonicalBody(p, tripStart?)` 单一构造器 —— 政策行 canonical body 是唯一权威面;闸侧 anchored policy 行(去锚点段)全文比对 `subject`/`statement`/`source`/`fetched_at`/`query_id`/`as_of`,任一改动 = `fact_anchor_unknown` fail-closed,与未知锚点同源(`as_of` 子串匹配是 #273 切片遗留宽松面,本切片闭合);合法 `review_by` 与 tripStart 派生提醒两种形态保留(`opts.tripStart` 与渲染同一传入即一致),不通过删除提醒或删除正例得到 green;不引入并行 fact type / supplier / 依赖 / vendor / WriteGate runtime / 通用 regex-NLP 改造 / live-policy 断言,亦不关父 #273。fact-gate-tests §14 14 断言(canonical + 5 failing-before + 2 legitimate + 多行归属 + 双 reminder 形态);isolated E2E `scripts/policy-anchor-359-e2e.ts` 走真实事实日志(`appendFacts` → `loadFactRegistry`)并以 registered fact-gate definition `gateArtifact` 验证篡改行 fail-closed,不启动 dsh 宿主、不触碰真实供应商/browser/policy service/共享状态。确定性 synthetic / isolated evidence,不宣称 live policy、supplier、M5/M6 准入。
