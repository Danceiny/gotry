...
- 2026-09-10 起,事实闸覆盖面政策渲染锚点闭合(issue #273,D-26 残余收口切片):renderPolicyFact 行内嵌 fact 锚点(与机/火/酒店同源 typed-anchor),闸侧锚点确定性回溯,手改/伪造锚点 = fact_anchor_unknown;锚点行 as_of 内容指纹(改写或删除日期而保留 fact_id 同源 fail-closed);POLICY_WORD 补「海关申报」(与「入境申报」同性质但被原 regex 漏掉),手写政策行缺 as_of → policy_without_as_of。fact-gate-tests §11 九断言(65 pass)。
