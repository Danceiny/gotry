...
> **状态速览**:
- 2026-09-10 起,产物视图进入 M4 队列(issue #285):Host 持久化 `presentationMeta` 并输出 `SearchPathsResultView`/`ReadResultView` 所需字段;当前安装的 DSH Web client 只对内建 `read/glob` 工具名注册 keyed renderer,不消费自定义 `gotry_artifacts_*` presenter,实际文件预览走已安装的 workspace/sidebar 文件树。读范围白名单 = stateRoot 根 + 会话工作目录(排除 node_modules/.git),扩展名白名单 = 文本类;跨 root / symlink 越界 / 缺失文件 / 超大文件(>2MB)统一返回 ok:false + error + hint。本层只读,WriteGate 红线不涉及。验收证据 = `scripts/artifacts-capability-tests.ts` 12 项隔离 fixture proof + `scripts/dsh-artifact-e2e.ts` 隔离 Host execute/meta/presenter 与版本更新 proof + smoke §15/§15b;Web 自定义卡片交互仍是 TODO。
