...
- 2026-09-08 起,state-cli 租户参数边界(issue #226/#241):账本 CLI 集中解析 cmd/positional/`--state-root`/`--tenant`/`--limit`,未知/重复/缺值/非法 numeric 先于任何 state-root 副作用 fail-closed;#241/PR243(606d1d2)补 `.5`/`+.5` 小数边界但待主线集成;`tick`/`export`/`whatif` 明确 local-only,`whatif` 仅是整库管理员 snapshot,不是租户导出。
- 2026-09-08 起,M4→M6 program 任务图(issue #225):`milestone-delivery-plan.md` 把 #227 Z3 Node24 间歇 heap corruption 阻断、#242 dsh-map-tools map 回归、#241/PR243 CLI `.5`/`+.5` 小数边界补丁 606d1d2(待主线集成)、#223 M4 scorer 硬阈/隐私、#228 显式 opt-in planning lifecycle 采集/脱敏导出、真实 N≥5 repeat cohort、#136 M5 首供应链 `hotelbyte-cli` 协议核验 + WriteGate、#229/#237 tenant 隔离与 #236 state-cli 租户边界(代码已入 main,最终集成验收仍等 #227/#242 + 新 SHA)、#230 ledger 验收、#137 P6 评审/试点条件拆成责任面/责任文件/E2E/否证/退出标准;后续实现 issue #231–#235;`write-gate-production-design.md` 仅为 proposal,不启封交易或 B2B 实现。
