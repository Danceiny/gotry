...
- Phase 1 environment bridge 当前态:default-off owner-local v3 descriptor 与单一 flat `action=tools|call|errors` 协议同源；`tools` 做 discovery，`errors` 返回闭合的 bridge protocol/infrastructure recovery inventory，每个 `call` 分支把 tool 名与 closed/bounded `input_schema` 绑定，模型面与 spawn 前 validator 不分叉。adapter 只接受 exact `gotry_benchmark_tool_result_v1`：concrete result 受非空 `output_keys` 正向键合同约束，finite exact `domain_outcomes` 以 exit 0 进入模型历史，非零退出仍是 infrastructure failure；bridge 不自动 retry。conformance 以最新 bridge response 作为 tagged terminal 时序下界，旧终态不能遮蔽后续 domain/failure。cold-start、headless one-shot、native definition-only、host-enforced writes/network isolation 与 default-off 产品边界不变；当前没有可归因 official score/uplift。
