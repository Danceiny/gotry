...
- 2026-09-10 起,#271 外层 liveness 收口：GoTry launcher 将 dsh child 放入独立 POSIX process group；异常 close/error 与 parent-only SIGINT/SIGTERM 经过 bounded TERM→KILL，确认 direct child/group 清空后保留原始终止语义，既有 incident observer/writer 仍是唯一记录面。Node24 真实安装 dsh `0.1.5-alpha.1` 的维护 proof 覆盖 child crash、rejected promise、spawn ENOENT、SIGINT/SIGTERM，并核对 stderr、incident、post-failure marker 与 descendant/group 清理。dsh SDK direct transport、内部 supervisor 和真实 deployment contract 仍是开放 TODO；本切片不改 vendor/lock、不安装 daemon、不提升任何真实里程碑 gate。
