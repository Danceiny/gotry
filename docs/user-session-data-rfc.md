...
- **授权闸(支柱④进代码,2026-08-29 第二批,founder 口径「这要用到用户的账号,所以必须跟用户确认」)**:动用用户本人登录态的工具(`gotry_session_search`)在 dsh `tools/pre-execute` 返回 `{kind:'ask'}` → 运行时 `ApprovalService`(dsh-base 默认挂载)→ web 审批卡,**allowed-once 逐次批准**;rejected/cancelled/无审批通道一律 fail-closed(headless 无用户在场 = 无授权——「明示授权 + 可撤回」的运行时具象);插件 config `sessionAccess: ask|off` 总闸 =「随时可关」;站点白名单=适配器注册表现状(仅 ctrip-flight)。**飞猪匿名通道不过闸**(调用不携带用户身份,无账号风控/PIPL 处理面;其对用户的义务由「只读 + jumpUrl 人完成交易 + 配额限流结构化 error」覆盖);smoke §13 三态断言;
