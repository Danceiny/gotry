...
原始失败只留下四项可观测特征：退出码 1、用时 20088 ms、stdout 0 字节、stderr 731 字节；日志正文已丢失，原始环境未再复现。这四项可约束可能的耗时序列，但不能单独确定失败阶段。
...
这些耗时与 initialize 在 10000 ms 后超时、随后关闭阶梯再消耗约 10000 ms 的情况一致，但不足以确定原始失败所在阶段。当时的脚本没有模型请求计数或有序阶段记录，事后无法排除握手完成后、首个请求发出前的停滞。现有诊断可区分 start → initialize 握手 → 就绪 → 首个模型请求：请求计数非零可证明握手已完成，但计数为零本身无法区分握手未完成与后续停滞。生产端口已把三段宽限压到各 500 ms，并配一次性预热进程（`ts/src/booking-surface/dsh-planner.ts`）；包验证没有预热，沿用默认值。
