...
The original failure left four observable facts: exit 1, 20088 ms, 0 stdout bytes, 731 stderr bytes. The log body is lost and the original environment never reproduced. Those four facts constrain the possible timing sequence, but do not identify the failure stage on their own.
...
The timings are consistent with a 10000 ms initialize timeout followed by roughly 10000 ms of teardown, but they do not identify the original failure stage conclusively. The original script had no model-request counter or ordered stage records, so a post-handshake stall before the first request cannot be excluded after the fact. Current diagnostics distinguish start → initialize handshake → ready → first model request: a nonzero request count proves the handshake completed, while a zero count alone cannot distinguish an incomplete handshake from a later stall. Production ports clamp the three graces to 500 ms each and run a one-shot warmup process (`ts/src/booking-surface/dsh-planner.ts`); the package proof has no warmup and keeps the defaults.
