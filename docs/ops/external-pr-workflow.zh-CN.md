...
| `errors[]` 非空 或 `errors` 非数组；`data` / `data.repository` / `data.repository.pullRequest` 缺失或 null；`pageInfo.hasNextPage === true`（分页不完整）；hasNextPage 缺失或非布尔；`totalCount` 缺失或非整数；`totalCount !== nodes.length`；节点字段畸形 | 闭口失败 — exit 2（绝不静默放行） |
...
- GraphQL query 为 YAML 内常量字符串；PR 数字经 `[1-9][0-9]*` 校验，`count` 为常量 `100`；两者只传给常量 query
...
```bash
node scripts/protect-parent-triggers-tests.mjs   # 21 个夹具用例（含畸形 envelope 与分页不完整闭口失败）
```

**状态**：本节描述已实现的守卫。生产 workflow 在每个 PR 上的实际运行要等到引入此 workflow 的 PR 被 root 合入后才生效；在此之前，本地复跑 + 同 SHA 的 `gh api graphql` 实时证据即为权威。
