...
- **离线 sf evidence 汇总（issue #335， Tracks #272）**：`sf-summary.ts --evidence-root <临时根>` 先按 producer canonical evidence filename 盘点批次，再以批次采集时间选最新候选；保留每条记录的 requested/effective/fallback、per-record capture/start 与 manifest/batch provenance，旧无批次记录标为 unknown/ineligible，缺失或损坏的最新批次 fail-closed。选中批次只要有挑战／守卫证据（#411），包括完整八条记录或 legacy 顶层 `sessionVerdict`，同样 fail-closed，并带 `challenge_stop_detected` 标注。该确定性 fixture 证据不代表实时登录态、库存、当前接口 shape 或 packaged connected/degraded calibration。
...
- **#411 sf live benchmark 挑战即停（2026-09-12）**：`sf-live-benchmark` 在首个 `challenged`／`challenge_stop`／`guard_violation` 后停止——不重试、不绕过，后续 session 与 comparator 在结构上不可再被调用，`challenged` verdict 原样保留。只有终止原因为 `completed` 且八条查询全部执行时，`batch_complete` 才为 true。`sf-summary` 对选中批次的挑战／守卫证据统一 fail-closed，包括完整批次与 legacy 顶层 `sessionVerdict`。生产节律保持 35 秒；仅隔离离线 harness mock 子进程 timer。回归：run-all §44c 覆盖首条、中途、末条挑战与完整八条批次，并同时断言落盘 JSON、CLI 输出、请求计数、临时根和零网络。这仍是 #272/D-13 名下的离线工程证据，真实登录态校准归位不动。
...
- **#411 挑战即停子任务（2026-09-12）**：sf-01..08 live benchmark 批次在首个 `challenged`／`guard_violation` 即停（RFC §3.5），保留结构化 `challenge_stop`／`no_spend_stop` 语义，并落盘可审阅的批次（`stop_reason` + 已尝试／未尝试清单）；挑战／守卫终止即使八条记录齐全也将 `batch_complete` 置为 false，`sf-summary` 经 `challenge_stop_detected` 对选中证据 fail-closed，并识别 legacy 顶层 `sessionVerdict`。生产节律保持 35 秒，只有隔离 harness mock 子进程 timer。仅离线工程证据（run-all §44c），真实浏览器／会话校准仍归 #272。
