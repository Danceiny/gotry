...
- **#341 地面接驳第一切片（#364 方向绑定）**：`gotry_feasibility_check` 只接受显式起点/终点经纬度与 `mode=driving`，通过已注册公开 `map_driving_route` 结果生成带 `asOf`、freshness/cache、provenance 与 evidence class 的路线事实；抵达方向（A→B）与返程方向（B→A）分别请求、缓存与降级，任一方向 miss/error/stale/不匹配仅回退该方向到静态估算，不借用另一方向的动态值；仅改具名 destination 的静态 `taxi` transfer 分钟（持有 `minutesOut` / `minutesRet` 覆盖供 `evaluateChoice` 按方向消费），`bus`/`bus_plus_taxi` 保持静态值并返回明确回退，静态价格始终标 `[静态包:估算]`。`asOf` 是宿主观察/查询时间而非 provider 发布时间；缓存命中与过期重查可见，provider 失败、非法坐标、不支持模式和绑定不一致均明确保留静态值；不覆盖 live traffic、transit/rail/fare，也不改变 #20/M5/M6 既有准入边界。2026-09-11 边界冻结套件把该离线边界钉闭（词汇封闭、含自相矛盾路线事实的完备回退分类、900 秒切换契约与证据标注完整性）；更广的剩余项是 gated 真实数据工作，非工程残余。
...
- **离线 sf evidence 汇总（issue #335， Tracks #272）**：`sf-summary.ts --evidence-root <临时根>` 先按 producer canonical evidence filename 盘点批次，再以批次采集时间选最新候选；保留每条记录的 requested/effective/fallback、per-record capture/start 与 manifest/batch provenance，旧无批次记录标为 unknown/ineligible，缺失或损坏的最新批次 fail-closed。挑战截断的部分批次（#411）同样 fail-closed，并带 `challenge_stop_detected` 标注。该确定性 fixture 证据不代表实时登录态、库存、当前接口 shape 或 packaged connected/degraded calibration。
...
- **时间感知**：确定性锚点层 `ts/src/time-anchor.ts` + 槽位抽取 `travel-slots.ts` + 槽位→日期解析 `slot-spec.ts`。**算术进代码，LLM 查卡不自算**。命名年份（四位年份，或按本轮锚点换算的相对年份：今年、明年、后年、年内）的未来规划在 `loop.ts` 的求解入口按本轮锚点过滤候选：当前年从参考日到年末，未来年为全年；过期年份明确拒绝，历史/回测意图旁路，不把参考日持久化到 `TripState`。
...
- **HotelByte 交易 bridge 与 unknown 查单对账契约切片（2026-09-11，issue #232，M5 备产）**：`ts/capabilities/hotelbyte-transaction.ts` 以纯函数落地 §7 对账状态机——book 执行分类（CLI exit0 非成功证明；超时/被杀/非 JSON/不可归因成功/部分确认一律归 unknown；显式业务失败归 supplier_failed）；对账种子 fail-closed（attemptId/customerReferenceNo/intent 幂等键/请求指纹缺一即拒；fact_id/审批 receipt 摘要/outbox 占位可追溯，审计事件带 probeDigest）；查单对账——恢复窗口（180s Phase1 + 最长 10min Phase2）内 miss 保持 unknown 且结构性禁止重订；窗口届满本身不是无订单证明，只转人工；绑定 attempt 的权威终态负证据（供应商取消/失败、带操作证据的人工无订单确认）是唯一允许新 intent 的路径；同 ref 多单/参考号不匹配/终态反证显式置 conflict 交人工复核，不静默当成功也不静默丢弃。outbox/dispatch 持久化归 #231，本切片只消费 `outboxIntentRef` 占位 ID。**非运行时激活：零真实供应商调用**（不 spawn 子进程、不联网、不读凭据；issue #318 的 `GOTRY_HBCLI_LIVE` opt-in 门保持默认关）；全部行为由 `ts/scripts/hotelbyte-reconcile-tests.ts`（run-all §60）离线证明。**spawn 级假 CLI 完整链路 E2E 已落地（issue #232 §5，PR 待链）**：`ts/scripts/hotelbyte-spawn-e2e-tests.ts`（run-all §66）以真实 `child_process` spawn 本地 shell fixture（可编程形态：正常 quote/book JSON、挂起、非 JSON 垃圾、pending 部分确认、exit0 无绑定成功、kill -9、显式业务失败；查单 miss/confirmed/auto-canceled/processing/多单/denied/crash/garbage），跑通 quote（check-avail）→approval（L3 receipt 摘要）→book→unknown（enterUnknown）→query-orders（advanceReconciliation）全链——套件内在同一批真实 spawn 输出上证明「无对账层的裸调用」（只看退码）会处理错（timeout 被判可重订失败→双订风险；exit0 垃圾/无绑定/部分确认被判成功；窗口内 miss 被判无订单），且每个场景断言契约终态分类与对账动作：timeout→miss→迟到成功收敛 reconciled_success 不重订；timeout→miss→迟到自动取消收敛 reconciled_failed（唯一新 intent 路径）；pending→processing→窗口届满转人工仍保持 unknown；多单显式 conflict；argv 回传证明 customerReferenceNo/ratePkgId 绑定穿越真实进程边界。唯一被执行的二进制是临时目录下的 fixture 脚本——零真实 hbcli/网络/凭据，所有 spawn 有界超时，`GOTRY_HBCLI_LIVE` 默认关且不参与。
- **sponsor 插件与同内核端到端复用证明切片（2026-09-11，issue #235，M6 备产）**：`ts/capabilities/sponsor-plugin.ts` 以纯函数在不改内核的前提下落地 sponsor 插件契约面——traveler principal/sponsor/BFF principal 三主体分离（构造 fail-closed；sponsorId 混同 traveler principal 即拒）、授权路由闭集 + session/order 绑定强制，覆盖复用走查的三个插件槽（入口/库存池/披露配置）。复用证明是结构性的，不是口号：模块只 import 内核函数（`booking_saga_fsm.v1` 状态机 + wish-pool 条件评分）零拷贝；`ts/scripts/sponsor-reuse-tests.ts` 从消费侧逐格钉死内核边表/拒绝表（内核漂移即套件红）；同一 runner（`runBookingScenario`）驱动 B2C/B2B 两种形态且 saga 边序列逐格相同——PR 的内核文件集 diff 为零。红线结构性编码：排序前强制内核条件全命中（佣金不可赎回未命中的旅行者条件；同分 tie 只按 sponsor 申报顺位且必须披露偏置；佣金数值从不进入排序键）；sponsor 披露摘要进入确认指纹（披露口径变化 = 指纹变化）；空 receipt 两种形态同拒（同一 WriteGate 规则，无 B2B 豁免）；取消退款走同一内核边表且回放零违例；A/B 同业务 id 场景按主体 scope 分区——事件/指纹/approval 互不串，跨 scope 重放 approval 指纹绑定失败。隔离：`ts/src/**` 零 sponsor-plugin 引用（C 端默认路径不动），B2C trace 过 `sponsorLeakFindings` 零发现。**非运行时激活：默认关，零真实调用**（`SPONSOR_RUNTIME_ACTIVATION_DEFAULT=false`；不 spawn 子进程、不联网、不读凭据；真实激活在 #137 M6 Entry 之后）；全部行为由 `ts/scripts/sponsor-reuse-tests.ts`（run-all §64）离线证明。真实旅行者效果与试点签约仍属 #137 M6 Exit，fixture 不可替代。
...
- **Booking unavailable/changed 恢复链契约切片（2026-09-11，issue #142 非门控切片）**：`ts/src/booking-surface/recovery-chain.ts` 以纯函数落地恢复链的结构化处置——检测把 action receipt 分类进既有 `ActionReceipt.status` 闭集（confirmed/changed/unavailable/inconclusive/unrelated；不连贯凭证 fail-closed，抛 `availability_receipt_incoherent`/`availability_observation_required` 同名错误；不可确认证据永远不得宣称市场无房）；受控恢复记录 requery/换版复查/替代候选三步闭集，计入 planner 三次调用预算语义，并以冻结引用对齐 availability-policy 的每酒店预算；用户沟通是结构性红线——落到不同酒店/报价必须先显式披露（静默换房换航拒绝）、版本位移必须携显式新旧版本说明、恢复耗尽只能携非空披露降级；审计事件走 `booking.copilot.recovery.*` 账本命名空间，每条带 receipt 摘要，链校验对齐 saga 形态、resolved/degraded 吸收。检测与合法性仍归 availability reducer（ADR-23）所有；本切片不接线 runtime seam，不动求解/评估分层。**非运行时激活：零真实供应商调用**；全部行为由 `ts/scripts/booking-recovery-chain-tests.ts`（run-all §65）离线证明。#142 gate 的「四 surface 真实库存」与「真实 Checkout/QueryOrders 证据」仍 gated no-spend，等待真实供应商/UAT。
...
- **#411 sf live benchmark 挑战即停（2026-09-12）**：`sf-live-benchmark` 在首个 `challenged` 后仍跑其余七条，且把该 verdict 改写为 `error`，双源裁决塌缩成 `source_unavailable`，丢失 `challenge_stop` 语义（RFC §3.5 要求触发验证码／风控即停止自动查询）。修复：批次在首个 `challenged`／`challenge_stop`／`guard_violation` 后立即终止——不重试、不绕过，后续 session 与 comparator 在结构上不可再被调用；`challenged` verdict 原样保留（`SessionVerdict`／`SessionEvidenceVerdict` 本是同一闭集，改写纯属多余）；run summary 新增 `stop_reason` + `batch_complete` + 已尝试／未尝试清单，构成可审阅的部分批次；`sf-summary` 新增 `challenge_stop_detected`，此类批次保持 `fail_closed`（绝不标完整／有效校准）。`GOTRY_SF_INTER_QUERY_DELAY_MS` 仅供离线测试缩短节律（非法值回退 35s 默认，不动停止逻辑）。回归：run-all §44c 以确定性 session 模块 overlay 驱动真实 CLI runner——首条挑战、中途挑战与完整八条批次三场景，输出结构与请求计数同时断言，临时根零网络。这仍是 #272/D-13 名下的离线工程证据，真实登录态校准归位不动。
...
- **Issue #2 未来年度规划窗口（2026-09-10；相对年份 2026-09-11）**：命名年份的未来意图——四位年份，或按本轮锚点换算的相对年份（今年、明年、后年、年内）——在 `time-anchor.ts` 派生本轮 `referenceDate`；注册工具 `gotry_feasibility_check` 对任何带日期候选默认施加宿主时钟 future 下界，显式 `planning.intent=future` 的请求年份再限制至年末。已结束年份不自动滚年，预期拒绝返回结构化校验结果而不进入 incident；完全 dateless 输入保持旧可行性计算，历史/回测必须明确 `historical` 模式。loop 继续只对显式给出的规划年份（四位或上述相对形式）应用窗口，否定过去推荐和多年份歧义不授予 historical 旁路。`time-eval-tests.ts` §6 与 `smoke.ts` 的 registered execute fixture 分别验证规划循环和实际 registered execute；参考日来自注入/宿主时钟，证据为隔离 fixture，不构成供应商或真实业务准入。
...
- **Issue #233 取消/退款独立结果与佣金披露契约层（2026-09-11，M5-4 pre-entry）**：write-gate 设计 §7/§8/§10 与 fact-writegate seam 的取消/退款/佣金措辞权威落为纯契约模块（`ts/src/booking-surface/cancel-refund-commission.ts`，run-all §62）——取消与退款是两个结果对象、状态封闭集互不重叠（合并成功态不可表达；「取消已确认 + 退款失败/待到账」的非对称终态可表达）；「已取消」措辞必须持有供应商显式取消回执，仅有提交记录时只写「已提交取消申请」；「已退款」结论必须绑定供应商退款/钱包退款权威证据，`cancel.serviceFee` 永不充当客户退款金额；佣金/返利/赞助披露（谁付/给谁/金额口径）在确认边界强制在场，unknown 不默认 none，其 canonical-JSON SHA-256 digest 绑入指纹字段 `commission_disclosure`——口径变化即 digest mismatch，旧 receipt 失效需重确认；分词闸保证账本分词与用户文案一致（仅 compensated 不产生退款到账措辞；退款待到账不得写成已到账）。**纯契约、default-off：零供应商/支付调用面、零账本/schema 变更、不接线任何 runtime 路径；#231 → #232 → #233 实施顺序与 M5 Entry/Exit 不变——这是 fixture 级工程证据，不构成真实交易准入。**
...
| D-39 地面接驳数据边界 | 离线边界已闭合并有机械钉住（2026-09-11 边界冻结套件，run-all §3b）：显式坐标驾车路线经公开 `map_driving_route`、具名静态 `taxi` transfer 的抵达方向（A→B）与返程方向（B→A）分别绑定与按方向隔离的缓存/降级、`minutesOut` / `minutesRet` 覆盖消费、静态价格保留、模式/位置词汇封闭（仅接受 `position=destination` 上的 `mode=driving`；seam 的 transit/walking/bicycling 路线一律拒绝、绝不伪装）、按方向完备的回退分类（provider error/miss——含「正距离零耗时」类自相矛盾路线事实——stale 重查失败、模式/位置不匹配、非法坐标、绑定不一致）、静态↔动态切换契约（固定 900 秒 TTL 边界、同分钟身份切换、按方向有界缓存）与证据标注完整性（每条路径保留静态价格标注、`trafficStatus: not-live-route-estimate`、宿主观察 `asOf`）。剩余为 gated 真实数据工作，另行授权前保持 no-spend：live traffic、transit/rail、fare、地址解析与更广泛 transfer 组合。该切片不改变 #20/M5/M6 gate；后续边界按 [#341](https://github.com/Danceiny/gotry/issues/341) 追踪。 |
...
- **#411 挑战即停子任务（2026-09-12）**：sf-01..08 live benchmark 批次在首个 `challenged`／`guard_violation` 即停（RFC §3.5），保留结构化 `challenge_stop`／`no_spend_stop` 语义，并落盘可审阅的部分批次（`stop_reason` + 已尝试／未尝试清单）；`sf-summary` 经 `challenge_stop_detected` 把此类批次标为 fail-closed。仅离线工程证据（run-all §44c），真实浏览器／会话校准仍归 #272。
