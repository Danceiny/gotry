...
- **工具描述首行生成 + doctor 宿主插件覆盖面(2026-09-04,issue #113,L1 残量收口)**:七个检索工具描述统一前置 channel-registry 生成的「服务意图 × 通道顺位」卡(选工具与失败改道两决策点同表,注册表加行自动一致);doctor 补齐 patch 宿主插件 `dsh-map-tools`/`dsh-tool-ask-user` 两态照亮(此前解析失败整块静默剔除无人知)——实测暴露 source 布局 dsh-map-tools 全布局缺席,地图工具一直被静默丢弃,补依赖渠道留 founder 拍板。**落地(2026-09-08,issue #202 热修)**:npm 依赖形态因 peer 冲突破坏严格 npm ci,改为自带 MIT payload(`vendor/map-tools/`,零运行时依赖,alpha.3 settings API 实证匹配),三面解析候选同加,map-payload-tests 5/5(run-all §55),随下个 npm 版本分发。
