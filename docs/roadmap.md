...
- **HotelByte Booking Copilot 产品验收并行线**:GoTry 以 v1/v2 共用单 BFF listener、task ownership 的 typed read actions 提供协作面，v2 为六个生命周期阶段、七个 phase 字面值（`terminal`/`error` 是两种终态结果）的 durable projection，生产 standalone 默认只接受 BFF 已绑定的 `user.turn`/receipt continuation，完整 principal + binding seam 才开放 `user.turn.ingress`，v1 保持 legacy 两态，`Book` 留在原 Checkout。tenant/customer 已有 canonical condition option 的 typed approval 本地垂直证据：精确回传、一次执行，mismatch/stale/expired/replay 在动作副作用前关闭；Draft 候选已有 exact SHA/schema/Linux Node 24+ABI provenance 与实际进程 health identity/ingress mode。合并 gate 仍是 tenant/customer/storefront/payment-link 四 surface 真实库存、unavailable/changed 恢复链及 Checkout/QueryOrders/清理证据。该线不启封 M5，也不以离线合同或 CI 替代业务验收。
...
- **Round 6 terminal diagnostics 当前态**:benchmark child 只观察最终结构化 `turn/end`，将模型/运行时与 blocked/max-token/aborted/interrupted 收敛为 allowlisted coarse enum；未知值统一 generic，message/raw stderr/path/prompt/key 不进入 FD3。per-session arbiter 仅写一次并保留更具体的 bridge/conformance 原因；恢复成功的 transient retry 不写失败。代码 SHA `c61600b` 的 ChinaTravel frozen treatment（`..._00001`，`deepseek-v4-flash`）在 49.546 秒后稳定输出 `child_runtime_error`，但仍是 0 terminal bytes、evaluator/official scores null；leakage 与本地凭证/端点扫描均为 0。该切片不改 retry、prompt、tools 或 scorer，也不产生 uplift；可评分终态与官方指标仍待独立 evidence。
- **Round 7 minimal kernel 当前态**：代码 SHA `edb9392896625adbb48abae4a2ecf968dbfc0349` 的 benchmark opt-in 仅保留 tool budget、model override、唯一 bridge、isolation/conformance；不装产品 prompt variables、process guards、consent 或普通工具，默认路径不变。system-prompt 投影为稳定 task-agnostic persona，root patch 仅接受 canonical `insert` 与 `system-prompt` 各一，其他 root item/变体 fail-closed。ChinaTravel frozen treatment UID `e20241028160248698752`（`easy`，`deepseek-v4-flash`）preflight pass、未回退，80.463s 后 runner exit 1，terminal 0 bytes/invalid，evaluator 未进入、official null、不可计分；白名单归因为 `child_bridge_runner_failed`，不产生 uplift 或 external benchmark closure。下一轮聚焦 generic bridge tool schema 与可恢复 domain-error contract。
- **Round 8 typed tool/result contract 当前态**：bridge v3 用单一深冻结 descriptor 声明 closed/bounded `input_schema`、必填非空 `output_keys` 和有限 `domain_outcomes`，同一 schema 同时进入 native request 与 spawn 前 validator；adapter 只接受 exact versioned ok/miss/error envelope。每个 primitive leaf 必须位于 declared output key 之下，顶层 record array 可用而无键 primitive/mixed array fail-closed。声明式 domain miss/error 必须 exit 0，恢复由模型依据固定 recovery enum 主动修参再调用，bridge 不自动 retry；非零退出继续归 infrastructure failure。conformance 区分 concrete result、domain outcome 与 failure，且 tagged terminal 必须晚于最新 bridge response：domain-only 可收敛，后续 infrastructure failure 拒绝，后续 concrete result 可恢复，旧 terminal 不得遮蔽更晚事实。source synthetic E2E 已覆盖两次模型调用与两次 adapter 执行，但 frozen treatment 尚未运行，provider/scorer/evaluator 未改，不产生 score/uplift 或 external benchmark closure。
...
**M3 工程面已推进但 Exit 未关闭；M4 自 2026-08-26 起由 founder 授权并行推进，不是 M3 Exit 证明。** M4 记忆域 T1 行为链已闭合（动态吸收→读回→效用→触达→度量），六层重设计已落地 `memory-design.md`，分期增量为 P1 旅行时间线→P2 同行人档案→P3 时间窗衰减→P4 双区会话后置。
  ...
  - **2026-08-30 同批 onboarding UX 闭环(issue #21 P3.6,后于 2026-09-02 商店上架后撤销,§3.3 职责返交)**:初版 5 步编排 + 剪贴板扩展路径 + 跨平台 GUI 面板(macOS osascript / Linux zenity / Windows msg / headless 终端降级,**不引 Electron**)+ 后台 health-watch ≤120s 探活 + 扩展一就位 stdout 翻绿自动重放同 query_id——用户侧初版降至 **3 次点击 + 0 次终端命令 + 装完零重跑**(对照上版 5 次点击 + 1 文件对话框 + 跨 app 切换 + 装完自己重跑);
