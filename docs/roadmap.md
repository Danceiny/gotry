...
- **账本 CLI 租户解析边界(2026-09-08,issue #226/#241)**:`state-cli` 集中解析 cmd/positional/`--state-root`/`--tenant`/`--limit`,未知/重复/缺值/非法 numeric 在触碰 state root 前拒绝;`tick`/`export`/`whatif` 明确 local-only,避免默认 local 异步结算、共享 legacy 文件名覆盖或整库 snapshot 被误当租户导出。`--tenant` 仅为账本 scope,不构成认证授权。#241/PR243(606d1d2)补 CLI `.5`/`+.5` 小数边界,当前 main 的 #236 尚无该补丁,需主线集成复验。

- **M4→M6 program 任务图(issue #225,2026-09-08)**:`docs/design/milestone-delivery-plan.md` 将 #227(Z3 Node24 间歇 heap corruption 合并阻断)、#242(dsh-map-tools map 回归)、#241/PR243(CLI `.5`/`+.5` 小数边界补丁 606d1d2,待主线集成)、#223(M4 scorer 硬阈/隐私)、#228(显式 opt-in planning lifecycle 采集/脱敏导出)、真实 N≥5 repeat cohort、#136(M5 首供应链 `hotelbyte-cli` 协议核验 + WriteGate,实现 #231/#232/#233)、#137(P6 评审+试点条件,实现 #234/#235)拆成 living 任务图;tenant ledger scope(#229)+ fold 回归(#237)+ state-cli 租户边界(#236)代码已入 main,ledger 验收 #230 已关闭,但最终集成验收仍等 #227/#242 与新 SHA 全栈证据。`docs/design/write-gate-production-design.md` 是 M5 proposal(可信 receipt 发行/消费权威、approval_claims 持久化、query miss 保持 unknown、HotelByte 版本/发布物区分),不启封交易实现;`docs/milestones/m6-b2b-reuse-walkthrough.md` 仍 draft,待 founder 评审。
