...
- **事实闸覆盖面·政策锚点全字段内容指纹(2026-09-10,issue [#359](https://github.com/Danceiny/gotry/issues/359),父 [#273](https://github.com/Danceiny/gotry/issues/273),D-26 残余收口)**:`policyCanonicalBody(p, tripStart?)` 与 `renderPolicyFact` 共用同一构造器,作为政策行 canonical body 的单一权威面;闸侧 anchored policy 行(去锚点段)全文比对 —— `subject`/`statement`/`source`/`fetched_at`/`query_id`/`as_of` 任一改动 → `fact_anchor_unknown` fail-closed,与未知锚点同源。`review_by`(事实自带)与 `tripStart` 派生提醒两种合法形态保留,闸侧 `opts.tripStart` 与渲染同一传入即一致;不靠宽松 substring 匹配,不通过删除提醒或删除正例得到 green。fact-gate-tests §14 14 断言(canonical + 5 failing-before + 2 legitimate + 多行归属);isolated E2E `scripts/policy-anchor-359-e2e.ts` 走真实事实日志(`appendFacts` → `loadFactRegistry`)并以 registered fact-gate definition 验证篡改行 fail-closed,不启动 dsh 宿主、不触碰真实供应商/browser/policy service/共享状态。确定性 synthetic / isolated evidence,不宣称 live policy、supplier、M5/M6 准入。
