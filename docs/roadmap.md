...
**v0.1.0 已发布**(tag,`31f8f41`+`5e06abd`):薄壳七段全部完成——品牌三页/意图路由三场景(洱海候选/云南同伴感知/普吉 workation)/休假语义/会话持久化/云南包(海拔递减+高血压红线为引擎一等数据)/Markdown 渲染/UX(加载指示器/新行程/优雅降级)。README 重写为用户视角+.env.example。**M3 剩余:种子用户 50-200 人(工程前提全部就绪,`./gotry shell` 即入口;等 remote 配置)**。
