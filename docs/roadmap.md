...
- **Phase 1 bridge 当前态**:Round 2 已有 default-off owner-local bridge contract，固定 executable/cwd/argv_prefix/allowlist 与 timeout/output caps；config 要求 writes forbidden/network denied，但真实 enforcement 由 owner/host OS 负责，synthetic E2E 不证明 OS sandbox。bridge 不引入 Python/benchmark runtime 产品依赖；frozen ChinaTravel treatment 仍未运行。
