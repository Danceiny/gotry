...
- **Issue #343 IANA tz offset 替换手写残余(2026-09-10)**:候选航班包 v2 路径走 `ts/src/tz-resolver.ts`(缓存 `Intl.DateTimeFormat`,无新依赖);DST gap/overlap 与未知 zone 在 parse 边界拒收;home zone 工作窗口通过 `readWallParts(homeZone, depUtcMs)` 直接投影已知 instant 到 home zone 墙时(权威),不再用 dep-local date 当 home date。`ts/src/model.ts` 的 `doorToDoorFromMove` 是门到门算术唯一权威入口(v2 用真实 UTC instant,v1 用墙上分钟差 - tzOffsetMin,字节不变);真实入口 adapter 只合并 profile schedule 并保留 pack homeZone,#343 真实 provider/session calibration 仍为 TODO,不扩展通用时区 NLP。当前本地证据为 33 项 focused tz-resolver、18 项 sanity、unified 4/4 与隔离 smoke,`data/flights_2026.json` v1 兼容编译。public path 仍只证明离线 fixture,不替代实时班期/票价/可售库存;全仓前台回归仍待执行。
