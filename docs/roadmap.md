...
- **账本 CLI 租户解析边界(2026-09-08,issue #226)**:`state-cli` 集中解析 cmd/positional/`--state-root`/`--tenant`/`--limit`,未知/重复/缺值/非法 numeric 在触碰 state root 前拒绝;`tick`/`export`/`whatif` 明确 local-only,避免默认 local 异步结算、共享 legacy 文件名覆盖或整库 snapshot 被误当租户导出。`--tenant` 仅为账本 scope,不构成认证授权。

- **M4→M6 program 任务图(issue #225,2026-09-08)**:`docs/design/milestone-delivery-plan.md` 将 #227(Z3 Node24 间歇 heap corruption 合并阻断)、#223(M4 scorer 硬阈/隐私)、#228(显式 opt-in planning lifecycle 采集/脱敏导出)、真实 N≥5 repeat cohort、#136(M5 首供应链 `hotelbyte-cli` 协议核验 + WriteGate)、#224(tenant ledger 隔离,已关闭但供 M6 proof 引用)、#226(state-cli 租户边界)、#137(P6 评审+试点条件)拆成 living 任务图;`docs/design/write-gate-production-design.md` 是 M5 proposal,不启封交易实现;`docs/milestones/m6-b2b-reuse-walkthrough.md` 仍 draft,待 founder 评审。
...
### M5:交易闭环
- **Entry**:M4 exit + 供应链协议。首供应链用户已选 `hotelbyte-cli`(参考 `/Users/danceiny/work/hotel-be`,只读调查中;商业协议未签,不复制内部代码)。**交付**:WriteGate 生产化——写权按 L0-L4 渐进授权(L2 建议/L3 具名 seam 确认带 receipt/L4 自动类),每级可回滚(RFC S4);预订/支付/退改;佣金披露(红线随行)。**设计基座已备(2026-08-29,issue #17 采纳/ADR-17)**:预订 saga 状态机词汇层(`booking_saga_fsm.v1`,`ts/src/booking-saga.ts` + `docs/design/booking-saga-fsm.md`)——字母表/四条边全函数边表/拒绝闭集/审计链校验,run-all §36 与账本物理对账。生产化 proposal 见 `design/write-gate-production-design.md`:request fingerprint/receipt 绑定 actor/tenant/金额币种/条款/有效期,一次确认,并发/崩溃恢复,HotelByte 30s/unknown/query-orders/Buyer/selector/OTP 缺口,取消与真实补偿分词,L4 撤回与佣金披露。启封时任何 booking seam 只许走该边表;空 receipt 物理 CHECK、seam 命名词汇、L2/L4 接线与审批等待态为 M5 交付物。
- **Exit**:预订零误操作事故;单位经济实测(对齐 D1 §8)。

### M6:B2B 包裹
- **Entry**:M5 exit + P6 推演确认。P6 draft 见 `milestones/m6-b2b-reuse-walkthrough.md`,仍待 founder 评审;#224 tenant 事件归属/投影重建隔离已关闭但仍需在 proof 中引用其证据,#226 state-cli 租户边界仍是 sponsor proof 前置安全门;#227 未解决前不把一次回归绿当 proof 稳定性证据。**交付**:traveler principal/sponsor 插件化,一个 B2B 场景(旅行社嵌入)零内核改动跑通;复用率必须带基准 SHA、内核文件集合、runtime loaded modules 分母与覆盖率,不得把 sponsor 收益或零内核 diff 当 traveler 效果代理。
- **Exit**:工程 proof 报告内核 diff=0、runtime 复用率实测数字(含基准 SHA/文件集合/分母/加载覆盖)与旅行社嵌入 E2E;商业半面为 B2B 试点签约或明确未签原因。
