...
- **酒店日期输入闸(2026-09-09,issue #283)**:founder 截图实证 `gotry_hotel_search` 入参 `checkIn`/`checkOut` 为空字符串时仍发起 hbcli 按当前窗口价返回(用户要下周五给的是今天的价)。新增纯函数 `ts/src/hotel-date-gate.ts` `evaluateHotelStayDates`,复用既有 `buildTimeAnchor`/`resolveSlotDate`:缺/空/单侧/非字符串/无法解析/倒序/同日 → 直接 `verdict=input_required` 五字段(reason/missing/raw/message/evidence)一致,不发 hbcli 不写 bridge-latency 不触发 ensureStateDir;通过闸进入既有 HBCLI_HOTEL_SEARCH 效应解译层与 `--check-in/--check-out` 旗标;presentResult 渲染为「酒店:<dest> 需要入住日和退房日」行动卡(summary 直传,正文给出具体日期示例)。16 断言维护测试覆盖空/完全缺失/单侧(双向)/混合缺失-空白/倒序/同日/无法解析/类型错误/有效日期 argv/自然语言解析(口径同 slot-spec,跨月用 Date 自动进位)/锚点卡词表/bridge-latency before-after 零增量 +1 行增量/隔离用户状态/空白串/presentResult 行动卡,隔离 stateRoot + 临时 fixture hbcli 跑真实 apply→execute→fixture 边界,无 HotelByte/凭证/共享用户数据写入;run-all §30d。该修复只闭合语义缺口,不启封 M5 写闸、不变 Booking 库存业务效果(D-29)。
