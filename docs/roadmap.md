...
- **事实闸覆盖面·政策锚点全字段内容指纹(2026-09-10,issue [#359](https://github.com/Danceiny/gotry/issues/359),父 [#273](https://github.com/Danceiny/gotry/issues/273),D-26 残余收口)**:`renderPolicyFact` 与注册 `gotry_fact_gate` 共用单一权威构造器;锚点行必须严格等于 canonical 整行,subject/statement/source/fetched_at/query_id/as_of 任一改动 → fact_anchor_unknown fail-closed,与未知锚点同源;锚点行不得含前/后置非空文本、缺失/重复/格式异常锚点。`opts.itinerary.trip_start` 桥接到 `opts.tripStart`,与 `review_by` 同为合法复核提醒形态;不通过删除提醒或删除正例得到 green。确定性 synthetic / isolated evidence,不宣称 live policy、supplier、M5/M6 准入;父 #273 残余债务保留。
