...
- **typed 参数契约迁移(2026-09-04,issue #112,D-30,两刀)**:四个高流量工具(flyai/session/hotel/weather)参数面 query blob → dsh typed ParameterSchemaSpec 平铺字段——模型可见逐字段 JSON Schema;flyai(kind required)与 hotel(destination required)宿主权入口即拒畸形参数(ToolFailure 形状不变),session 无公共 required 保留 interpretArgs 容忍层(blob 归一照走业务闸);迁移锁 smoke §12/channel-registry-tests §8。普通模型对参数形状不再靠猜。余量工具迁移与 flyai canary 记 D-30。
