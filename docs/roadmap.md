...
- **Issue #2 未来年度规划窗口(2026-09-10)**:命名年份的未来意图由本轮时间锚点派生;在 `runTurn` 与注册工具 `gotry_feasibility_check` 的求解前边界,任何带日期候选即使省略 planning 也默认过滤参考日前日期,显式 future 年份再限制至年末。预期日期拒绝返回结构化 validation result,过期请求不滚年,完全 dateless 保持旧计算,历史/回测必须显式 historical;否定过去推荐和多年份歧义不误判为历史。`time-eval-tests.ts` §6 + `smoke.ts` 的 registered execute fixture 以注入时钟验证,隔离证据不构成真实供应商准入。
