...
- **typed 参数契约迁移(2026-09-04,issue #112,D-30,三刀)**:11 个工具参数面 query blob → dsh typed ParameterSchemaSpec 平铺字段——四高流量(flyai/session/hotel/weather)+ 七高频(session_login/doctor/artifacts_list/artifacts_read/fact_gate/agent_reach/turn_handoff_list);刀法总纲:有 required 字段的工具宿主权入口拒畸形参数(ToolFailure 形状不变),全可选工具保留 interpretArgs 容忍层(blob 归一照走业务闸);agent_reach 的 action 不设 enum(反射桥开放面);迁移锁 smoke §12/channel-registry-tests §8。普通模型对参数形状不再靠猜。余量工具迁移与 canary 记 D-30。
