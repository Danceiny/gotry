...
- **#279 携程机票 malformed 响应闸(2026-09-10)**:合法空列表= `miss`,有效命中= `hit`,未知/畸形形状= `error`;兼容 `parseBatchSearch` 永不抛错,扩展/CDP 均不把 parser error 伪装成 miss。隔离扩展 fixture 仅证明本地解析/编排,不满足 #272 live interface calibration、真实 supplier evidence 或 M4/M5/M6 admission。
 - **Issue #343 IANA timezone behavior**:flight-pack v2 resolves explicit IANA zones and local dates to UTC instants, rejects unknown zones and DST gaps/overlaps, and uses UTC instants for elapsed duration. The dsh/mock adapter path retains the v2 pack `homeZone`; profiles supply schedule only, explicit vacation removes the trip work-window restriction, and numeric v1 behavior remains compatible. This deterministic contract does not prove live schedules, prices, availability, or inventory; see [`docs/data-sources.md`](data-sources.md).
  - **#352 FlyAI transport malformed 响应闸(2026-09-10)**:精确空 `data.itemList: []` 保持 `miss`;非空列表只要含 malformed sibling 即返回结构化 `error`,不生成负库存事实;仅完整 typed batch 才 `hit`。`flyai-tests.ts` 为隔离假 CLI/production effect/fact sidecar 的离线工程 proof,不构成真实 provider/UAT 或 issue closure。
...
- **事实闸覆盖面·机票/车次 claim 分流(2026-09-10,issue [#299](https://github.com/Danceiny/gotry/issues/299),父 [#273](https://github.com/Danceiny/gotry/issues/273))**:完整 G/D/C/Z + 3–4 位车次进入独立 `trains` 集合,不误删 `CZ8582`/`9C8781` 等合法双字母航司;flight/rail verdict 按 `kind` 隔离,FlyAI train 结构化 exact-date hit/miss 沿真实工具→typed fact log→canonical anchor→gate 路径,缺 train 证据输出 `rail_claim_unverified`;12306 session 暂不接 fact registry,须先补 typed parser outcome + seat availability/freshness contract,列表不补票价。回归在 fact-gate-tests 与 smoke 的隔离 seam,仍仅确定性 fixture 证据,#299/#273 保持 open,不宣称 M4/M5 closure 或真实业务证据。
