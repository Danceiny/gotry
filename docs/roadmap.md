...
- Round 1 聚焦 GoTry 自身:模型每轮第 16 次真实工具派发注入收敛上下文,第 18 次是最后一个 body；同一步第 19 次起返回 `TOOL_BUDGET_EXHAUSTED`,该批 `step/end` 后下一步 native 请求抑制继承工具 schema。run-all §45 已覆盖 Cordis integration/并行/turn+session 生命周期及真实 npm-mode dist + dsh headless 离线 E2E(18 body、1 structured refusal、下一请求 text-only、非空 final)；CI 以当前 SHA tarball 的隔离 pnpm consumer 入口重放同一链路,不借 root 开发树。**Round 1 后续(ADR-24)**:真实轨迹(用户安排行程)显示「步数预算」对产品用户体验是负向的——`TOOL_BUDGET_EXHAUSTED` 作为 tool result 回灌后模型在同一 step 内继续派发 3+ 次工具调用、token 截断、用户拿不到答复。闸语义从「步数」换为「wall-clock」(默认 60s/120s,可由 env 覆盖);产品路径(`apply()` 默认分支)不再装闸,benchmark opt-in 路径才装——产品用户不再被评测闸误伤。run-all §45 改名 `agent-planning-turn-deadline-{tests,e2e}.ts`,断言集合加上 `enabled: false` 时 `ctx.on` 不被订阅的产品路径不变量。
