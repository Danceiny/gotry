...
- **Round 8 typed tool/result contract 当前态**：bridge v3 用单一深冻结 descriptor 声明 closed/bounded `input_schema`、`output_keys` 和有限 `domain_outcomes`，同一 schema 同时进入 native request 与 spawn 前 validator；adapter 只接受 exact versioned ok/miss/error envelope。声明式 domain miss/error 必须 exit 0，恢复由模型依据固定 recovery enum 主动修参再调用，bridge 不自动 retry；非零退出继续归 infrastructure failure。source synthetic E2E 已覆盖两次模型调用与两次 adapter 执行，但 frozen treatment 尚未运行，provider/scorer/evaluator 未改，不产生 score/uplift 或 external benchmark closure。
