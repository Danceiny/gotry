...
- **#271 外层 dsh liveness(2026-09-10,当前状态覆盖上方 Phase A 记录)**:GoTry launcher 以独立 POSIX process group 管理 dsh child；非零/信号 close 与 parent-only SIGINT/SIGTERM 走 bounded TERM→KILL，close/error incident 在退出前同步写入。Node24 真实安装 `dsh@0.1.5-alpha.1` 的 child crash、rejected promise、subprocess spawn ENOENT、SIGINT/SIGTERM proof 均核对 stderr、incident、post-failure marker 和 descendant/group 清理。dsh SDK direct transport / 内部 supervisor / 真实部署 contract 仍开放，不改 vendor/lock，不改变 M3/M4-M6 真实 evidence gate。
