...
**M1 已退出**(commit `bb880f3`):真 LLM(MiniMax-M2,provider-neutral 适配器)通过 `replay-real.ts` 重放验收——同一开场白 3 轮完成 Kimi 的 13 轮,首轮问出工作窗口与已订资源、全程零日历错误、终轮交付已验证方案。S1 契约走查转入 M2 Entry 前置。**当前 = M2 实时数据**,Entry 差两个创始人输入:S1 走查、机票数据源决策(无商业合作期免费/开源优先,见 `tech-strategy.md`)。
