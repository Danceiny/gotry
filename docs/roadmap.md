...
- 外部验收仍是 TODO:Round 1 treatment-attested installed GoTry 已实际进入冻结 case，但 exact DeepSeek 为 environment unavailable/schema-invalid 0，GLM 为 300 秒 timeout。Round 2 bridge contract 与 synthetic E2E 已通过，frozen ChinaTravel treatment 仍待在该 seam 上运行；之后才可恢复原 5-query manifest并按 registry 评测。不得声称分数提升或 external benchmark closure。
- **Phase 1 bridge 当前态**:Round 2 已有 default-off owner-local bridge contract，固定 executable/cwd/argv_prefix/allowlist 与 timeout/output caps，并可按工具配置 `allowed_output_keys` 正向 visible-output 键合同，未声明键 fail-closed 且不反射。opt-in 仅允许 cold-start；每个 benchmark agent 强制 native、原始 bridge definition-only，最终 assembly 与 downstream pre-step 双闸拒绝额外 schema/同名 shadow/PTC 漂移。agent disposal 释放 scoped effect；插件先卸载时 live agent 保持 fail-closed quarantine 到 agent/process disposal，HMR 不热挂。config 要求 writes forbidden/network denied，但真实 enforcement 由 owner/host OS 负责，synthetic E2E 不证明 OS sandbox。bridge 不引入 Python/benchmark runtime 产品依赖；frozen ChinaTravel treatment 仍未运行。
