...
- **typed 参数契约迁移第一刀(2026-09-04,issue #112,D-30)**:最高流量工具 `gotry_flyai_search` 参数面 query blob → dsh typed ParameterSchemaSpec 平铺七字段——模型可见逐字段 JSON Schema + execute 前宿主权校验(ToolArgsError→guard 兜 ADR-13 ToolFailure,迁移锁 smoke §12/channel-registry-tests §8);普通模型对参数形状不再靠猜。余量工具迁移与 flyai canary 记 D-30。
