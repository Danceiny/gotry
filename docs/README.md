...
| [design/itinerary-deck-renderer.md](design/itinerary-deck-renderer.md) | Itinerary deck renderer + shared doc-contract layer (internal slice, issue #564): deterministic slide derivation, pure-CSS scroll-snap zero-script paging, anti-drift lock with the single-page renderer (identical rejection sets); no product entry in this slice |
| [design/recall-trigger.md](design/recall-trigger.md) | Recall trigger evaluator + tick scheduler + why-now card (internal slice, issue #577, Phase D): 5-class RecallReason closed set, PeriodicTickSource default-disabled (opt-in), source tag structurally mandatory, wish-pool read-only; zero product-code setInterval activation; no push, no mutation | 
| [design/share-adapters.md](design/share-adapters.md) | Share adapters + HMAC share token + consent state machine + shareDeck flow (internal slice, issue #573, Phase C): 4 no-op adapter stubs, token sign/verify, consent-required vs consent-revoked explicit split, 7-class ShareFailureReason closed set; no SDK, no dsh tool registration, no runtime activation |
...
- [Open issues execution plan](superpowers/plans/2026-09-25-open-issues-execution-plan.md): dated proposal for remaining issue acceptance, product-first sequencing, and real-evidence gates.
