...
| [design/recall-trigger.md](design/recall-trigger.md) | Recall trigger evaluator + tick scheduler + why-now card (internal slice, issue #577, Phase D): 5-class RecallReason closed set, PeriodicTickSource default-disabled (opt-in), source tag structurally mandatory, wish-pool read-only; no setInterval activation, no push, no mutation | 
