...
| **时区** | ⚠️ 手写数据包 v1(tz_offset_min/origin_tz_offset_min)并存候选 ✅ v2 IANA 路径(`ts/src/tz-resolver.ts` + `parseFlightPackToSpecV2`,Issue #343,2026-09-10;缓存 `Intl.DateTimeFormat`,无新依赖;DST gap/overlap/未知 zone 在 parse 边界拒收,不静默 fallback 到机器时区或 `+08:00`;覆盖中国出境/回程、DST 切换前后、跨日)。**v1 数值口径保留为版本化兼容迁移**(`{version:2}` 显式 opt-in),`data/flights_2026.json` 维持 v1 不动 | 动态(运行时常量)+ 静态 v1 | `[运行时权威:Intl.DateTimeFormat]`(v2) / `[静态包:估算]`(v1 数值) | v2 显式 IANA;v1 在所有现存 pack 自然消亡前保留兼容编译 |
...
- **M4 当前工程残余**:
  ...
  - **#343 IANA tz offset 替换手写(候选实现)**:见 §2 时区行 + `ts/src/tz-resolver.ts` + `ts/src/model.ts` 的 `doorToDoorFromMove`;v1 数值口径兼容保留(`{version:2}` 显式 opt-in,`data/flights_2026.json` 维持 v1 不动);root 3 条 counterexamples 已纳入 33 项 focused path,另有 18 项 sanity;全仓前台回归仍待执行。public path 仍只证明离线 fixture,不替代实时班期/票价/可售库存。
- **触发后置残余**:
