...
| **地面交通(接驳/铁路)** | ⚠️ 段内 transfer 硬编码在数据包(minutes/priceCny);**`dsh-map-tools` vendored 路由/时长能力存在**(OSRM via dsh-map-tools 走**公开 OSRM 回退**,非自托管);deterministic planner 仍消费静态 minutes/price,运行时未合入,并行工作由 [#341](https://github.com/Danceiny/gotry/issues/341) 跟踪 | 静态(运行时) | `[静态包:估算]` | OSRM via `dsh-map-tools` 入 deterministic planner;12306 无开放 API 不接 |
...
- **M4 当前工程残余**:
  - **OSRM via dsh-map-tools** remains a parallel engineering track tracked by [#341](https://github.com/Danceiny/gotry/issues/341); deterministic planner still consumes static transfer minutes/prices.
- **触发后置残余**:
