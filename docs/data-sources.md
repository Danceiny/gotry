...
- When multiple trusted extension Origins poll one bridge, the cookie-name check and its immediately following search stay on the Origin that claimed the check; another Origin cannot submit that job's result. This is Origin affinity, not Chrome-profile identity: separate searches and profiles sharing one Origin still require live verification under [#272](https://github.com/Danceiny/gotry/issues/272).
