...
| **Ground transport (transfers/rail)** | ⚠️ **#341 first slice**: accepts only explicit origin/destination lat/lng and `mode=driving`; `ts/capabilities/ground-transfer.ts` delegates route results through the registered public `map_driving_route`, querying and binding the arrival direction (A→B) and the return direction (B→A) separately — a miss/error/stale/mismatch on one side only falls back that side to the static estimate; the candidate transfer holds `minutesOut`/`minutesRet` overrides consumed per direction by `evaluateChoice`; minutes bind only to the named static `taxi` transfer; `bus`/`bus_plus_taxi` do not call the route provider, and static prices are not rewritten | Route estimate + per-direction bounded cache (direction = origin/destination pair); **not realtime traffic** | Route facts = `map_driving_route` + `asOf` (host observation/query time, not provider publish time)/freshness/cache; fallback = `[静态包:估算]`; self-contradictory route facts (positive distance with zero duration) are rejected as provider miss (2026-09-11 boundary freeze) | live traffic, transit/rail, fares, address resolution, and broader transfers are still not integrated — gated real-data work tracked at [#429](https://github.com/Danceiny/gotry/issues/429), offline boundary pinned 2026-09-11; 12306 is not assumed by this slice |
...
| **Exchange rates** | ❌ None (all CNY hardcoded); **multi-currency FX remains trigger-deferred** — the China-outbound launch currently relies on the CNY settlement path; implementation starts when the **first real non-CNY supplier quote, user budget, or destination demand** actually appears (per [#344](https://github.com/Danceiny/gotry/issues/344)); until then no floating-rate source is consulted | — | — | Multi-currency FX (free tier such as Exchangerate.host, or hotel-be) |
...
- **M4 current engineering remainder**:
  - **#341 ground-transfer first slice**: explicit coordinates + `mode=driving` enter the deterministic planner adapter via the registered public `map_driving_route`; the arrival direction (A→B) and the return direction (B→A) are queried, cached, and degraded separately — a miss/error/stale/mismatch on one side only falls back that side to the static estimate, and the candidate transfer holds `minutesOut`/`minutesRet` overrides consumed per direction by `evaluateChoice`; only the static `taxi` transfer receives route minutes, `bus`/`bus_plus_taxi` keep static values with an explicit fallback, and static prices are annotated `[静态包:估算]`. Stale cache entries are re-queried per direction; invalid coordinates, unsupported modes, and binding mismatches keep static values per direction; one-off live OSRM, realtime traffic, transit/rail/fare, and address resolution are not covered.
  - Rail/transit, fares, live traffic, and non-explicit coordinate resolution beyond the first slice remain follow-up boundaries tracked at [#429](https://github.com/Danceiny/gotry/issues/429).
- **Trigger-deferred remainder**:
  ...
  - **#344 multi-currency FX**; trigger = the first real non-CNY supplier quote, user budget, or destination demand (the China-outbound launch currently relies on the CNY settlement path; until the trigger fires, no floating-rate source is consulted).
...
- When multiple trusted extensions poll one bridge, the cookie-name check and its immediately following search stay on the Origin and service-worker client ID that claimed the check; another client cannot claim the search or submit that job's result. The ID is random and ephemeral: a service-worker restart fails the pinned search within its timeout instead of switching Chrome profiles. Older extensions without a client ID retain Origin-only routing. Separate searches and the default live path still require verification under [#272](https://github.com/Danceiny/gotry/issues/272).
