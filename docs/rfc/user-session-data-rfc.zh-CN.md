...
### 2.2 技术路线：会话复用三通道，扩展桥为 PRIMARY

| 通道 | 机制 | 2026 现状 | 定案 |
|---|---|---|---|
| A. attach 用户日常 Chrome | Chrome 136（2025-05）起默认 profile 禁用调试端口；Chrome 144+ 恢复官方路径（`chrome://inspect/#remote-debugging` 手动开一次） | 要求用户 Chrome ≥144 + 手动开开关；同一时刻仅一个调试客户端 | **⚠ 降级为显式后备（2026-08-30 founder 定案）**：chrome-devtools-mcp #825 实锤 Chrome 144+ **每次连接都弹浏览器权限框且无持久化批准**，逐连接弹窗不可用（founder：「授权操作太频繁了，根本无法使用」）。保留 `GOTRY_SESSION_TRANSPORT=cdp` 显式 opt-in 供诊断/测试，不再自动回退 |
| B. 专用持久 profile | Playwright `launchPersistentContext`（可用用户已装 Chrome，零浏览器下载）；登录一次跨会话复用 | playwright-mcp 默认模式，最成熟 | **仅测试/后备**（匿名裸窗口实测无人会登录，founder「不能匿名实例」） |
| C. **扩展接管** | **MV3 扩展 + 本地桥**：一次性安装；扩展在**自己的标签页**被动嗅探站点自身发出的检索响应（扩展零写行为），经 `127.0.0.1` 长轮询回传 Node；不用 `chrome.debugger`（无警告条） | gotry 自研 `extension/`（GoTry Session Bridge，4 文件零构建，manifest 固定 key=扩展 ID 跨机器稳定）+ Node 侧 `session/extension-bridge.ts`（`node:http` 长轮询桥，零新依赖；origin 白名单） | **✅ PRIMARY（2026-08-30 founder 定案，issue #21）**：系统弹窗从「每连接 1 次」降为 **0 次**；授权模型=一次性安装（约 30 秒）+ manifest key 派生固定 ID origin 白名单 + 会话内授权闸不变；`needs-extension` verdict 为 waiting no-spend 用户门 |
...
- **飞猪 FlyAI 开放平台**（flyai.open.fliggy.com；CLI `@fly-ai/flyai-cli`，skill 仓 `alibaba-flyai/flyai-skill`）：机/酒/景/度假全场景，「实时直连官方商品库」。
...
Comet 事故（Brave 2025-08 披露，Reddit 评论藏注入→跨站接管账户）证明 SOP/CORS 对 agentic 浏览器失效；Anthropic 红队：注入成功率 23.6%→11.2%（有防护），OpenAI 官方承认「 unlikely to ever be fully solved」。**对策不是更好的模型，是更小的爆炸半径**：只读白名单动作集 + 交易端点在传输层物理拦截 + 页面内容进模型前打不可信标注 + 结构化任务意图（站点+查询词+字段）偏离即中止。gotry 的 WriteGate 红线在这里提前兑现为 **ReadGuard**（见 §3.4）。
...
### 3.3 Onboarding UX 与分发通道（2026-09-02 商店上架后定稿）

**职责返交**：LLM 由 dsh 管理、扩展安装由浏览器商店管、CLI 自举由 `gotry setup` 管。此前「3 次点击 + 0 次终端命令」的 5 步 wizard 因越界管浏览器（`open` 弹窗、`pbcopy` 动剪贴板、`osascript/zenity` 抢 GUI）与 dsh 渲染层（stdout 文字墙代替 verdict），于 2026-09-02 商店上架后撤销。

**现行形态**：

- **安装 = 浏览器的事**：用户去 [Chrome 应用商店](https://chromewebstore.google.com/detail/gotry-session-bridge/oeajpiccmonococjcegddlooeeohlbgd) 点「添加至 Chrome」（2026-09-02 已上架，0 终端命令、0 gotry CLI 介入）；自动更新归商店。
- **渲染 = dsh UI 的事**：`sessionFlightSearch` / `sessionLogin` 在 `needs-extension` 时返回 verdict 字段 `{ verdict:'needs-extension', installUrl, installAction:'add-to-chrome' }`，dsh 原生 presentResult 层把 `installUrl` 渲成可点链接；gotry 不再写 CLI 文字墙。
...
路由权威：`ts/src/backend/modules/session-search.ts`。可选字段（如 `installUrl`、`installAction`）权威：`ts/capabilities/session-search.ts`。最小契约：

- **路由**：`POST /v1/session/search`；JSON 体 `{ supplier, query? }`，其中 `query` 为 `{ entryUrl?, timeoutMs? }`——`query` 及其字段均可省略。M0 仅接受 `supplier='dida-portal'`，其余 `400`。
- **鉴权**：`Authorization: Bearer <GOTRY_BACKEND_SESSION_API_KEY>`。缺/错 bearer → `403 { ok:false, error:'forbidden' }`。缺 `GOTRY_BACKEND_SESSION_API_KEY` → **fail-closed**：路由返 `503 { ok:false, error:'GOTRY_BACKEND_SESSION_API_KEY 未配置(fail-closed)' }`——在 supplier 检索被调用前即拒。
- **`200` 体**：始终含 `{ ok, verdict, supplier, rates, evidence, latencyMs, fetchedAt }`；`rates` 不可得时为 `[]`。异常结果带 `error`。
  - `installUrl` + `installAction` 由能力层结果经 helper 透出，`200` 与 `429` 两条路径都过该 helper。当前 `needs-extension` 结果两字段都带（能力层设置）；普通结果可能省略。契约不保证其他 verdict 永不携带可选字段。消费方 UI 用 `installUrl`（Chrome Web Store 可点链接）+ `installAction='add-to-chrome'` 渲染安装入口——后端不主动发起安装。
...
- **通道 B（Chrome Web Store，已上架 2026-09-02）**：商店材料与隐私政策见 `../ops/extension-webstore-submission.md`、`../ops/extension-privacy.md`。

**离线合同（run-all §40/§43）**：

- （1） `gotry setup wizard --dry-run` 零网络零浏览器零剪贴板零 GUI spawn，只校验命令输出与退出码
- （2） health-watch 三时序分支：0ms ready / 中途 ready / 120s+1ms 超时
- （3） wizard 不调任何 `spawn`（`open/pbcopy/xclip/osascript/zenity/msg/clip` 全退役）
- （4） 单条 query 的 retry-after-watch:watch 内 ready → 自动重放同一 query_id，产物命中同一 evidence 文件
- （5） `needs-extension` verdict 含 `installUrl` + `installAction:'add-to-chrome'`
- （6） 通道 A：资产名/URL 合同 + 打包脚本防漂移 / dist-manifest fail-closed / 版本比较 / 回环 e2e 六态 / CLI 单行 JSON 契约

**与既有约束的勾稽**：传输层零新依赖纪律（wizard 无 spawn import；health-watch 是 Node http 轮询）；ReadGuard 不变（onboarding 不碰 cookie 值，只验证扩展心跳与登录态 cookie 名存在性）；登录产品化不变（`gotry_session_login` 由扩展在用户 Chrome 弹登录入口，与商店安装是同一条扩展的两种用途）；「随时可关」三重：扩展卡片开关 + `GOTRY_SETUP_EXTENSION=0` + `sessionAccess: off`。
...
- **cdp 车道（显式后备）**：保留上述原四层——puppeteer 请求拦截 classifyRequest 双因子 abort + 审计 JSONL + fail-closed（guard 装不上整会话打不开）。
...
- **登录 bootstrap**：`scripts/session-login.ts`——attach 用户日常 Chrome → 打开登录入口标签 → 人登录 → 只读轮询票据 cookie 名（不读值不碰凭证），`needs-login` 文案指向它；**测试永不自动开浏览器窗口**（live 节 `GOTRY_SESSION_LIVE=1` opt-in——founder 反馈「匿名窗口反复打开携程/界面闪退」= 测试骚扰，当日根治）。
- **登录态持久化（2026-08-28 founder 纠偏：「打开浏览器必须有登录态，不能是匿名实例」）**：默认 profile 挪 `~/.gotry/session-profile`（持久，登录态不丢）；`sessionFlightSearch` 登录闸：匿名默认拒（verdict=`needs-login`）；`allowAnonymous` 仅限链路自检且证据链标 `anonymous=自检态`。远期若需 profile 迁移：AES-GCM 落盘 + 密钥进 OS keychain（macOS `security`/Windows DPAPI/Linux libsecret；keytar 已死不用），0600 权限。登录永由用户人工完成（agent 不碰密码/OTP/验证码，对齐 Operator 的 takeover 模式与总纲 3.5「敏感信息模型永不接触」）。
...
| 阶段 | 状态 | 预算 |
|---|---|---|
| **P0 尽调** | ✅ 2026-08-28 完成 | 实用 1 tick |
| **P1 骨架** | ✅ 2026-08-28 完成 | 实用 1 tick |
| **P2 面上** | ◐ 2026-08-28 主体完成（余项待登录态） | 实用 3 tick |
| **P3 产品化** | ◐ 2026-08-28 切片 1/2 完成 | 实用 2 tick |
| **P3.5 传输层定案：扩展桥** | ✅ 2026-08-30 完成（方案 C 升 PRIMARY） | 1 tick |
| **P4 分发通道（ADR-21）** | ✅ 通道 A 落地 2026-08-30；通道 B 商店上架 2026-09-02 | 1 tick |
| **P3.6 Onboarding UX 职责返交** | ✅ 2026-09-02 完成（wizard 退化为离线健康探活等待） | 0 tick（职责返交） |
| **P3.7 双源 e2e 真跑批（goal 2）** | ✅ 2026-08-30 完成（commit `60669f8` + PR #66 follow-up） | 1 tick |
| **P3.8 Issue #67 static vendor + 默认桥生命周期** | ✅ 2026-08-30 | 1 tick |
...
- **Exit（已达成）**：①FlyAI 无 key 实测可用，8 工具全只读，机/火车票官方通道开（备忘进 data-sources.md §8）；②高德获取步骤落 tokens.md（等 key）；③PoC 两轮零风控零交互，主接口已识别（`search/batchSearch` ~550KB + 低价日历 ~81KB，脚本 `ts/scripts/session-attach-poc.ts`，playwright-core 1.62.1 devDep）；④决策表已结算。**结论修正：机票/铁路主链路改走 FlyAI，会话面收缩为「携程 C 端交叉验证 + 美团本地 + 官方通道盲区」**。

#### P1 骨架

- **内容**：transport + ReadGuard + adapter 接口 + 首个适配器（**携程机票**，与现有航班静态包直接对账）；证据链 `[会话:*]`；隔离 stateRoot 测试（对齐巡检状态纪律，绝不动 dsh-runtime 真实状态）。
- **Exit（已达成）**：`capabilities/flyai.ts` + `capabilities/session-search.ts` + `session/{transport,read-guard,adapters/ctrip-flight}` 五件落地；`session-tests.ts` 25 断言全绿（ReadGuard 双因子/驼峰复合写词/fixture 解析/城市码表三值/节律闸 cooldown/live FlyAI hit/live 会话嗅探 hit+guard 零拦截+审计文件不出现）；run-all §24 全栈 ALL GREEN。双源对照首记录（FlyAI ¥230 vs 会话 ¥1611）——**价差已解释（2026-08-28）：FlyAI 最低价为南京中转+跨天衔接链，携程首屏 ¥1611 为直达档；双源对照断言口径=按 journeyType 分桶、逐段日期对齐后再比**，价差本身即交叉验证的价值论据。
...
- **Exit**：fixture 合同已纳入确定性回归；真实跑批仍需 Chrome 权限确认和 CDP 握手。

#### P3 产品化

- **内容**：`gotry_flyai_search`+`gotry_session_search` 双工具 ✅（smoke §12，hit/限流双合法终态）；人格契约 **（19） 三级路由** ✅（仓根 yml，直达/中转分桶）；e2e §14 实证记录 ✅；六状态面同步 ✅；run-all §25-27 全绿。
- **Exit**：真模型 e2e **flyai 侧 ✅**（e2e §15,8ddb997：真模型实际调用 flyai 工具，三源证据链并存，多 lane 协同实证）；**待登录态**：真模型 e2e session 侧一例 + 双源 sf-01..08 跑批（风控触发次数=0 实测期口径同此）。

#### P3.5 传输层定案：扩展桥

- **动因（founder 实测）**：「Chrome attach 逐连接权限框太频繁，根本无法使用」——chrome-devtools-mcp #825 实锤 Chrome 144+ 每连接弹框且无持久化批准，CDP 路线对产品不可用。
- **落地**：`extension/` GoTry Session Bridge（MV3 四文件零构建，manifest 固定 key=扩展 ID 稳定；SW 长轮询；MAIN-world 嗅探；cookie-names 只取名字）+ `session/extension-bridge.ts`（node:http 回环桥，零新依赖，origin 白名单，长轮询 <20s 维持 SW 存活）+ `extension-channel.ts` 三 job 封装 + session-search/login 车道路由（扩展默认，cdp 显式 `GOTRY_SESSION_TRANSPORT=cdp`，不静默回退）+ `gotry setup` 扩展落位（`~/.gotry/extension`，幂等，`GOTRY_SETUP_EXTENSION=0` 可跳）。
- **Exit（已达成）**：系统弹窗 0 次/会话；run-all §38 全离线合同 23 断言（manifest/固定 ID 派生/Node↔扩展常量防漂移/origin 403/长轮询幂等/心跳/闭环/超时/needs-extension no-spend/waiting_extension）；bootstrap-tests 5/5；真实 sf-01..08 双源跑批待用户一次性装扩展后收尾（门禁从「开调试端口+每连接点框」降为「装一次扩展」）。

#### P4 分发通道（ADR-21）

- **动因**：founder 指令「产物下载和安装也得做成更好的用户体验，可以用 github 作为分发渠道」；Chrome 平台约束下双通道分工——GitHub Releases 只能改善下载，一键装+自动更新只有商店。
- **Exit（已达成）**：通道 A：`--extension-from=github` 下载链（稳定资产名/SHA256/key 钉扎/原子交换/失败降级 bundled）+ `scripts/package-extension.mjs` 打包（只产产物，上传确认制）；通道 B：商店材料两篇（见 §3.3）+ 2026-09-02 过审上架（D-25 清偿）；run-all §43 + bootstrap-tests 8/8。

#### P3.6 Onboarding UX 职责返交

- 内容即 §3.3 的「职责返交」重设（2026-09-02），不重复展开。
- **Exit（已达成）**：run-all §40 onboarding-tests 9/9（dry-run 零网络零 GUI / health-watch 三时序 / wizard 不调 spawn / retry-after-watch 同 query_id 重放 / withAutoRetry 端到端 / 探针边界常量 / extensionDir 契约 等）；bootstrap-tests 8/8（wizard dry-run + 真实路径 + 扩展分发两条）。

#### P3.7 双源 e2e 真跑批（goal 2）

**#272 维护基线（2026-09-26）**：`sf-01..08` 的查询 ID、顺序、路线和日期以 `ts/data/session-golden-20.json` 为准，SHA-256 `3be197830e5a6aa6e1309684a1fce4a736ad2a845c67b6f2f1b8a4ad3ccf977a` 固定在 `ts/data/sf-golden-manifest.json`。真实跑批器在创建证据目录或联系供应商之前拒绝查询及对照清单漂移。八条的携程可用性与传输行为都须在明确获准、已登录的浏览器会话中核验；手工对照和 `--golden=static` 仅是离线确定性参照。若修订查询集，须审核新清单修订并另开证据批次，不得继承旧批次的真实结果。当前真实证据缺口和下次检查点见 [#272](https://github.com/Danceiny/gotry/issues/272)。
...
下一次获准的 `sf-live-benchmark` 批次会在每条原始查询证据中记录 `sessionTransportShape`，只用固定枚举标明携程 `batchSearch` 信封结构或预检、挑战、嗅探超时状态，不记录字段值。隔离的扩展审计只保存站点 Origin 和该形状，不保存路线／日期 URL、页面标题、票据名字或响应正文。机票预检与检索作业失败时使用主机固定文案，不把可能含私有回包细节的扩展错误原文写进证据。合成会话桩标为 `not_observed`。这些标签帮助审阅传输漂移；未取得真实已登录批次前，仍不能据此确认实时库存。
...
- **Exit（已达成）**：登录态连续两轮 8-query：official 均 8/8 hit、fallback 0；session 分别 3/8 与 5/8 hit，全部可评分 hit（3+5 条）均 13/13=100%，非 hit 明示 miss。默认桥 parked timer/socket `unref`，wizardless `keepBridge` 不变；§38 24/24、§40 9/9。
...
**决策门（新增，回写总纲 §5.2）**：G7 用户会话数据面立项 / G8 12306 纳入与否（**推荐暂缓**：判例全在写侧但平台态度最烈，等链路稳定单独评审）/ G9 官方通道尽调批准。三门均已于 2026-08-28 结算（见 §0 决策表）。
...
- **复用矩阵修订提案**（总纲 §2 新行）：`@fly-ai/flyai-cli | MIT | import(npx spawn,零渠道知识管道层) | 飞猪官方只读检索通道(机/火/酒/POI),交易经 jumpUrl 由人完成`；`playwright-core | Apache-2.0 | import(devDep) | cdp 后备车道传输(Chrome 144+ 逐连接弹窗实测不可产品化,2026-08-30 降级为诊断/显式 opt-in)`；
  ...
  - `Chrome Web Store 上架清单(2026-09-02 上架,§3.3)| 平台原生 | reference | 商店上架材料(单一用途/权限理由/隐私披露/文案/后续发版流程)+ wizard 退化为离线健康探活等待,不动 pbcopy/osascript/zenity,安装 = 浏览器的事、渲染 = dsh UI 的事`；`GitHub Releases + 平台 tar | 平台能力/原生 | reference | 扩展分发通道 A(2026-08-30,ADR-21,§3.3):稳定资产名 + SHA256 dist-manifest + tar -xzf 解压,零新 npm 依赖`。
