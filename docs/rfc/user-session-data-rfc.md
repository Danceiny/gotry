...
**#272 maintenance baseline (2026-09-26)**: the `sf-01..08` query IDs, order, routes, and dates are frozen from `ts/data/session-golden-20.json` by SHA-256 `3be197830e5a6aa6e1309684a1fce4a736ad2a845c67b6f2f1b8a4ad3ccf977a`, pinned in `ts/data/sf-golden-manifest.json`. The live runner rejects query or comparator drift before creating evidence directories or contacting a supplier. All eight cases require an explicitly authorized, logged-in browser session to establish Ctrip availability and transport behavior; manual and `--golden=static` comparisons are offline, deterministic references only. A revised query set needs a reviewed manifest revision and a new evidence batch; it cannot inherit the prior live result. Current live gaps and the next checkpoint are tracked in [#272](https://github.com/Danceiny/gotry/issues/272).

The offline `sf-summary.ts` `status=ok` certifies selected-batch integrity only (`status_scope=batch_integrity_only`), not live availability. Raw and rebuilt records label static/manual comparators `comparator_evidence_scope=deterministic_only`; `live_provider_comparable` excludes them even when the raw double-source state is `comparable`. Logged-in session availability still needs separate, reviewable live evidence.

For the next authorized `sf-live-benchmark` batch, each raw query record carries `sessionTransportShape`: a fixed, value-free classification of the Ctrip `batchSearch` envelope or a precheck, challenge, or sniff-timeout state. An isolated extension audit stores only the site origin and that shape; it omits the route/date URL, page title, cookie names, and response body. Flight precheck and search-job errors use fixed host-authored summaries instead of extension-supplied error text, since the latter may contain private response details. Synthetic session stubs report `not_observed`. These labels help review transport drift but do not certify live inventory without a real logged-in run.
