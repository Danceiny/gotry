...
| [design/recall-trigger.md](design/recall-trigger.zh-CN.md) | 召回触发评估器 + tick 调度器 + why-now 卡（内部切片，issue #577，Phase D）：5 类 RecallReason 闭集、PeriodicTickSource 默认关闭（opt-in）、source tag 结构性必现、wish-pool 只读；不激活 setInterval、不推送、不 mutation | 
...
| [evaluation/flyai-drain-bounded-report.zh-CN.md](evaluation/flyai-drain-bounded-report.zh-CN.md) | FlyAI 主进程退出后的 stderr drain：共享 spawn-bounded 所有权、drain-timeout fail-closed 分类与回归锁定（#516） |
