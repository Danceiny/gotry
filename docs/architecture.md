...
| `ts/scripts/product-metrics.ts` `ts/data/product-metrics-fixture.json` | **M3 cohort 证据评分面(Issue #22)**:阈值冻结 manifest + 脱敏 cohort/nightly schema + 定稿率/NPS/POI 幻觉率 scorer；fixture 与真实证据分流，未知字段 fail-closed | ✅ run-all §33；真实 cohort 待收集 |
...
- **工具可用性硬化与外部依赖自举(2026-08-29,Issue #24)**:npm 形态用户轨迹暴露三工具「不可用」观感,双 lane 并行修复、PR #33 合流——weather 双源地理编码(Open-Meteo 人口/行政级排序 + Nominatim 兜底,「普吉岛」直查可用);flyai 非业务形状进 error 终态 + 过去日期预校验 + miss/error 分陈述;hbcli 静态包按目的地过滤。本 lane 独有增量:hbcli ENOENT 人话化 + 候选路径回退(~/.local/bin、~/.staicli/current)+ 离线 flyai 套件(run-all §7b)+ bootstrap 套件(§7c)+ **安装期外部依赖自举**(founder 指令「按上游本身的方式装好」):hbcli 官方 install.sh / agent-reach 官方 pip 入包内 .venv / flyai 免装,postinstall --auto 非致命 + `gotry setup` 手动入口;hotelbyte-cli 定性更正为公开仓(原 D-19 记账改口径入 D-22)。
...
| D-13 会话适配器维护面(RFC user-session-data-rfc) | **部分清偿 2026-08-29**:action-cache + 金标准输入 + #21 字段 fixture scorer/双源 shape gate 已落；站点接口改版仍可能断(当前携程 batchSearch),真实 sf-01..08 与 ≥90% 证据待 Chrome attach 握手,断时降级 `[实时API:flyai]` 主链路 |
...
| D-18 M3 Exit 真实 cohort 证据缺口 | **进行中(Issue #22)**:公开面已有冻结 manifest、严格脱敏 schema、确定性 scorer 与 synthetic fixture 守门；私有真实样本尚未进入 `ts/gotry-state/evidence/m3/`。只有 50–200 人真实 cohort 同时达到定稿率 ≥40%、NPS ≥40、POI 幻觉率 <1% 且窗口内 nightly real-LLM 可复跑，才允许业务达标；无样本时不关闭 M3 Exit。 | `product-metrics.ts`;run-all §33 |
...
| D-22 hbcli 实时链的用户侧就绪面 | **2026-08-29 记账(Issue #24,PR #33;本 lane 原以 D-19 记账,合流让位于 main lane 的 M4 repeat cohort 条目后改号)**:初判「内部仓无公开分发渠道」有误——hotelbyte-cli 是公开仓(github.com/hotelbyte-com/hotelbyte-cli,MIT,官方 install.sh 装原生二进制)。已落地:安装期自举(bin/gotry-bootstrap.js,hbcli 官方脚本 + 候选路径回退)+ 未装/无凭证时静态包按目的地过滤降级。**残余**:实时数据需用户侧凭证(`hbcli auth set-credentials` 或 HOTELBYTE_TOKEN)且 UAT 证书状态随上游波动——凭证属用户资产不自动配,未配时静态包为默认形态。赎回时机:上游提供免凭证只读面或 UAT 稳定。 |
