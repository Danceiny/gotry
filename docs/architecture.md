...
- **工具可用性硬化(2026-08-29,Issue #24 修复)**:npm 形态用户轨迹暴露三工具「不可用」观感——①weather:「普吉岛」GeoNames 无条目致地理编码失败,新增 `resolvePlace` 别名阶梯(产品词表别名优先→原样→去后缀+population≥1000 防同名村假阳性);②flyai:Sentinel 限流输出(合法 JSON 无 data.itemList)被吞成 0/0 静默 miss,改判 error+限流标注,summary 三分支并显式提示过去日期;③hbcli:npm 形态未装 hotelbyte-cli 是常态,ENOENT 降级原因人话化,静态包 UI 计数修复(对象形状此前恒显「无结果」)。新增离线 flyai 套件(run-all §7b)+weather §6/hbcli §6 断言。
...
| D-19 hbcli 实时源无公开分发渠道 | **2026-08-29 记账(Issue #24)**:hotelbyte-cli 属 founder workspace 内部仓,非 npm 依赖、无公开安装渠道——npm 形态用户必然 ENOENT,**静态包 `data/hotels_2026.json` 是 npm 形态的设计默认而非故障**(降级文案已人话化、UI 计数已修复)。赎回时机:hotelbyte-cli 公开分发(或实时酒店源有公开替代)时恢复实时默认。(编号跳过 D-17/D-18:并行 lane 在制品已占用) |
