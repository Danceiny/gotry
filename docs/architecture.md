...
- **typed 参数契约迁移第一刀(2026-09-04,issue #112,D-30)**:最高流量工具 `gotry_flyai_search` 参数面从 `query:{type:'json'}` blob 迁 dsh typed ParameterSchemaSpec——平铺七字段(`kind` enum flight|train|hotel required + from/to/date/checkIn/checkOut/keyWords),`parameterSchemaSpecToJsonSchema` 投影模型可见逐字段 JSON Schema,`validateArgs` 在 execute 前宿主权校验;畸形参数(缺 kind/枚举外值/legacy blob 包裹)入口即结构化拒绝,ToolArgsError 经 guardToolExecute 保持 ADR-13 ToolFailure 形状(smoke §12 + channel-registry-tests §8 双迁移锁);条件必填(机/火要 from/to/date,酒店要 to)不在 schema 强制,仍由 execute 结构化报错给修正方向。余量(session/hotel/weather 等其余工具)与普通模型 canary 仍在 D-30。
...
| D-30 检索工具 typed 参数契约迁移部分完成(ADR-25 配套;2026-09-04 flyai 已迁,余 session/hotel/weather 等) | 普通模型性能的最大单项杠杆(tool-orchestration-design.md §4③):高流量工具参数面曾是无类型 `query: json` blob,模型看不到逐字段 schema——dsh `defineTool` 原生支持 typed ParameterSchemaSpec(object/properties/enum/required + execute 前 validateArgs),blob 是本仓的选择不是上游限制。**已迁**:`gotry_flyai_search` 平铺七字段(模型可见 JSON Schema+宿主权入口拒绝,ToolFailure 形状迁移锁 smoke §12/channel-registry-tests §8)。**余量**:session/hotel/weather 及其余工具迁移;每个首批工具一轮普通模型 canary(flyai canary 待跑,跑前不合主干发布面);`interpretArgs` 留作旧形态容忍层 | `ts/src/index.ts` 参数面;dsh-tools schema;issue #102 同模式 |
