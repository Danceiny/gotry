...
- **#341 地面接驳第一切片**:`gotry_feasibility_check` 只接受显式起点/终点经纬度与 `mode=driving`,通过已注册公开 `map_driving_route` 结果生成带 `asOf`、freshness/cache、provenance 与 evidence class 的路线事实;仅改具名 destination 的静态 `taxi` transfer 分钟,`bus`/`bus_plus_taxi` 保持静态值并返回明确回退,静态价格始终标 `[静态包:估算]`。`asOf` 是宿主观察/查询时间而非 provider 发布时间;缓存命中与过期重查可见,provider 失败、非法坐标、不支持模式和绑定不一致均明确保留静态值;不覆盖 live traffic、transit/rail/fare,也不改变 #20/M5/M6 既有准入边界。
...
| License | MIT |
| 人格 | 行为契约 23 条;锚点卡与记忆 brief 注入 persona。**运行时组合唯一来源 = 仓根 `cordis.gotry-patch.yml`** |
...
- **#299/#355 机票/车次 claim 分流与 12306 typed rail facts(2026-09-10,D-26/#273 残余)**:`extractClaims` 将 G/D/C/Z + 3–4 位车次收进独立 `trains`,完整 token 判定不误删合法双字母航司(含 `CZ8582`/`9C8781`);`flightClaimVerdict` 只看 `kind:'flight'`,`railClaimVerdict` 只看 `kind:'train'` exact-date/route/code/bookable 事实,缺证据输出 `rail_claim_unverified`。FlyAI 与 12306 session 均沿真实注册工具→typed fact log→canonical anchor/`gotry_fact_gate`;12306 parser 区分 recognized nonempty/recognized empty/malformed/transport-runtime error,正事实还要求 invocation route/date binding、`captured_at_non_future_and_max_age_15m`、`canWebBuy=Y` 与闭集可用座位 token。列表无票价,不新增 RailFact 渲染器或 static-schedule 降级。证据仍限确定性 adapter/fixture,不宣称实时 12306 或 M4/M5/M6 gate。
- **#355 12306 session typed rail facts(2026-09-10,父 #299/#273)**:session parser 区分 `recognized-nonempty`、`recognized-empty`、`malformed`、`transport-runtime-error`;只有 invocation 产生的 route/date/batch binding 及 `captured_at_non_future_and_max_age_15m` 新鲜时间可进入事实侧车。正事实还要求 `canWebBuy=Y` 与闭集可用座位 token,列表路径不生成价格;genuine empty 只生成一条 exact-date 负事实,其他失败态与无座位行不落事实。已有 rail-only claim、canonical renderer、typed anchor 与 `gotry_fact_gate` 复用,证据仍限隔离 deterministic fixture,不代表实时 12306 可售性或 M4/M5/M6 准入。
...
L2 编排:对话循环 ts/src/loop.ts —— LlmPort(mock✅/真✅ provider-neutral) + 确定性访谈 + 求解挂载
    └ dsh 插件 gotry-tools:✅ 已在真实 dsh headless 运行时端到端(68ea364 rc.1 起实证;2026-08-29 曾用 vendored `0.1.2-alpha.1`;Round 5 起 source/package 共用 root-pinned `0.1.2-alpha.3`;2026-09-09 #268 迁移到 230 包精确 `0.1.5-alpha.1`)——模型经 pi-ai(MiniMax-M2)主动调用 gotry_feasibility_check 并引用引擎数字;组合见 cordis.gotry-patch.yml(bin/gotry-inner.js 运行时生成;ts/ 下旧副本已退役 2026-08-28)
...
- **#299 机票/车次 claim 分流(2026-09-10,父 #273)**:完整 G/D/C/Z + 3–4 位车次独立入 `trains`,合法双字母航司(含 `CZ8582`/`9C8781`)保留在 flights;flight/rail verdict 按 fact kind 隔离,FlyAI train 结构化 exact-date hit/miss 沿现有工具→typed fact log→canonical anchor→`gotry_fact_gate` 路径,12306 session registry hookup 延后至 typed parser outcome + seat availability/freshness contract。缺 train 证据仍以 `rail_claim_unverified` fail-closed,不使用 static schedule、train price 或不存在的 RailFact 渲染器;确定性 fixture 不构成真实供应商证据,#299/#273 保持 open。
- **#355 12306 session 接入(2026-09-10,父 #299/#273)**:上述 deferred hookup 已落地到 `gotry_session_search kind=train` 的真实注册工具路径。typed parser 对 malformed/nonempty mixed rows fail-closed;仅 route/date/batch binding 一致、时间不未来且不超过 15 分钟、`Y`+已识别座位 token 的行生成 train fact。empty 只生成负事实,malformed/transport/challenge/无座位零事实;仍只用确定性 fixture,不宣称 live 12306、价格或 M4/M5/M6 gate。
...
- **Issue #341 地面接驳接缝**:公开 map-tool 只作为 provider 边界,不深导入 `OsrmClient`;路线估算在 capability 层解析为确定性分钟事实后才进入既有 solver 输入,故 solver/evaluate 不执行网络。第一切片只把路线分钟接到具名 destination 的静态 `taxi` transfer,价格沿用静态包;live traffic、transit/rail 与 fare 不在边界内。
...
| D-26 事实闸覆盖面缺口(ADR-19) | **部分收口**:酒店 claim 已入闸(2026-09-04,issue #118,HotelFact 第三形态+渲染原语锚点回溯);政策事实生产端 v1 已落(2026-09-05,issue #141,C 档领事服务网 cs.mfa.gov.cn → PolicyFact 落账,Timatic/Sherpa° 后议);政策渲染锚点闭合 + 锚点行 `as_of` 日期边界 + 海关申报关键词(2026-09-10,issue #273,`renderPolicyFact` typed-anchor + 仅核对渲染行「截至日期」与事实 `as_of` + `POLICY_WORD` 补海关申报);机/火共享原语增加 fare role 绑定内的可比较 CNY 硬价边界(2026-09-10,issue #300,canonical/明确票价标签字段才比较,带非票价文字标签的分隔金额不参与,裸/未归属或畸形 fare money = `unverified_price_claim`,数值不一致 = `price_contradicted`,不含酒店 `priceRaw`);**非关键词住宿 category(度假村/青旅/青年旅舍/别墅/公寓)+ lodging heading 下无 token 可住断言 + 政策词 13 词有限并集(EVUS/ETA/eVisa/疫苗/疫苗接种/健康申报/隔离/工作签/居留/返程签/护照有效期/黄皮书/保险)直读入闸(2026-09-10,issues #301/#302,fact-gate-tests §12/§13,run-all §39)**。**#299 车次分流切片**另将 G/D/C/Z 完整 token 与航班 claim 分离,flight/rail verdict 按 kind 隔离,FlyAI train 结构化 exact-date hit/miss 沿真实工具→typed fact log→canonical anchor→fact gate 路径;12306 session 仍仅为公开查询列表,待 typed parser outcome + seat availability/freshness contract 后接 registry。无 RailFact 渲染器、static-schedule 或真实供应商可售性替代。现有未知锚点仍 fail-closed;价格容忍度、汇率/多币种策略仍待 owner/founder 拍板,本切片不宣称关闭 D-26 全量残余。残余:反向抽取仍为正则启发式(手写产物),不保证 100% claim 召回,根治方向=产物只由渲染原语单向生成;policy statement 全文指纹不在本切片;M5 WriteGate 接线预订类写工具时复审(归 [#136](https://github.com/Danceiny/gotry/issues/136)/[#231](https://github.com/Danceiny/gotry/issues/231))。代码依据:`ts/src/artifact-gate.ts`/`ts/src/bookable-facts.ts`;run-all §39。(公开追踪 = [#273](https://github.com/Danceiny/gotry/issues/273)/[#299](https://github.com/Danceiny/gotry/issues/299)/[#300](https://github.com/Danceiny/gotry/issues/300)/[#301](https://github.com/Danceiny/gotry/issues/301)) |
...
| D-39 地面接驳数据边界 | 第一切片仅涵盖显式坐标驾车路线、公开 `map_driving_route` 边界、具名静态 `taxi` transfer 的分钟覆盖与静态价格回退；`bus`/`bus_plus_taxi`、live traffic、transit/rail、fare、地址解析和更广泛的 transfer 组合仍未纳入。该切片不改变 #20/M5/M6 gate；后续边界按 [#341](https://github.com/Danceiny/gotry/issues/341) 追踪。 |

> **D-26 current update (#355, 2026-09-10)**:the 12306 session deferred note above is superseded by the typed parser/outcome, invocation-bound batch identity and route/date, `captured_at_non_future_and_max_age_15m`, and closed-set seat-availability contract now wired to `session:12306-train` facts and the existing train anchor/gate. This remains deterministic fixture evidence only; it does not prove live 12306 availability, prices, or M4/M5/M6 admission.
