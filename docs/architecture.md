...
- M4→M6 program 计划已落为 issue #225 living 任务图与 M5 WriteGate proposal;它只排依赖与验收,不改变 M4/M5/M6 Entry/Exit;#227/#242 未闭合前相关 PR 保持 draft。
...
- **状态权威 = 单文件 SQLite 账本**(ADR-15,`ts/src/state-ledger.ts`):events append-only + 投影 fold 可重建 + 红线进事务 + 工单 durable 恢复(exactly-once)+ pending_writes saga。旧 JSON/JSONL 降级为单向导出视图。`state-cli` 操作面集中严格解析 cmd/positional/`--state-root`/`--tenant`/`--limit`,未知/重复/缺值/非法 numeric 在触碰账本前 fail-closed;#241/PR243 补 `.5`/`+.5` 小数边界且待主线集成;`--tenant` 只是 scope,不是认证授权。
...
| `ts/scripts/state-cli.ts` | **账本操作面**(ADR-15):migrate/log/stats/rebuild/rewind/forget/pw-* 走集中 parser 后按 tenant scope 传给 StateLedger;未知/重复/缺值/非法 numeric fail-closed。#241/PR243 补 `.5`/`+.5` 小数边界且待主线集成。export/tick/whatif 明确 local-only:分别会写共享 legacy 文件名、调用本地异步结算、生成整库管理员 snapshot(不是租户 export),非 local 在 mkdir/openDb/solve/write 前拒绝 | ✅ run-all §29;TODO #241 主线集成 |
...
- **state-cli 租户参数与未隔离命令边界(2026-09-08,issue #226/#241)**:账本 CLI 的 `rootOf`/`tenantOf`/业务 positional 串位收敛为一次集中解析;`--state-root`/`--tenant`/`--limit` 可前后等价,未知/重复/缺值/非法 numeric 在触碰 state root 前拒绝。`tick`/`export`/`whatif` 明确 local-only:非 local 不再可能调用默认 local 异步结算、覆盖共享 legacy 文件名,或把整库 snapshot 冒充租户导出;`--tenant` 仍只是账本 scope,不是认证授权。#241/PR243(606d1d2)补 CLI `.5`/`+.5` 小数边界,当前 main 的 #236 尚无该补丁,最终集成需主线复验。
...
| D-33 M4→M6 program 证据采集与 gate 闭合缺口 | issue #225 已把任务图落入 `docs/design/milestone-delivery-plan.md`;当前 open 项:① #227 Node24 Z3 间歇 heap corruption 阻断合并;② #242 dsh-map-tools map 回归(独立于 #227);③ #241/PR243(606d1d2)CLI `.5`/`+.5` 小数边界待主线集成,当前 main #236 尚无;④ #223 固定 M4 scorer N≥5/≥50% 与 HMAC/strict schema;⑤ #228 显式 opt-in planning lifecycle 本地采集+脱敏导出(现无采集器,不得用历史 wish 日志伪造 cohort);⑥ 真实 N≥5 repeat cohort 与 reflux baseline;⑦ M5 首供应链 `hotelbyte-cli` 的协议/Buyer/路由/对账/UAT 核验(协议尚未取得签署/授权证据);⑧ P6 founder review 与试点商业条件;实现 issue #231/#232/#233/#234/#235。tenant ledger scope(#229)+ fold 回归(#237)+ state-cli 租户边界(#236)代码已入 main,ledger 验收 #230 已关闭,但最终集成验收仍等 #227/#242 与新 SHA 全栈证据。M5 proposal 见 `docs/design/write-gate-production-design.md`;未满足前不启封交易/B2B 实现。 |
...
| D-32 state-cli 租户参数串位与未隔离命令扩面 | **核心代码已入 main 2026-09-08(issue #226),最终集成 TODO**:集中 parser 剥离 flag 值,拒绝未知/重复/缺值/非法 numeric;`tick`/`export`/`whatif` 对非 local 在 mkdir/openDb/solve/write 前 fail-closed;跨进程 §29 覆盖零目录创建、root hash 不变、合法顺序等价与 tenant scope 读面。#241/PR243(606d1d2)补 CLI `.5`/`+.5` 小数边界,当前 main 的 #236 尚无该补丁;待主线合入后以新 SHA 复验 |
