...
- **政策锚点全字段内容指纹(issue [#359](https://github.com/Danceiny/gotry/issues/359),父 #273,D-26 残余收口)**:`policyCanonicalBody(p, tripStart?)` 是政策渲染行的单一权威面;闸侧 anchored policy 行(去锚点段)直接全文比对 canonical body —— `subject`/`statement`/`source`/`fetched_at`/`query_id`/`as_of` 任一改动 → `fact_anchor_unknown` fail-closed,与未知锚点同源。legitimate `review_by`(事实自带)与 `tripStart` 派生的复核提醒两种合法形态保留,闸侧 `opts.tripStart` 与渲染同一传入即一致;`renderPolicyFact`/`policyCanonicalBody` 共用同一构造器,不复制拼装逻辑。fact-gate-tests §14(14 断言,canonical + 5 failing-before + 2 legitimate + 多行归属);isolated E2E `scripts/policy-anchor-359-e2e.ts` 走真实事实日志(`appendFacts` → `loadFactRegistry`)并以 registered fact-gate definition 验证篡改行 fail-closed,不启动 dsh 宿主、不触碰真实供应商/browser/policy service/共享状态。残余:反向抽取仍为正则启发式、价格容忍度/汇率/多币种与 M5 WriteGate 按既有债务复审。
...
- **事实闸覆盖面·政策锚点全字段内容指纹(2026-09-10,issue [#359](https://github.com/Danceiny/gotry/issues/359),父 [#273](https://github.com/Danceiny/gotry/issues/273),D-26 残余收口切片)**:①`policyCanonicalBody(p, tripStart?)` 与 `renderPolicyFact` 共用同一构造逻辑,作为政策行 canonical body 的单一权威面;闸侧 anchored policy 行(去锚点段)全文比对 canonical body —— `subject`/`statement`/`source`/`fetched_at`/`query_id`/`as_of` 任一改动 → `fact_anchor_unknown` fail-closed,与未知锚点同源。②不靠宽松 substring 匹配,改 `subject` 保留 fact_id+as_of、改 `statement`/相反 `statement`、改 source/fetched_at/query_id 任一均被闸拒绝,产物不得冒充「已验证方案」。③legitimate `review_by`(事实自带)与 `tripStart` 派生的复核提醒两种合法形态保留,`opts.tripStart` 与 `renderPolicyFact(p, tripStart)` 同一传入即一致;不通过删除提醒或删除正例得到 green。fact-gate-tests §14 14 断言(canonical + 5 failing-before + 2 legitimate + 多行归属);isolated E2E `scripts/policy-anchor-359-e2e.ts` 走真实事实日志(`appendFacts` → `loadFactRegistry`)并以 registered fact-gate definition 验证篡改行 fail-closed,不启动 dsh 宿主、不触碰真实供应商/browser/policy service/共享状态。父 #273 仍 open,本切片为 D-26 残余收口的进一步收口,不关闭 #273 全部范围;确定性 synthetic / isolated evidence,不宣称 live policy、supplier、M5/M6 准入。
...
| D-26 事实闸覆盖面缺口(ADR-19) | **部分收口**:酒店、政策、机/火价格与非关键词住宿/有限政策词集已按上列切片入闸;**#299/#355** 已完成机票/车次 claim 分流与 `gotry_session_search kind=train` typed rail facts:parser 区分 empty/nonempty/malformed/transport,混合畸形行 fail-closed,正事实以 host-captured invocation route/date/batch 与精确 response URL binding 为准,行内 startTrainDate 仅校验并保留、非未来且不超过 15 分钟的采集时间、`canWebBuy=Y` 与闭集可用座位 token,列表不补票价,仅 genuine empty 生成负事实;**#359** 已将 anchored policy 行内容指纹从「仅核对 as_of」扩到 `subject`/`statement`/`source`/`fetched_at`/`query_id`/`as_of` 全字段,改写/删除任意字段 → `fact_anchor_unknown`,legitimate `review_by` 与 `tripStart` 派生提醒两种合法形态保留。现有未知锚点仍 fail-closed;反向抽取仍为正则启发式,价格容忍度/汇率/多币种策略与 M5 WriteGate 仍按既有债务复审。本条只记录隔离 deterministic fixture 与本地 gate 证据,不代表 live 12306/供应商库存或价格,也不代表 M4/M5/M6 准入或 D-26 全量关闭(公开追踪 = [#273](https://github.com/Danceiny/gotry/issues/273)/[#299](https://github.com/Danceiny/gotry/issues/299)/[#355](https://github.com/Danceiny/gotry/issues/355)/[#359](https://github.com/Danceiny/gotry/issues/359)) |
