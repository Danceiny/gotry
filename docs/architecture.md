...
- Agent 工具循环的预算闸已从「步数」换为「用户主观时间」(ADR-20):产品路径(`apply()` 默认分支)**不装** per-turn 闸——`ts/src/turn-deadline.ts` 的 `installTurnDeadline` 默认 `enabled:true`,但产品路径显式不调用,真实用户的会话以 wall-clock 与 S5 异步回访为唯一终止条件。Benchmark opt-in 路径(`benchmarkEnvironmentConfigPath` 非空)才装闸,默认软阈 60s / 硬阈 120s,可通过 `GOTRY_TURN_DEADLINE_SOFT_MS` / `GOTRY_TURN_DEADLINE_HARD_MS` 覆盖；软阈之后注入 `TURN_DEADLINE_SOFT` 收敛上下文,硬阈之后经 execute waterfall 返回 `error.info.code=TURN_DEADLINE_EXHAUSTED` 且 body 零执行,再在该批 `step/end` 隐藏 agent 继承的工具 schema,让下一次 native 请求进入 text-only final。延迟到 `step/end` 是必要的:若硬阈即改 live registry,同批后续调用会在预算 waterfall 前退化成不可归因的 `unknown tool`。无 agent 的程序化调用不计数,turn/session 终态释放状态与限制。CI 先打当前 SHA 的 tarball、在隔离 pnpm consumer 中解析 dsh peer closure,再把该安装入口交给同一 E2E,不借 root 开发树冒充发布形态。
...
| `ts/src/turn-deadline.ts` | Agent 每轮 wall-clock 预算:产品路径不装(默认 `apply()` 不调用 `installTurnDeadline`);benchmark opt-in 装,默认 60s 软收敛/120s 硬上限,可由 `GOTRY_TURN_DEADLINE_SOFT_MS` / `_HARD_MS` 覆盖;同 step 超硬阈结构化拒绝、下一步 native 工具 schema 抑制 | ✅ run-all §45 |
...
| 24 | per-turn 预算闸的产品-评测分流:产品路径不装 turn-deadline、benchmark opt-in 装 wall-clock 闸 | 见 §8.24 | 真实用户出现「等太久没拿到答复」的可观测证据时,先复审 S5 异步回访是否在该路径生效;评测端出现 60s 软阈太紧导致 ground-truth 任务被提前打断时,先调 env 覆盖,再考虑软阈提升 | `ts/src/turn-deadline.ts`;`ts/src/index.ts` 默认 `apply()` 不装;`ts/scripts/agent-planning-turn-deadline-{tests,e2e}.ts`;`scripts/run-all-tests.sh` §45 |
...
#### 8.24 ADR-24：per-turn 预算闸的产品-评测分流（turn-deadline）
原 `ts/src/tool-budget.ts` 是「工具步数」闸：16 次软收敛、18 次硬上限、同一步第 19 次起结构化拒绝。轨迹证据显示该闸对真实用户体验是负向的——`TOOL_BUDGET_EXHAUSTED` 作为 tool result 回灌，模型在同一 step 内继续派发 3+ 次工具调用直至 token 用尽，用户拿不到最终答复。复盘认定：「步数/token」是**评测面**的闸（与 Round 7 minimal kernel 中的 model override、唯一 bridge、isolation/conformance 同源），不是产品面；产品面唯一合理的预算是**用户主观时间**（wall-clock）+ S5 异步回访（已有的「一小时后回来看看」）。

迁移规则：
- `ts/src/tool-budget.ts` 整文件废弃为 `ts/src/turn-deadline.ts`，语义从步数换为 wall-clock（默认 60s 软阈 / 120s 硬阈，可由 `GOTRY_TURN_DEADLINE_SOFT_MS` / `_HARD_MS` 覆盖）。enterFinalOnly（超阈后该批 `step/end` 抑制继承工具 schema，下一次 native 请求 text-only）原样保留，因为它是结构性闸，不是步数闸。
- `installTurnDeadline(ctx, { enabled })` 提供显式 opt-out；产品路径 `apply()` 默认分支**不调用**，benchmark opt-in 路径才调用。这是 ADR 的核心不变量：产品用户不再被评测闸误伤。
- `run-all §45` 的 Cordis integration / 离线 dsh headless E2E 改名 `agent-planning-turn-deadline-{tests,e2e}.ts`，断言集合扩展为：(a) `enabled: false` 时 `ctx.on` 不被订阅；(b) wall-clock 推进至 softMs 注入 `TURN_DEADLINE_SOFT`；(c) 推进至 hardMs 返回 `TURN_DEADLINE_EXHAUSTED` 结构化拒绝；(d) `step/end` 触发 schema 抑制。
- 复审触发：真实用户出现「等太久没拿到答复」的可观测证据时先看 S5 异步回访是否生效；评测端 60s 太紧时先调 env 覆盖再考虑阈值上调。

明确拒绝的备选：保留原步数闸作为产品约束（在轨迹失败面前这条不再成立）；把阈值交给 dsh 宿主动态传（仓内依赖引入宿主契约、违反 gotry/dsh 分层纪律）；不区分产品/评测让用户也能配置阈值（评测面泄露到产品面、违背 ADR-23 类「最小可见面」原则）。
...
- **Evaluation → Agent 优化 Round 1(Discussion #78,ChinaTravel canary 反馈)**:冻结 grounding-v3 canary 的首例终态通过,第二例暴露 planner 在重复工具调用中超过 300s；因此不生成 5-query 聚合。GoTry 侧把收敛边界下沉到 `tools/execute`:16 次软提示、18 次 body 上限、同一步第 19 次起结构化拒绝,并在 `step/end` 让下一步 native request text-only。Round 1 treatment-attested installed GoTry 已实际运行，exact DeepSeek 因 environment unavailable/schema-invalid 得 0，GLM 为 300s timeout。run-all §45 同时覆盖 Cordis integration/并行/生命周期与真实 `bin/gotry-inner.js → dist → dsh headless` 离线 E2E(18 个正常结果、1 个结构化拒绝、下一请求零工具 schema、非空 final)；CI 对当前 SHA 打包并在隔离 pnpm consumer 中安装,由 `GOTRY_BUDGET_E2E_BIN` 重放同一链路。frozen ChinaTravel treatment 与 5-query 恢复仍是 D-28 外部验收,离线 runtime E2E 不替代 benchmark evidence。**Round 1 后续(ADR-24)**:真实轨迹(dh 用户安排行程)显示「步数预算」对产品用户体验是负向的——`TOOL_BUDGET_EXHAUSTED` 作为 tool result 回灌后模型在同一 step 内继续派发 3+ 次工具调用、token 截断、用户拿不到答复。闸语义从「步数」换为「wall-clock」(默认 60s/120s,可由 env 覆盖);产品路径(`apply()` 默认分支)不再装闸,benchmark opt-in 路径才装——产品用户不再被评测闸误伤。run-all §45 改名 `agent-planning-turn-deadline-{tests,e2e}.ts`,断言集合加上 `enabled: false` 时 `ctx.on` 不被订阅的产品路径不变量。
