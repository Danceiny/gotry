...
- **事务化状态基座落地(2026-08-28,ADR-15)**:业界调研(loopx/DBOS/Temporal/Restate/LangGraph/Letta/Claude Code transcript 学派/SQLite durable 学派/TigerFS/SagaLLM/Cockroach 七失效模式)收敛五件套(append-only 账本/投影/约束即红线/步骤日志/saga 补偿)→ `transactional-state-rfc.md` 立例、founder 当日 accepted(「按你的建议来」,D1-D5 全按建议结算)。TS-0..TS-4 一次落地:`state-ledger.ts` 账本 + 五状态工具写路径接线 + brief/nudge/metrics 读路径回退兼容 + `state-cli` 操作面 + durable 工单崩溃恢复 exactly-once(§28 子进程 exit 9 实证)+ pending_writes saga;run-all §28(44 断言,含双形态)/§29(14 断言),全栈 §1-§29 ALL GREEN。TS-5 触发式后置=D-15。**同日 ADR-16 双形态冻结**:本地+Web 一套账本语义,tenant_id 一等字段(schema v2),同步=事件复制非状态翻译;§28 双形态断言(同库双租户互不串)。
- **工具可用性硬化(2026-08-29,Issue #24 修复)**:npm 形态用户轨迹暴露三工具「不可用」观感——①weather:「普吉岛」GeoNames 无条目致地理编码失败,新增 `resolvePlace` 别名阶梯(产品词表别名优先→原样→去后缀+population≥1000 防同名村假阳性);②flyai:Sentinel 限流输出(合法 JSON 无 data.itemList)被吞成 0/0 静默 miss,改判 error+限流标注,summary 三分支并显式提示过去日期;③hbcli:ENOENT 降级原因人话化,静态包 UI 计数修复(对象形状此前恒显「无结果」)。新增离线 flyai 套件(run-all §7b)+weather §6/hbcli §6 断言。**同日追加外部依赖自举(founder 指令「安装 gotry 时按上游本身的方式装好外部依赖」)**:hotelbyte-cli 经查为公开仓(此前「内部仓无分发渠道」判断有误,D-19 同日改口径)——`bin/gotry-bootstrap.js` 以官方方式补装可选外部依赖(hbcli 官方 install.sh / agent-reach 官方 pip 入包内 .venv / flyai 免装),npm postinstall 自动跑(--auto 非致命:CI/GOTRY_SETUP_SKIP 跳过),`gotry setup` 手动入口,hbcli.ts 候选路径回退(~/.local/bin、~/.staicli/current);bootstrap-tests 3 断言(run-all §7c)。
