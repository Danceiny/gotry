...
| D-12 loopx RFC 映射升级四接缝 | **RFC accepted(2026-08-27 founder「按建议执行」)**;S1 已落地(`tool-packet.ts` 平铺 observation envelope + interpretArgs,ADR-13);**S2+S3 已落地同日**(`memory-utility.ts` 效用 sidecar + wish_id/muted + `gotry_wish_pool_list` 0..1 召回,ADR-14);S4 WriteGate L0-L4 词汇留 M5 前;多用户 AaaS 方向见 RFC §6.5 远期采纳面 |
