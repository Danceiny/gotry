...
| D-29 | Booking Copilot lacks real-inventory product acceptance. Engineering contracts, fixtures, and reproducible artifacts do not substitute for this journey. | Freeze exact GoTry, hotel-be, and hotel-fe SHAs; exercise tenant, customer, storefront, and payment-link; recover at least one unavailable/changed quote through re-search, a new CheckAvail, and the original Checkout; keep `Book` in Checkout; reconcile unknown outcomes through QueryOrders and capture cleanup evidence. Product tracking: [#142](https://github.com/Danceiny/gotry/issues/142). |
