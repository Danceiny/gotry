...
| D-30 检索工具 typed 参数契约迁移部分完成(ADR-25 配套;2026-09-04 两刀已迁 flyai/session/hotel/weather 四个高流量工具,余其余工具) | 普通模型性能的最大单项杠杆(tool-orchestration-design.md §4③):高流量工具参数面曾是无类型 `query: json` blob,模型看不到逐字段 schema——dsh `defineTool` 原生支持 typed ParameterSchemaSpec(object/properties/enum/required + execute 前 validateArgs),blob 是本仓的选择不是上游限制。**已迁**:flyai(七字段,kind required 宿主权即拒)+ session(十字段;无公共 required,保留 interpretArgs 容忍层)+ hotel(weather)——迁移锁 smoke §12/channel-registry-tests §8。**余量**:其余工具逐刀迁移;每个首批工具一轮普通模型 canary(flyai canary 待跑,跑前不合发布面);`interpretArgs` 留作旧形态容忍层 | `ts/src/index.ts` 参数面;dsh-tools schema;issue #102 同模式 |
