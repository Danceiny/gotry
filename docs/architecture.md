...
- **指标面板第一切片(2026-09-05,issue #138,ADR-11 质量层工程面)**:`ts/scripts/build-metrics-report.ts` 只读聚合既有落盘侧车(不新增数据源)——事实闸 verdict 分布与 blocked 率(bookable-facts.jsonl,同 fact_id 取最新)/通道健康 down·cooldown(channel-health.jsonl,与 doctor 持久面同口径 30 天窗)/事故面(incidents.jsonl,7 天窗)/桥延迟百分位与超预算计数(bridge-latency.jsonl,>500ms=复审触发锚点)/账本与 doctor 报告存在性→单一 markdown;坏行跳过同侧车纪律,空根成型,collect 全程零写入(测试以目录条目集合不变断言);评测三层运行结果仍由 run-all-tests.sh/CI 承载,v1 不重跑评测。持续观测/可视化面留后续切片(metrics-report-tests §1-6 全绿,run-all §51)
...
- **typed 参数契约迁移(2026-09-04,issue #112,D-30,五刀收官)**:第一刀:最高流量工具 `gotry_flyai_search` 参数面从 `query:{type:'json'}` blob 迁 dsh typed ParameterSchemaSpec——平铺七字段(`kind` enum flight|train|hotel required + from/to/date/checkIn/checkOut/keyWords),`parameterSchemaSpecToJsonSchema` 投影模型可见逐字段 JSON Schema,`validateArgs` 在 execute 前宿主权校验;畸形参数(缺 kind/枚举外值/legacy blob 包裹)入口即结构化拒绝,ToolArgsError 经 guardToolExecute 保持 ADR-13 ToolFailure 形状(smoke §12 + channel-registry-tests §8 双迁移锁)。第二刀:`gotry_session_search`(十字段平铺)/`gotry_hotel_search`/`gotry_weather_check`——session 三意图无公共 required 字段,根 schema 开放无法宿主权拒 blob,按设计 §4③ 保留 `interpretArgs` 容忍层(blob 归一后照走业务闸,smoke 迁移锁钉死);skills-contract 契约对齐。第三刀:七个模型高频工具(session_login/doctor/artifacts_list/fact_gate/agent_reach/turn_handoff_list/artifacts_read;fact_gate 的 itinerary 保 object+additionalProperties:true,收紧归 #118 D-26;agent_reach 的 action 不设 enum——反射桥 D-4a' 开放面)。第四刀:`query:` blob 清零(wish_pool/trip_log/flight_verify/anything/web/video/github;trip_log 的 evidence P0 红线进 schema;web_search 字符串 blob 退役)。第五刀:最后三个 payload 形态——`gotry_feasibility_check`(payload 结构化对象,嵌套权威在 unified 引擎契约不越层强约束)/`gotry_motivation_save`(profile 闭合对象,**evidence P0 反伪造红线进嵌套 schema 由宿主权闸**,smoke §1 断言适配)/`gotry_companion_save`(companion 闭合对象 label/evidence required,additionalProperties:false 与负面清单同向)。**23/23 注册工具全部 typed 契约化**。条件必填仍由 execute 结构化报错给修正方向;**刀法总纲:有 required 字段的工具宿主权拒 blob,全可选工具保留容忍层**。余量收口:**普通模型 canary 已跑**(2026-09-04,`ts/scripts/typed-contract-canary.ts` 运行器——真实 LLM(MiniMax-M2.7)× 投影后工具 JSON Schema × 强制 tool_choice,10/10 一次成型 100%;二级观察:无锚点卡时模型裸猜年份 2024/2025,由既有时间锚点卡+过去日期闸在真实会话纠正,非 D-30 缺陷),**D-30 全面清偿**。
...
| D-30 检索工具 typed 参数契约迁移(ADR-25 配套) | **已清偿**(2026-09-04,issue #112 五刀:23/23 工具 typed 契约化,evidence 类 P0 红线进 schema;普通模型 canary MiniMax-M2.7 10/10 一次成型,`typed-contract-canary.ts` 运行器入仓供后续契约迁移复用) |
