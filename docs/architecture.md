...
- **FlyAI transport malformed 响应闸(issue #352)**:`gotry_flyai_search` 对精确空 `data.itemList: []` 保持业务 `miss`;非空 transport 列表经 typed 条目解析后若零有效项,返回 `via=flyai-error` 的结构化 `error` 与 `0/N valid typed transport entries` evidence,从而不进入 `factsFromFlyai` 的负事实分支;有效混合响应保持 `hit` 并暴露 `n/N`。`flyai-tests.ts` 以隔离假 CLI 覆盖 production effect、事实侧车、空列表、命中、混合、退出失败与 429；这只是本地离线工程证据,不构成真实 provider/UAT 或 issue closure。
...
- **#299 机票/车次 claim 分流(2026-09-10,D-26/#273 残余)**:`extractClaims` 将 G/D/C/Z + 3–4 位车次收进独立 `trains`,完整 token 判定不误删合法双字母航司(含 `CZ8582`/`9C8781`);`flightClaimVerdict` 只看 `kind:'flight'`,`railClaimVerdict` 只看现有 FlyAI 真实工具路径注册的结构化 `kind:'train'` exact-date/route/code/bookable 事实,缺证据输出 `rail_claim_unverified`。FlyAI train hit/miss 继续沿 `gotry_flyai_search` → `factsFromFlyai` → `appendFacts` → typed anchor/`gotry_fact_gate`;12306 session 目前仅保留公开查询列表,其 parser outcome + seat availability/freshness contract 未满足前不接 fact log,不把 malformed/empty/error 变成负事实。列表无票价,不新增 RailFact 渲染器或 static-schedule 降级。当前仍只有确定性 adapter/fixture 证据,不宣称真实供应商可售性或关闭 #299/#273。
...
- **#352 FlyAI transport malformed 响应隔离修复(2026-09-10)**:精确空 `data.itemList: []` 保持 `miss`;非空列表全无有效 typed transport 条目返回结构化 `error`,不生成负库存事实;混合响应保留有效 sibling 并以 `n/N` evidence 暴露原始比例。`flyai-tests.ts` 以 production effect、隔离假 CLI 与事实侧车覆盖该边界及既有 429/退出失败语义;这是离线工程证据,不满足真实 provider/UAT 或 issue closure。
