...
- **检索类**:酒店(`gotry_hotel_search` hbcli 桥 + **会话面 ctrip-hotel**:用户登录态真实价,被动嗅探;**酒店日期输入闸(issue #283)**=缺/空/单侧/非字符串/无法解析/倒序/同日日期 → `verdict=input_required` 不发 hbcli 不写延迟日志,通过闸才进 HBCLI_HOTEL_SEARCH 解译层与 `--check-in/--check-out` 旗标,详见 §9/§10、纯函数 `ts/src/hotel-date-gate.ts`)、天气、Anything 通用搜索、网页、视频字幕、GitHub、飞猪官方检索(机/火/酒)、会话检索(机/酒/火;火车=12306 公开查询面)
...
- **酒店日期输入闸(2026-09-09,issue #283)**:founder 截图实证——`gotry_hotel_search` 入参 `checkIn`/`checkOut` 为空字符串时仍发起 hbcli,上游按当前窗口价返回(用户要下周五给的是今天的价)。新增 `ts/src/hotel-date-gate.ts` `evaluateHotelStayDates`(纯函数,复用既有 `buildTimeAnchor`/`resolveSlotDate`,不动时间锚点/slot-spec/其它工具/dsh 依赖/planner/incident observer):缺/空/单侧/非字符串/无法解析/倒序/同日任一命中 → 直接返回 `verdict=input_required` 配 `missing`/`reason`/`raw`/`message`/`evidence` 五字段一致,**不调用 `interpretEffect`、不发 hbcli、不写 bridge-latency、不触发 ensureStateDir**(no spawn);闸通过路径进入既有 HBCLI_HOTEL_SEARCH 效应解译层与日期旗标 `--check-in/--check-out`,有效自然表达(`下周五`/`8.20`/`本周五+3天`/`明天`)经 slot-spec 解析进 fixture CLI 的精确 argv;既有错误降级(降级到静态包、不抛错、证据链标注)、`HBCLI_HOTEL_SEARCH` 注册表行与断路/重试策略原样保留。presentResult 闸失败渲染为「酒店:<dest> 缺日期(<reason>)」行动卡(summary/message 直传),不退回旧版「无结果」空卡。13 断言维护测试进入 `ts/scripts/hotel-date-gate-tests.ts`(隔离 stateRoot + 临时 fixture hbcli,finally 中清理,无 HotelByte/凭证/共享用户数据写入);run-all §30d。该修复只闭合语义缺口,不构成 M5 写闸启封、不变 Booking 库存业务效果,问题面归 D-29。
...
- **启动一次性 doctor 摘要(2026-09-04,issue #114,L2)**:`npx gotry web`/headless 启动时(bin/gotry-inner.js 在 spawn dsh 前)以分离子进程后台跑只读体检(`gotry-bootstrap.js doctor --summary`)——有待处理项打一行 stderr,全 ok 静默零输出;零写盘、detached+unref 不阻塞启动、恒 exit 0、benchmark 面豁免。「初始化时可见」取代「会话中段撞错」;bootstrap-tests §10 钉行为。
...
| D-36 酒店日期闸缺位(hotel-date-gate) | **已清偿 2026-09-09(issue #283)**:founder 截图实证 `gotry_hotel_search` 收到空/缺日期仍派 hbcli 按当前窗口价返回,语义丢失红线。新增纯函数 `ts/src/hotel-date-gate.ts` `evaluateHotelStayDates`(复用既有 `buildTimeAnchor`/`resolveSlotDate`,不动时间锚点/slot-spec/其它工具/dsh 依赖/planner/incident observer);闸失败路径直接 `verdict=input_required` 配 `missing`/`reason`/`raw`/`message`/`evidence` 五字段一致,不调 `interpretEffect`、不发 hbcli、不写 bridge-latency、不触发 ensureStateDir,通过闸才进 HBCLI_HOTEL_SEARCH 解译层与 `--check-in/--check-out` 旗标。presentResult 闸失败路径渲染为「酒店:<dest> 缺日期(<reason>)」行动卡(summary/message 直传),不退回旧版「无结果」空卡。维护测试 `ts/scripts/hotel-date-gate-tests.ts` 13 断言覆盖空/完全缺失/单侧(双向)/混合缺失-空白/倒序/同日/无法解析/类型错误/有效日期 argv/自然语言解析(口径同 slot-spec)/锚点卡词表(跨月用 Date 自动进位)/bridge-latency before-after 零增量/+1 行 增量/隔离用户状态/空白串/presentResult 行动卡;run-all §30d。该工程闸只闭合语义缺口,不启封 M5 写闸、不变 Booking 库存业务效果(D-29)。 |
