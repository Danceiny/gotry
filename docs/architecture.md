...
| D-30 检索工具 typed 参数契约迁移部分完成(ADR-25 配套;2026-09-04 三刀已迁 11 工具,余 trip_log/anything/web/github/video/wish/motivation/companion 等) | 普通模型性能的最大单项杠杆(tool-orchestration-design.md §4③):高流量工具参数面曾是无类型 `query: json` blob,模型看不到逐字段 schema——dsh `defineTool` 原生支持 typed ParameterSchemaSpec(object/properties/enum/required + execute 前 validateArgs),blob 是本仓的选择不是上游限制。**已迁**(刀法总纲:有 required 字段的工具宿主权拒 blob,全可选工具保留 interpretArgs 容忍层):flyai/session/hotel/weather 四高流量 + session_login/doctor/artifacts_list/artifacts_read/fact_gate/agent_reach/turn_handoff_list 七高频——迁移锁 smoke §12/channel-registry-tests §8。**余量**:其余工具逐刀迁移;每个首批工具一轮普通模型 canary(flyai canary 待跑,跑前不合发布面);fact_gate 的 itinerary 嵌套收紧归 #118 D-26 | `ts/src/index.ts` 参数面;dsh-tools schema;issue #102 同模式 |
