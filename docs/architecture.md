...
### 外部 benchmark 泛化(Round 1–12,Discussion #78,截至 Round 12 全部 diagnostic-only)
...
- **现状**:ChinaTravel grounding-v3 的冻结 5-query canary 只完成 1 个可评分终态,第二例在 planner 重复工具调用中触发 300s timeout——当前没有合法 5-query 聚合。Round 1–7 逐轮事实见 §9「外部 benchmark 泛化」;全部 frozen treatment 均为 diagnostic-only、official scores null。
