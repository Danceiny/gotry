...
- **Evaluation → Typed benchmark tool contract (Round 8)**：bridge v3 以单一深冻结 descriptor 派生模型可见 native schema 与 spawn 前校验，closed input schema 明示 required/type/enum，adapter output 只接受 exact versioned ok 或已声明 miss/error tuple。domain outcome 是 exit-zero transport success，模型可依据固定 recovery enum 修正参数再调用；bridge 本身不自动 retry，非零退出仍是 infrastructure failure。source synthetic E2E 已覆盖 miss→模型修参→第二次调用→tagged terminal；它不是 frozen benchmark evidence。provider routing、scorer、evaluator 与默认产品路径不变，新 treatment/official score 尚未产生。
...
Round 8 以 v3 typed descriptor 清偿「模型只看见工具名/非结构化 JSON」和「domain miss 被混入 infrastructure failure」两处通用合同缺口：同一冻结 input schema 同时服务 native request 与 spawn 前 validator，exact result envelope 只允许声明过的 status/code/recovery tuple；bridge 零自动重试，模型收到 `revise_arguments` 后才能主动构造第二次调用。单元与 loopback source E2E 只证明合同可证伪，不是 ChinaTravel 或其他公开 benchmark 的 matched treatment。D-28 仍开放；provider/scorer/evaluator 未改，frozen run、official score 与 uplift 归因仍待独立 evidence。
