...
- **政策锚点全字段内容指纹(issue [#359](https://github.com/Danceiny/gotry/issues/359),父 #273,D-26 残余收口)**:渲染与闸共用单一权威构造器;锚点行必须严格等于 canonical 整行(subject/statement/source/fetched_at/query_id/as_of),任一改动 → `fact_anchor_unknown`,与未知锚点同源;锚点行不得含前/后置非空文本、缺失/重复/格式异常锚点,或「借合法锚点写相反政策」的借用形态。`opts.itinerary.trip_start` 桥接到 `opts.tripStart`,与 `review_by` 同为合法复核提醒形态。残余:反向抽取仍为正则启发式;价格容忍度、汇率/多币种与 M5 WriteGate 按既有债务复审。
