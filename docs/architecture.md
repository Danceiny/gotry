...
- **map-tools 自带 payload 交付(2026-09-08,issue #202 热修)**:`dsh-map-tools@0.5.1` 的 peer 依赖(`dsh-tools >=0.1.2-rc.1`)与 alpha.3 闭包冲突致严格 `npm ci` 失败——交付形态改为 **gotry 自带 MIT payload**(`vendor/map-tools/`,28 文件,upstream HorusJiang/dsh-map-tools v0.5.1 适配:零运行时依赖,settings API 与 alpha.3 5 参 installSection 天然匹配),ts/package.json 移除该依赖(严格解析恢复,lock 零残留),npm files 携带 payload,inner/bootstrap/doctor 三面候选链同加(成对纪律),map-payload-tests 5/5 钉 7 工具注册(run-all §55);依赖未变时区 payload 即上游原样,settings 适配实证无需改动
