...
- **Issue #2 未来年度规划窗口(2026-09-10)**:命名年份的未来意图在 `time-anchor.ts` 派生本轮 `referenceDate`;`loop.ts` 的规划循环与注册工具 `gotry_feasibility_check` 均在求解前过滤候选级/段级日期。当前年严格限制为参考日至 12 月 31 日,已结束年份不自动滚年,显式历史/回测保持原查找语义;否定过去推荐和多年份歧义不授予 historical 旁路。`time-eval-tests.ts` §6 与 `smoke.ts` 的 Issue #2 fixture 分别验证规划循环和实际 registered execute;参考日来自注入/宿主时钟,证据为隔离 fixture,不构成供应商或真实业务准入。
...
Issue #2 的未来年度窗口现由时间锚点与 planner 入口确定性执行,不新增并行时间引擎或 Python 运行时;历史/回测旁路保持原语义,因此不作为未清偿债务重复登记。
