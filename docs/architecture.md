...
| D-25 扩展商店上架未提交(一键装/自动更新仍缺,ADR-21 分发 B 轨) | 材料就绪(2026-08-30):`docs/extension-webstore-submission.md`(单一用途/权限理由/隐私披露/文案/founder 清单)+ `docs/extension-privacy.md`(隐私政策 URL)+ `scripts/package-extension.mjs` store-zip 产物;**赎回条件**:founder 注册开发者账号($5)→ 上传 zip → 粘贴材料 → 提审;过审后 wizard 增补「已装商店版跳过 dev-mode 三步」检测 |
