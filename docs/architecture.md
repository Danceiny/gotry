...
- **typed 参数契约迁移(2026-09-04,issue #112,D-30,四刀)**:第一刀:最高流量工具 `gotry_flyai_search` 参数面从 `query:{type:'json'}` blob 迁 dsh typed ParameterSchemaSpec——平铺七字段(`kind` enum flight|train|hotel required + from/to/date/checkIn/checkOut/keyWords),`parameterSchemaSpecToJsonSchema` 投影模型可见逐字段 JSON Schema,`validateArgs` 在 execute 前宿主权校验;畸形参数(缺 kind/枚举外值/legacy blob 包裹)入口即结构化拒绝,ToolArgsError 经 guardToolExecute 保持 ADR-13 ToolFailure 形状(smoke §12 + channel-registry-tests §8 双迁移锁)。第二刀:`gotry_session_search`(十字段平铺)/`gotry_hotel_search`(destination required + checkIn/checkOut/adults)/`gotry_weather_check`(place/lat/lng/mode/month/days)同刀法迁移——其中 session 三意图无公共 required 字段,根 schema 开放无法宿主权拒 blob,按设计 §4③ 保留 `interpretArgs` 容忍层(blob 归一后照走业务闸,smoke 迁移锁钉死);hotel 因 destination required 宿主权即拒;skills-contract 契约对齐(destination/checkIn/checkOut 字面量进参数描述)。第三刀:七个模型高频工具——`gotry_session_login`/`gotry_doctor`/`gotry_artifacts_list`/`gotry_fact_gate`(全字段可选,容忍层;fact_gate 的 itinerary 保 object+additionalProperties:true,嵌套形状归一在闸内,收紧归 #118 D-26)/`gotry_agent_reach`(action 不设 enum——反射桥按 D-4a' 透传上游新增渠道,gotry 零改动)/`gotry_turn_handoff_list`(容忍层)/`gotry_artifacts_read`(path required,宿主权即拒)。第四刀:余下全部 `query:` blob 工具清零——`gotry_wish_pool_list`(容忍层)/`gotry_trip_log`(destination/start/evidence required,**evidence 是 P0 溯源红线,进 schema 由宿主权闸**)/`gotry_flight_verify`(callsign)/`gotry_anything_search`(keyword+contentType enum)/`gotry_web_search`(url;字符串 blob 形态退役,smoke §6 改锁宿主权拒绝)/`gotry_video_subtitle`(url)/`gotry_github_search`(顶层 query 字符串)。条件必填仍由 execute 结构化报错给修正方向;**刀法总纲:有 required 字段的工具宿主权拒 blob,全可选工具保留容忍层**。余量:`gotry_feasibility_check`(payload)/`gotry_motivation_save`(profile)/`gotry_companion_save`(companion)三个引擎/记忆面 blob 与普通模型 canary 仍在 D-30。
...
| D-30 检索工具 typed 参数契约迁移部分完成(ADR-25 配套;2026-09-04 四刀已迁 18 工具,`query:` blob 清零,余 feasibility/motivation/companion 三个 payload 形态) | 普通模型性能的最大单项杠杆(tool-orchestration-design.md §4③):高流量工具参数面曾是无类型 `query: json` blob,模型看不到逐字段 schema——dsh `defineTool` 原生支持 typed ParameterSchemaSpec(object/properties/enum/required + execute 前 validateArgs),blob 是本仓的选择不是上游限制。**已迁**(刀法总纲:有 required 字段的工具宿主权拒 blob,全可选工具保留 interpretArgs 容忍层):四高流量 + 七高频 + wish_pool/trip_log(evidence 红线进 schema)/flight_verify/anything/web/video/github——迁移锁 smoke §12/§6/channel-registry-tests §8。**余量**:feasibility_check(payload)/motivation_save(profile)/companion_save(companion)三个引擎与记忆面 blob;普通模型 canary(flyai 待跑,跑前不合发布面);fact_gate 的 itinerary 嵌套收紧归 #118 D-26 | `ts/src/index.ts` 参数面;dsh-tools schema;issue #102 同模式 |
