...
- **事实闸价格边界(issue [#300](https://github.com/Danceiny/gotry/issues/300),父 [#273](https://github.com/Danceiny/gotry/issues/273))**:flight/train exact-date 可下单行的可比较 CNY 硬价不一致 = `price_contradicted` blocked;缺失/模糊/非数字/不同币种不猜换算,酒店 `priceRaw` 不参与。价格容忍度、汇率和多币种策略仍待 owner/founder 决策。
...
本切片补齐机/火共享原语的行内硬价边界:exact-date 事实价为 CNY 时,`¥NNN`/`CNY NNN` 的可靠不一致会产生 `price_contradicted`;起价/约价、非数字、缺失和不可比较币种不做换算或容忍度推断,酒店 `priceRaw` 保持打码语义。不关闭 [#273](https://github.com/Danceiny/gotry/issues/273) 或 D-26 全部残余。
...
| D-26 事实闸覆盖面缺口(ADR-19) | **部分收口**:酒店 claim 已入闸(2026-09-04,issue #118,HotelFact 第三形态+渲染原语锚点回溯);政策事实生产端 v1 已落(2026-09-05,issue #141,C 档领事服务网 cs.mfa.gov.cn → PolicyFact 落账,Timatic/Sherpa° 后议);政策渲染锚点闭合 + 锚点行 `as_of` 日期边界 + 海关申报关键词(2026-09-10,issue #273,`renderPolicyFact` typed-anchor + 仅核对渲染行「截至日期」与事实 `as_of` + `POLICY_WORD` 补海关申报);机/火共享原语增加可比较 CNY 硬价矛盾闸(2026-09-10,issue #300,`price_contradicted`,不含酒店 `priceRaw`)。现有未知锚点仍 fail-closed;价格容忍度、汇率/多币种策略仍待 owner/founder 拍板,本切片不宣称关闭 D-26 全量残余。残余:反向抽取仍为正则启发式(手写产物),不保证 100% claim 召回,根治方向=产物只由渲染原语单向生成;跨供应商价差/多币种策略、Rail/火车 claim 分类、度假村/青旅/别墅类非关键词住宿 category 仍记残余;M5 WriteGate 接线预订类写工具时复审(归 [#136](https://github.com/Danceiny/gotry/issues/136)/[#231](https://github.com/Danceiny/gotry/issues/231))。代码依据:`ts/src/artifact-gate.ts`;run-all §39。(公开追踪 = [#273](https://github.com/Danceiny/gotry/issues/273)/[#300](https://github.com/Danceiny/gotry/issues/300)) |
