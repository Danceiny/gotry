...
| D-26 事实闸覆盖面缺口(ADR-19) | **部分收口**:酒店 claim 已入闸(2026-09-04,issue #118,HotelFact 第三形态+渲染原语锚点回溯);政策事实生产端 v1 已落(2026-09-05,issue #141,C 档领事服务网 cs.mfa.gov.cn → PolicyFact 落账,Timatic/Sherpa° 后议);政策渲染锚点闭合 + 锚点行 `as_of` 日期边界 + 海关申报关键词(2026-09-10,issue #273,`renderPolicyFact` typed-anchor + 仅核对渲染行「截至日期」与事实 `as_of` + `POLICY_WORD` 补海关申报);机/火共享原语增加 fare role 绑定内的可比较 CNY 硬价边界(2026-09-10,issue #300,canonical/明确票价标签字段才比较,带非票价文字标签的分隔金额不参与,裸/未归属或畸形 fare money = `unverified_price_claim`,数值不一致 = `price_contradicted`,不含酒店 `priceRaw`);**非关键词住宿 category(度假村/青旅/青年旅舍/别墅/公寓)+ lodging heading 下无 token 可住断言 + 政策词 13 词有限并集(EVUS/ETA/eVisa/疫苗/疫苗接种/健康申报/隔离/工作签/居留/返程签/护照有效期/黄皮书/保险)直读入闸(2026-09-10,issues #301/#302,fact-gate-tests §12/§13,run-all §39)**。现有未知锚点仍 fail-closed;价格容忍度、汇率/多币种策略仍待 owner/founder 拍板,本切片不宣称关闭 D-26 全量残余。残余:反向抽取仍为正则启发式(手写产物),不保证 100% claim 召回,根治方向=产物只由渲染原语单向生成;跨供应商价差/多币种策略、Rail/火车 claim 分类、policy statement 全文指纹不在本切片;M5 WriteGate 接线预订类写工具时复审(归 [#136](https://github.com/Danceiny/gotry/issues/136)/[#231](https://github.com/Danceiny/gotry/issues/231))。代码依据:`ts/src/artifact-gate.ts`;run-all §39。(公开追踪 = [#273](https://github.com/Danceiny/gotry/issues/273)/[#300](https://github.com/Danceiny/gotry/issues/300)/[#301](https://github.com/Danceiny/gotry/issues/301)/[#302](https://github.com/Danceiny/gotry/issues/302)) |
