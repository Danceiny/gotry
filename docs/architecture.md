...
- 外部 benchmark:七轮 frozen treatment 全部 diagnostic-only、official scores 为 null,不声称 uplift(D-28);逐轮事实见 §9,工程合同见 `evaluation/benchmark-environment-bridge.md`。
...
- **CI 双层修复(2026-09-08,#208;main 自 #197 起全红、rc.19 系带病发布)**:①`Install ts dependencies` 红根因 = runner npm 升级后裸 `npm ci` 强制校验 peer,而 ts/package-lock.json 一直是 `--legacy-peer-deps` 模式生成(dsh 家族 peer 闭包从不物化进 ts/node_modules,运行时由锁定 dsh runtime 提供)——CI ts 步骤与 CONTRIBUTING 同步显式带该旗标,并移除 dsh-map-tools 冗余依赖(vendor 分发已覆盖 source 布局)。②typecheck 五连红(turn-deadline 的 session 事件)根因 = 类型面隐性依赖 peer 意外物化:`session/event`/`session/disposed` 声明在 dsh-session 的 cordis Events augmentation 里,pnpm 隔离布局下 root 侧同名包的 augmentation 合并进的是另一个 cordis 实例——显式 `import type` + `dsh-session@0.1.2-alpha.3` 进 ts dependencies,overrides 把其 peer 闭包递归带出的六个包钉在 alpha.3 防 rc.1 混版本;lock 全量 resolved 指向 registry.npmjs.org。

### 外部 benchmark 泛化(Round 1–10,Discussion #78,official score 仍为空)
...
- **Round 10(per-tool typed result contract hardening)**:在 Round 8 的单一 flat `tools|call|errors` 协议上，将 owner-local v3 descriptor 的 closed/bounded `input_schema` 同源投影为每工具独立 `call` 分支并在执行前复用；`output_keys` 强制非空，`domain_outcomes` 限定为 finite exact tuple。adapter 只接受 exact `gotry_benchmark_tool_result_v1`：concrete result 的每个 primitive leaf 必须位于声明键下，domain outcome 必须 exit 0，非零退出仍是 infrastructure failure；conformance 以最新 bridge response 为 tagged terminal 时序下界，domain 后失败拒绝、后续 concrete result 可恢复。该轮不改 provider、scorer、evaluator 或默认产品路径；冻结 treatment 与 score 只能由后续独立证据给出。
...
- **现状**:ChinaTravel grounding-v3 的冻结 5-query canary 只完成 1 个可评分终态,第二例在 planner 重复工具调用中触发 300s timeout——当前没有合法 5-query 聚合。Round 1–10 逐轮事实见 §9「外部 benchmark 泛化」；Round 9 只证明治理链路首次存活，Round 10 的新 frozen treatment 尚待独立执行，当前没有可归因的 official score/uplift。
...
**词表内赎回 2026-08-22**:feasibility + 酒店/天气/Anything/AgentReach 五工具 presentResult 结果卡(可行性:候选判定+预算行;酒店:N 家(实时/静态);天气:ok/降级;Anything:N hits;AgentReach:✅/🔧/📦/❌ verdict)+ 12 工具 kind 图标分类(search/fetch/execute/edit,零 other);**地图位已解 2026-08-22**:宿主插件 dsh-map-tools v0.4.4(7 个 map_* 原生工具:驾/公/步/骑路线+地理编码+POI,零 key 走 OSRM,高德可后配)。dsh-map-tools 仍是可选宿主插件:legacy vendored runtime 可提供,root/package 形态按当前依赖可解析性挂载;patch 条目占位、缺依赖整块剔除不挡启动;root `./gotry` 统一走 inner。
