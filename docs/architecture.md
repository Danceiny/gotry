...
- **事实闸覆盖面·政策渲染锚点闭合 + 海关申报关键词(2026-09-10,issue #273,D-26 残余收口切片)**:①`renderPolicyFact` 行尾内嵌 `<!-- fact:<fact_id> -->` 锚点——政策渲染原语与机/火/酒店同源 typed-anchor;产物经原语生成即自带溯源锚,闸侧锚点确定性回溯,手改/伪造锚点 = `fact_anchor_unknown`;启发式 `POLICY_WORD` 仅覆盖手写/历史产物。②锚点行内容指纹:闸侧对 anchored policy 行提取 `截至 YYYY-MM-DD` 与事实 `as_of` 比对,改写或删除日期而保留 fact_id = `fact_anchor_unknown` ——与未知锚点同源 fail-closed(hand-edited 不可信面收口)。③`POLICY_WORD` 补「海关申报」(与「入境申报」同性质但被原 regex 漏掉)——手写政策行缺 as_of → `policy_without_as_of` 直接违例。fact-gate-tests §11 九断言(65 pass,red→green)。残余:机/火车次价差、Rail/火车 claim 分类、度假村/青旅/别墅类非关键词住宿 category、policy statement 全文指纹(11g 当前闸不抓,根治方向=渲染原语只从结构化事实生成)、M5 WriteGate 接线预订类写工具时复审(归 #136/#231)。
