...
- **检索类**:酒店(hbcli 桥 + **会话面 ctrip-hotel**:用户登录态真实价,被动嗅探)、天气、Anything 通用搜索、网页、视频字幕、GitHub、飞猪官方检索(机/火/酒)、会话检索(机/酒/火;火车=12306 公开查询面)
...
- **HotelByte embedded Booking Copilot 是只读协作面，不是 M5 写闸启封**:`booking.surface.v1` 与 `booking.surface.v2` 共享同一 BFF listener、task ownership 与闭合 typed read-action profile；v2 通过 version+schema hash 逐请求 dispatch。生产 standalone 默认运行 `bff-bound-turn-only`，只接受 deployment-key 认证且已由 BFF 绑定的 `user.turn`/receipt continuation；浏览器 `user.turn.ingress` 只有在完整 authenticated principal/scope + BFF trusted binding seam 下才开放，并原子解析为服务端生成的 `taskId + turnId + contextRef + surface + allowedActions`。已呈现的 must-relaxation option 可作为同一 ingress 的单个 typed approval 原样回传；BFF 在幂等 request journal 前关闭 task/context authority mismatch，该 journal 仅是控制面幂等与审计记录；GoTry 再于 planner/operation 前关闭完整 tuple、stale、expired 与 replay，FE 于 adapter/component/page effect 前复核同一证据。bound-turn-only 模式对 ingress 在 ledger/planner 副作用前返回 typed 503；GoTry 不接受浏览器自赋身份或能力。v2 durable ledger 投影包含六个生命周期阶段、七个 phase 字面值（`terminal`/`error` 为两种终态结果），v1 保持 legacy 两态投影；must blocker 只能由绑定 task/context/source turn/action/receipt/presentation key、随机 delivery nonce 与 option digest 的一次性 approval 放行。部署候选固定 exact GoTry SHA/schema 与 Linux glibc + Node 24/ABI provenance，鉴权 health 回显实际进程 identity 与 ingress mode；四 surface 真实库存、不可订重试链与原 Checkout/订单观察仍是外部 UAT gate，未通过前不得宣称产品验收或可合并。
- py 树仅剩 `gotry_feasibility` oracle,产品运行时零 Python 依赖(D-7 清偿)。
- Phase 1 外部 benchmark bridge 默认关闭:仅接受当前 POSIX uid 持有、非 symlink、非 group/world-writable 的绝对 config,固定 executable/cwd/argv,无任意 shell；v3 config 用 `tools[]` descriptor 同时声明 name/description、闭合且有界的 `input_schema`、必填非空 `output_keys` 与有限 `domain_outcomes`。同一深冻结 schema 既投影到 native request，也在 spawn 前校验 required/type/enum/额外字段；runner 只收到最小环境，gold/oracle/expected answer/score 等键递归 fail-closed。runner 结果必须是 exact versioned envelope：`ok` 携带结构化 result，或 exit-zero 的已声明 `miss|error` code/recovery；result 中每个 primitive leaf 必须位于已声明 output key 之下，顶层 record array 可用而无键 primitive/mixed array 拒绝。非零退出、timeout、truncation 与坏 JSON 仍是 infrastructure failure。bridge 不自动重试，模型只能依据声明 recovery 自行修正后再次调用。opt-in 只允许 cold-start + headless one-shot：发现 live agent 即拒绝；每个 agent 强制 native、root-captured `gotry_benchmark_environment` definition-only，最终 prompt assembly 与 downstream `agent/pre-step` 双重重验 exact schema/definition，额外工具、同名 shadow 或 PTC transport 漂移均在模型请求前 fail-closed；局部安装自动回滚，agent disposal 释放 scoped effect；若插件先卸载，live agent 保留 fail-closed quarantine，直至 agent/process disposal，HMR 不得在旧 agent 上热挂。agent conformance 只接受配对的 exact concrete result 或 declared domain outcome，并要求 tagged terminal 严格晚于最新 bridge response；domain-only 可收敛，domain 后的 infrastructure failure 拒绝，已有 concrete result 后的 optional failure 只能由更新后的 terminal 收敛。缺 call 或坏终态最多一次模型纠偏，格式纠偏不得重复派发 bridge，第二次失约稳定 fail-closed。parent CLI 对 stdout 有界缓冲并用同一 parser 二次验收，失败正文不出门。config 的 writes forbidden/network denied 仍由 owner/host OS enforcement，synthetic E2E 不证明 OS sandbox；bridge 不携带 hidden query loader，也不把 Python harness 变成产品依赖。Round 8 不改 provider routing、scorer 或 evaluator；新 frozen treatment 尚未执行，不声称 score uplift 或 external benchmark closure。
...
- **Round 8 typed tool/result contract**：v3 owner-local config 以单一 descriptor 生成 discovery、native parameter schema、spawn 前 runtime validator、输出正向键合同与 domain tuple 校验。input schema 只接受闭合 object、required、标量 enum/const 与带 items 的 array，并对字节、深度、节点、属性和 enum 总量设硬界；未知 keyword、开放嵌套对象和 schema mutation 均 fail-closed。adapter 只接受 exact `gotry_benchmark_tool_result_v1` 的 ok/miss/error envelope；声明式 miss/error 以 exit 0 返回 transport success，非零退出仍归 infrastructure failure。恢复由模型根据有限 recovery enum 重新构造参数，不由 bridge 自动重试。该切片不改 provider/scorer/evaluator；冻结 treatment 尚未执行，无 score/uplift 结论。
...
Phase 1 外部 benchmark bridge 默认关闭：owner-local v3 config 以 `tools[]` 深冻结 descriptor 声明每个工具的 name/description、closed/bounded `input_schema`、必填非空 `output_keys` 与有限 `domain_outcomes`；同一 schema 同时进入 native request 与 spawn 前 validator，adapter 只接受 exact `gotry_benchmark_tool_result_v1` result/domain/failure envelope。conformance 将 concrete result、domain outcome 与 failure 分离：domain-only 终态可收敛，但其后的 infrastructure failure 仍拒绝，后续 concrete result 可恢复；bridge 不自动 retry。固定 direct argv、最小 runner 环境、参数/输出结构上限、递归 no-oracle 键拒绝都由产品 seam 执行；opt-in 仅接受 cold-start + headless one-shot，并把每个 agent 固定为 native、原始 bridge definition-only，最终 assembly 与 downstream pre-step 任一 surface 漂移都 fail-closed。Round 3 conformance 把 prompt 中 CLI/shell/Python 操作映射为 allowed native `query.action=call`，只有配对 result/domain 后才接受 owner-local `terminal_output` 指定的单一 tagged JSON object；缺 call 或坏终态最多一次纠偏，格式纠偏不再派发 bridge，headless parent 用同一 parser 二次验收 stdout。agent disposal 释放 scoped effect；插件先卸载时 live agent 保持 quarantine 至 agent/process disposal，HMR 不热挂。writes forbidden/network denied 仍必须由 owner/host OS enforce，synthetic E2E 不证明 OS sandbox。bridge 不携带 hidden query loader,不把 Python harness 变成产品依赖。Round 8 frozen treatment 尚未运行，provider/scorer/evaluator 与默认产品路径不变，不产生 score/uplift 或 external benchmark closure。
...
Booking Copilot 是既有工作台内的 BFF-only embedded read-action 面：v1 保持兼容，v2 使用 closed typed contract；两者由同一 listener、task ownership 与 ledger 按请求 version+schema hash dispatch。生产 standalone 的 `bff-bound-turn-only` 模式只接受已由 BFF 绑定的 internal `user.turn`/receipt continuation；v2 浏览器只提交安全 opaque `requestKey`/可选 `taskHandle`，以及用户刚从 canonical question 明确选择的至多一个 typed approval。只有完整 authenticated principal/scope + BFF trusted binding seam 才能把 `user.turn.ingress` 原子返回为服务端生成并持久绑定的 `taskId + turnId + contextRef + surface + allowedActions`；BFF 在 claim request journal 前拒绝 approval 的 task/context authority mismatch，journal claim 只承担幂等与审计，不代表 approval 有效；完整 option tuple、revision/expiry 与一次性 replay 由 GoTry 在 planner/operation 前验证，FE 在 adapter/component/page effect 前再次验证，不能靠静默重绑变成有效授权。bound-turn-only 下 ingress 在任何 ledger/planner side effect 前返回 typed 503；taskHandle 在 actor scope 内只能绑定一个 task/context。must blocker 只能经 runtime 持久化并实际呈现的 option 放行，且 approval 逐字段绑定 task、context、source turn、source action、source receipt digest、canonical presentation key、随机 delivery nonce 与 option digest，只能消费一次。
...
- **Evaluation → Typed benchmark tool contract (Round 8)**：bridge v3 以单一深冻结 descriptor 派生模型可见 native schema 与 spawn 前校验，closed input schema 明示 required/type/enum，adapter output 只接受 exact versioned ok 或已声明 miss/error tuple。`output_keys` 必填非空；每个 primitive leaf 必须位于 declared key 之下，顶层 record array 可用而无键 primitive/mixed array 拒绝。domain outcome 是 exit-zero transport success，模型可依据固定 recovery enum 修正参数再调用；bridge 本身不自动 retry，非零退出仍是 infrastructure failure。conformance 将 concrete result、domain outcome 与 failure 分离，并要求 tagged terminal 严格晚于最新 bridge response：domain-only 可收敛，domain 后的 infrastructure failure 拒绝，后续 concrete result 可恢复，旧 terminal 不能遮蔽更晚事实。source synthetic E2E 已覆盖 miss→模型修参→第二次调用→tagged terminal；它不是 frozen benchmark evidence。provider routing、scorer、evaluator 与默认产品路径不变，新 treatment/official score 尚未产生。
...
Round 8 以 v3 typed descriptor 清偿「模型只看见工具名/非结构化 JSON」和「domain miss 被混入 infrastructure failure」两处通用合同缺口：同一冻结 input schema 同时服务 native request 与 spawn 前 validator，exact result envelope 只允许声明过的 status/code/recovery tuple；非空 output-key 合同拒绝未被 declared key 覆盖的 primitive/mixed array，conformance 以最新 bridge response 作为 terminal 时序下界，旧终态不能遮蔽更晚 domain/failure。bridge 零自动重试，模型收到 `revise_arguments` 后才能主动构造第二次调用。单元与 loopback source E2E 只证明合同可证伪，不是 ChinaTravel 或其他公开 benchmark 的 matched treatment。D-28 仍开放；provider/scorer/evaluator 未改，frozen run、official score 与 uplift 归因仍待独立 evidence。
...
| D-29 Booking Copilot 真实库存产品验收 | typed read-action/BFF/task ledger、tenant/customer 一次性 relaxation approval 垂直证据与可复现 Linux 产物只证明工程边界；不证明供应商库存、不可订恢复或 Checkout/订单状态业务效果 | **open**:冻结三仓 exact SHA 后，在 tenant/customer/storefront/payment-link 四 surface 跑真实库存；至少一条 unavailable/changed 报价必须经重新搜索、显式单条件放宽、新 CheckAvail、原 Checkout 恢复；Book 仍仅由 Checkout 授权，并以 QueryOrders/清理证据收口 |
